...
# Set AEO time horizon in years (used in "value_listfinder" function below to check correctness of microsegment value lists)
aeo_years = 32

# Define a series of dicts that will translate imported JSON microsegment name to AEO microsegment(s)
...
bldgtypedict = {'single family home':1,'multi family home':2,'mobile home':3,'all/NA':''}

# Fuel type dict    
fueldict = {'electricity (on site)':'SL','electricity (grid)':'EL', \
'natural gas':'GS', \
'distillate':'DS', \
'other':('LG','KS','CL','SL','GE','NG','WD'), \
'NA':'' \
}  # Note that currently in RESDBOUT.txt, electric resistance heaters are categorized under GE (geothermal) fuel. SL (solar) fuel is attached to water heating, but the end technology is only listed as "Solar", which will not currently be included in our analysis.  


# End use dict
endusedict = {'heating':'HT','secondary heating':('SH','OA'),'cooling':'CL', \
'fans & pumps':'FF', \
'ceiling fan':'CFN', \
'lighting':'LT', \
'water heating':'HW', \
...
'cooking':'CK', \
'drying':'DR', \
'TVs':{'TV':'TV','set top box':'STB','DVD':'DVD','home theater & audio':'HTS','video game consoles':'VGC'}, \
'computers':{'desktop PC':'DPC','laptop PC':'LPC','monitors':'MON','network equipment':'NET'}, \
'other (grid electric)':{'clothes washing':'CW','dishwasher':'DW','freezers':'FZ','other MELs': ('BAT','COF','DEH','EO','FP','MCO','OA','PHP','SEC','SPA')}, \
...
'boiler (electric)':'ELEC_RAD','ASHP':'ELEC_HP','GSHP':'GEO_HP', \
'central AC':'CENT_AIR','room AC':'ROOM_AIR', \
'linear fluorescent':'LFL','general service':'GSL','reflector':'REF','external':'EXT', \
'furnace (NG)':'NG_FA','boiler (NG)':'NG_RAD','NGHP':'NG_HP', \
'furnace (distillate)':'DIST_FA','boiler (distillate)':'DIST_RAD', \
'furnace (kerosene)':'KERO_FA','furnace (LPG)':'LPG_FA','stove (wood)':'WOOD_HT','resistance':'GE2', \
'secondary heating (kerosene)':'KS','secondary heating (LPG)':'LG','secondary heating (wood)':'WD', 'secondary heating (coal)':'CL', \
'NA':'' \
}

# Technology types (demand) dict 
technology_demanddict = {'windows conduction':'WIND_COND', 'windows solar':'WIND_SOL', \
'wall': 'WALL', \
'roof': 'ROOF', \
'ground' : 'GRND', \
'infiltration' : 'INFIL', \
...
def value_listfinder_filterformat(txt_filter):
    """ Given a filtering list for a microsegment, format into a string that can be entered into a regex comparsion in value_listfinder function """
    # Set base comparefrom string
    comparefrom = '.*'
    for element in txt_filter: # Run through all elements of the filtering list  
        # If element is a tuple, join the tuple into a single string, put brackets around it for regex comparison
...
def value_listfinder(mstxt_supply, txt_filter): # Note: in the future, will need to add a third input (mstxt_demand) to provide thermal load components data
    ...
    # Check whether current microsegment is a heating/cooling "demand" technology (handled differently)
...
def value_replacer_listassemble(mstxt_supply,filterdata):
    """ Given filtering information from JSON, translate to filter info for .txt and find a list of energy values from .txt given filter """
    print filterdata
...
def value_replacer_main():
    """ Import .txt and JSON files; run through JSON objects; find analogous .txt information; replace JSON values; update JSON """
    args = sys.argv[1:]    
    txtfile = args[0]
    jsonfile = args[1]
    # Import .txt file    
    ...
    with open(jsonfile,'r') as js:
        msjson = json.load(js)
        # Run through JSON objects, determine replacement information to mine from .txt file, and make the replacement
        # Census level
                            ...
                            for techtype in tech:
                                ...
                                if heatcooltech:  
                                    ...
                                    if isinstance(endusedict[endusetype],dict):                                   
                                        # Set subendusetype to the tech type to reconcile JSON/txt difference in handling
