    ...
    def __init__(
        ...
        try:
            ...
            if "shape" in self.tsv_features.keys() and \
                ...
                # Specify TSV file location
                if opts.web:
                    query = ("SELECT load_shape_file FROM "
                             + db["schema"]
                             + ".ecm WHERE id = %s")
                    cursor.execute(query, (opts.id,))
                    content = cursor.fetchone()
                    csv_shape_file_loc = content[0]
                else:
                    csv_shape_file_loc = path.join(base_dir,
...
def prepare_packages(packages, meas_update_objs, meas_summary,
    ...
    for p in packages:
        ...
        # Specify calculation method string
        if opts.web is True:
            if opts.site_energy is True:
                energy_calc_method = "site"
            elif opts.captured_energy is True:
                energy_calc_method = "captured"
            else:
                energy_calc_method = "fossil_equivalent"
        ...
        for m in measures_to_add:
            ...
            if opts.web is True:
                # Get uncompeted data from the PostgreSQL database for
                # measure m and the current calculation method
                # and insert it into a unitary length list
                query = (
                    "SELECT uncomp_data FROM "
                    + db["schema"]
                    + ".ecm_data_view WHERE name = %s "
                    + "AND tsv_view = %s AND energy_calc_method = %s")
                cursor.execute(query, (m, opts.tsv, energy_calc_method))
                response = cursor.fetchone()
                meas_summary_data = [response[0]]
            else:
            ...
            if len(meas_summary_data) == 1:
                ...
                meas_obj.technology_type = meas_summary_data[0][
                    "technology_type"]
                if opts.web is True:
                    # Get competed data from the PostgreSQL database for
                    # measure m and the current calculation method
                    query = (
                    ...
                    # Load and set competition data for missing measure object
...
def main(base_dir):
    ...
    if opts.web is True and opts.is_tsv is True:
        regions = "EMM"
    else:
        if opts.alt_regions is True:
            input_var = 0
            # Determine the regional breakdown to use (EMM (1) vs. AIA (2))
            while input_var not in ['1', '2']:
                input_var = input(
                    "Enter 1 to use an EIA NEMS Electricity Market Module "
                    "(EMM) geographical breakdown or 2 to use an AIA climate "
                    "zone geographical breakdown: ")
    ...
    # When running on the web, if an ECM has TSV features, automatically
    # detect the current iteration of predefined TSV metrics to use
    if opts.web is True and opts.tsv:
        # Specify appropriate TSV perspective for the current call to main()
        tsv_metrics_map = {
            "summer_peak": ['2', '2', '1', '2', '2', '2'],
            "summer_off_peak": ['2', '3', '1', '2', '2', '2'],
            "winter_peak": ['2', '2', '2', '2', '2', '2'],
            "winter_off_peak": ['2', '3', '2', '2', '2', '2']}
        tsv_metrics = tsv_metrics_map[opts.tsv]
    ...
    elif opts.web is False and opts.tsv_metrics is True:
    ...
    if opts.web is True:
        ...
        query = ("SELECT name, definition FROM "
                 + db["schema"]
                 + ".ecm WHERE id = %s AND NOT is_package")
        cursor.execute(query, (opts.id,))
        content = cursor.fetchone()
        ...
        meas_toprep_indiv_names = [
            x for x in listdir(handyfiles.indiv_ecms) if
            x.endswith(".json") and 'package' not in x]

        # Import all individual measure JSONs
        for mi in meas_toprep_indiv_names:
            with open(path.join(
                    base_dir, handyfiles.indiv_ecms, mi), 'r') as jsf:
                try:
                    ...
                    # included in database of prepared measure attributes
                    ...
                    # removed the "site_energy," "captured_energy" or
                    # "tsv_metrics" cmd line arguments and the measure
                    # definition was not already prepared using these settings
                    if all([meas_dict["name"] != y["name"] for
                       ...
                       (opts is None or opts.tsv_metrics is False and
                        ...
                        # Add copies of the measure that examine multiple
                        # scenarios of public health cost data additions,
                        # assuming the measure is not already a previously
                        # prepared copy that reflects these additions (judging
                        # by name)
                        if opts is not None and opts.health_costs is True and \
    ...
    if opts.web is True:
        # Get the package ECM JSON content from the PostgreSQL database
        query = ("SELECT definition FROM " + db["schema"] +
                 ".ecm WHERE id = %s AND is_package")
        cursor.execute(query, (opts.id,))
        content = cursor.fetchone()

        # Insert JSON content into the appropriate list
        if content:
            meas_toprep_package.append(content[0])
    else:
        ...
        with open(path.join(base_dir, *handyfiles.ecm_packages), 'r') as mpk:
            try:
                ...
                raise ValueError(
                    ...
                    handyfiles.ecm_packages + "': " + str(e)) from None
        ...
        for m in meas_toprep_package_init:
            ...
            # b) the package is new, c) package does not already have
            # competition data prepared for it; or d) package "benefits"
            # and/or "contributing_ECMs" parameters have been edited from
            # a previous version
    ...
    if len(meas_toprep_indiv) > 0 or len(meas_toprep_package) > 0:
        ...
        if opts.web:
            ...
            for ind, m in enumerate(meas_prepped_objs):
                ...
                # Specify TSV perspective string, if applicable
                if opts.is_tsv:
                    tsv_method = opts.tsv
                else:
                    tsv_method = ''
                ...
                psgUpsertTable(
                    opts.id, sql_calc_mtd, tsv_method, sql_uncomp, sql_comp)
        else:
...
def psgUpsertTable(ecm_id, calc_mtd, tsv_view, uncomp, comp):
    """CHECK - add docstring"""
    try:
        query = (
            "INSERT INTO "
            + db["schema"]
            + ".ecm_data (ecm_id, energy_calc_method, tsv_view, uncomp_data, "
            "comp_data) VALUES (%s,%s,%s,%s,%s) "
            "ON CONFLICT ON CONSTRAINT "
            + db["schema"]
            + "_ecm_data_ecm_id_tsv_view_energy_calc_method_uniq "
            "DO UPDATE SET uncomp_data = EXCLUDED.uncomp_data, "
            "              comp_data = EXCLUDED.comp_data")

        cursor.execute(query, (
            ecm_id, calc_mtd, tsv_view, json.dumps(uncomp), json.dumps(comp)))
        connection.commit()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--web", action="store_true",
                        help="Flag for use in web app")
    # Optional flag for use only with --web to indicate what ECM to process
    parser.add_argument("--id",
                        help="ECM id number for the ECM to be processed")
    ...
    if opts is not None and opts.web is True:
        ...
        # Figure out if ECM has TSV features
        query = ("SELECT is_tsv FROM "
                 + db["schema"]
                 + ".ecm WHERE id = %s")
        cursor.execute(query, (opts.id,))
        response = cursor.fetchone()
        opts.is_tsv = response[0]  # Acting as a backdoor global variable

        # For all ECMs, run total annual calculations
        # Run through all three energy accounting/calculation methods
        opts.tsv = ''  # Set opts.tsv initial value of empty string
        ...
        # If ECM has TSV features, reconfigure computation for TSV
        if opts.is_tsv:
            opts.sect_shapes = True
            # Specify the TSV perspectives to auto-calculate
            tsv_views = ['summer_peak', 'summer_off_peak',
                         'winter_peak', 'winter_off_peak']

            # Set configuration for site energy only
            opts.site_energy = True
            opts.captured_energy = False

            # Complete calculations for the specified TSV perspectives
            for tsv_view in tsv_views:
                opts.tsv = tsv_view  # Acting as a backdoor global variable
                main(base_dir)
