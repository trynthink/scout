...
from os import listdir, getcwd, stat, path, walk
    ...
    def __init__(self, base_dir, handyfiles):
        ...
        with open(path.join(base_dir, *handyfiles.ss_data), "r") as ss:
            try:
                ...
                raise ValueError(
                    "Error reading in '" + handyfiles.ss_data + "': " + str(e)
                ) from None
        ...
        self.in_all_map = {
            ...
            "end_use": {
                ...
                "commercial": {
                    ...
                    "natural gas": ["cooling", "water heating", "cooking", "heating"],
                ...
                "commercial": {
                    "supply": {
                        "electricity": {
                            ...
                            "cooking": ["electric_range_oven_24x24_griddle"],
                            "PCs": [None],
                        ...
                        "natural gas": {
                            ...
                            "cooking": ["gas_range_oven_24x24_griddle"],
                            "heating": [
        ...
        mktnames_non_all = self.append_keyvals(self.in_all_map, keyval_list=[]) + [
            "supply",
            "demand",
        ]
        # Find the full set of valid names for describing a measure's
...
class EPlusMapDicts(object):
    ...
    def __init__(self):
        ...
        self.enduse = {
            ...
            "cooking": ["interior_equipment_gas", "interior_equipment_other_fuel"],
...
class EPlusGlobals(object):
    ...
    def __init__(self, eplus_dir, cbecs_sf_byvint):
        ...
        self.eplus_coltypes = [
            ...
            ("service_water_heating_other_fuel", "<f8"),
            ("total_gas", "<f8"),
            ("total_other_fuel", "<f8"),
    ...
    def find_vintage_weights(self):
        ...
        expected_eplus_vintage_yr_bins = [handydicts.structure_type["new"]] + list(
        ...
        if sorted(self.eplus_vintages) == sorted(expected_eplus_vintage_yr_bins):
            ...
            for k in eplus_vintage_weights.keys():
                ...
                # square footage that corresponds to that vintage in cbecs
                    ...
                    for k2 in self.vintage_sf.keys():
                        ...
                        cbecs_match = re.search(r"(\D*)(\d*)(\s*)(\D*)(\s*)(\d*)", k2)
                        ...
                        if (
                            cbecs_yr >= handydicts.structure_type["retrofit"][k][0]
                            and cbecs_yr < handydicts.structure_type["retrofit"][k][1]
                        ):
            ...
            if new_weight_sum != 1:
                ...
                raise ValueError(
                    "Incorrect retrofit vintage weight total when"
...
class Measure(object):
    ...
    def __init__(self, handyvars, site_energy, capt_energy, **kwargs):
        ...
        if len(self.name) > 45:
            raise ValueError("ECM '" + self.name + "' name must be <= 45 characters")
        self.remove = False
    ...
    def fill_eplus(self, msegs, eplus_dir, eplus_coltypes, eplus_files, vintage_weights, base_cols):
        ...
        eplus_perf_in = [
            (eplus_dir + "/" + x)
            for x in eplus_files
            if any([y.lower() in x for y in bldg_type_names])
        ]
    ...
    def fill_mkts(self, msegs, msegs_cpl, convert_data, tsv_data, opts):
        ...
        for ind, mskeys in enumerate(ms_iterable):
            ...
            if mskeys[2] in ["single family home", "mobile home", "multi family home"]:
            ...
            if mskeys[0] == "secondary":
                ...
                if (
                    mskeys[2] in bldgs_costconverted.keys()
                    and (
                        bldg_sect != "residential"
                        or all(
                            [
                                x not in self.cost_units
                                for x in self.handyvars.cconv_bytech_units_res
                            ]
                        )
                    )
                    and (not isinstance(self.installed_cost, dict))
                ):
                    cost_meas, cost_units = [x for x in bldgs_costconverted[mskeys[2]]]
                ...
                if ind == 0 or isinstance(self.market_scaling_fractions, dict):
                    mkt_scale_frac = self.market_scaling_fractions
                if ind == 0 or isinstance(self.market_scaling_fractions_source, dict):
                ...
                if self.fuel_switch_to is None:
                    ...
                    if opts is not None and opts.site_energy is True:
                        ...
                        intensity_carb_base, intensity_carb_meas = [
                            {
                                yr: self.handyvars.carb_int[bldg_sect][mskeys[3]][yr]
            ...
            for i in range(0, len(mskeys)):
                ...
                if (
                    ...
                    and mskeys[i] in base_cpl.keys()
                    or mskeys[i] in ["primary", "secondary", "new", "existing", None]
                ):
                    ...
                    if isinstance(perf_meas, dict) and mskeys[i] in perf_meas.keys():
                        perf_meas = perf_meas[mskeys[i]]
                    if isinstance(perf_units, dict) and mskeys[i] in perf_units.keys():
                        perf_units = perf_units[mskeys[i]]
                    if isinstance(cost_meas, dict) and mskeys[i] in cost_meas.keys():
                        cost_meas = cost_meas[mskeys[i]]
                    if isinstance(cost_units, dict) and mskeys[i] in cost_units.keys():
                    ...
                    if (
                        ...
                        and mskeys[i] in mkt_scale_frac_source.keys()
                    ):
                        mkt_scale_frac_source = mkt_scale_frac_source[mskeys[i]]
            ...
            # Otherwise update all stock/energy/cost information for each year
                ...
                if mskeys[0] == "primary":
                    ...
                    if mseg["stock"] == "NA" or (
                        bldg_sect == "residential"
                        and "$/unit" not in cost_units
                        and self.measure_type == "add-on"
                    ):
                        sqft_subst = 1
                ...
                if mskeys[0] == "primary":
                    try:
                        ...
                        if (
                            ...
                            or any(
                                [
                                    y[1] in [0, "NA"]
                                    for y in base_cpl["performance"]["typical"].items()
                                ]
                            )
                            or any(
                        ...
                        if bldg_sect == "residential" and (
                            "$/unit" not in cost_units and self.measure_type == "add-on"
                        ):
                        ...
                        # considered across the heating and cooling services
                            ...
                            cost_base, cost_base_units = [
                                {
                                    yr: base_cpl["installed cost"]["typical"][yr] * 2
                                    for yr in self.handyvars.aeo_years
                                },
                    ...
                    cost_base, perf_base, life_base = [
                        ...
                        for x in [cost_meas, perf_meas, 10]
                    ]
                ...
                ) and (mskeys[0] == "secondary" or cost_base_units == cost_units):
                ...
                if mskeys[4] == "lighting" and self.end_use["secondary"] != [None]:
                ...
                if mskeys[0] == "secondary":
                    ...
                    add_stock = {
                        key: val * new_existing_frac[key] * 1000000
                ...
                # Total energy use
                add_energy = {
                    key: val * site_source_conv_base[key] * new_existing_frac[key]
                ...
                if self.time_sensitive_valuation is not None and (
                    mskeys[0] == "secondary"
                    or (mskeys[0] == "primary" and mskeys[3] == "electricity")
                ):
                ...
                for adopt_scheme in self.handyvars.adopt_schemes:
                    ...
                    add_dict = {
                        ...
                        "lifetime": {
                            "baseline": {
                                yr: life_base[yr] * add_stock_total[yr]
                                for yr in self.handyvars.aeo_years
                            },
                            "measure": life_meas,
                        },
    ...
    def gen_tsv_facts(self, tsv_data, mskeys, bldg_sect, rel_perf):
        ...
        if mskeys[0] == "primary":
            try:
                ...
                if mskeys[4] == "secondary heating":
                    ...
                    load_fact = tsv_data["load"][mskeys[1]][bldg_sect]["electronics"]
        ...
        # and heating or cooling load shapes)
            ...
            # are included in secondary microsegments
                ...
                raise ValueError(
                    "Invalid secondary microsegment type for " + "measure: " + self.name
                )
        ...
        updated_tsv_fracs = {
            "energy": {"efficient": energy_scale_eff},
            "cost": {"baseline": cost_scale_base, "efficient": cost_scale_eff},
            "carbon": {"baseline": carb_scale_base, "efficient": carb_scale_eff},
        }
    ...
    def apply_tsv(self, base_load, mskeys, rel_perf):
        ...
        for a in [
            x
            for x in self.handyvars.tsv_order
            if x in tsv_adjustments.keys() and tsv_adjustments[x] not in [{}, None]
        ]:
            ...
            try:
                ...
                start_stop = [tsv_adjustments[a][x] for x in ["start", "stop"]]
                ...
                try:
                    ...
                    if start_stop[0] <= start_stop[1]:
                        applicable_hrs = list(range(start_stop[0] - 1, start_stop[1]))
                    else:
                        applicable_hrs = list(range(0, start_stop[1])) + list(
                            range(start_stop[0], 24)
                        )
                except TypeError:
            ...
            # user-specified number of hours
                ...
                if len(applicable_hrs) == 24:
                    ...
                    eff_load = {
                        yr: [
                            eff_load[yr][x + offset_hrs]
                            if (x + offset_hrs) < 24
                            else eff_load[yr][0 + ((x + offset_hrs) - 24)]
                            for x in range(0, 24)
                        ]
                        for yr in self.handyvars.aeo_years
                    }
            ...
            # across a user-specified hour range
                ...
                if (
                    "custom_load" in tsv_adjustments[a].keys()
                    and tsv_adjustments[a]["custom_load"] is not None
                ):
                    ...
                    eff_load_frac_peak = {
                        yr: [eff_load[yr][x] / peak_load[yr] for x in range(24)]
                        for yr in self.handyvars.aeo_years
                    }
                    ...
                    "custom_savings" in tsv_adjustments[a].keys()
                    and tsv_adjustments[a]["custom_savings"] is not None
                ):
    ...
    def convert_costs(
                ...
                if isinstance(cval["conversion factor"]["value"], dict):
                    ...
                    if cval["original units"] == "$/ft^2 floor":
                        ...
                        if mskeys[5] is not None and any(
                            ...
                            convert_units *= next(
                                x[1] for x in cval_bldgtyp.items() if x[0] in mskeys[5]
                            )
                        # Case with $/ft^2 floor to $/unit cost conversion
                        # for non-lighting technology (one unit per house)
                        else:
                            convert_units *= cval_bldgtyp["all other technologies"]
        ...
        if convert_units != 1 and isinstance(convert_units_data, list):
            cost_meas_units_fin = cost_base_yr + convert_units_data[-1]["revised units"]
        # Cost year/unit adjustments involving a single stage of conversion
        elif convert_units != 1:
            cost_meas_units_fin = cost_base_yr + convert_units_data["revised units"]
        ...
        if cost_meas_units_fin == cost_base_units:
            ...
            if verbose:
                ...
                if (
                    bldg_sect != "residential"
                    or cost_meas_noyr not in self.handyvars.cconv_bytech_units_res
                ) and (
                    cost_meas_noyr in self.handyvars.cconv_bybldg_units
                    or isinstance(self.installed_cost, dict)
                ):
                    user_message += " for '" + mskeys[2] + "'"
                elif (
                    cost_meas_noyr in self.handyvars.cconv_bytech_units_res
    ...
    def partition_microsegment(
        ...
        for yr in self.handyvars.aeo_years:
            ...
            carb_total[yr] = carb_total_init[yr] * mkt_scale_frac * tsv_carb_base
            ...
            if adopt_scheme == "Adjusted adoption potential" and mskeys[0] == "primary":
                # PLACEHOLDER
            ...
            if mskeys[0] == "primary":
                ...
                if mskeys[-1] == "new":
                    ...
                    if stock_total_init[yr] != 0:
                        ...
                        try:
                            new_stock_base_frac = (
                                stock_total_init[str(self.market_entry_year - 1)]
                                / stock_total_init[yr]
                            )
                        except KeyError:
                ...
                # has already been captured by the baseline technology
                    ...
                    # Handle case with lifetime or retro. rate distributions
                        ...
                        # calculation are set to lists with the same length
                        # (and that list elements are of the float type)
                        base_repl_rt_max, captured_base_frac, captured_eff_frac = [
                            numpy.repeat(float(x), self.handyvars.nsamples)
                            if type(x) in (int, float)
                            else x
            ...
            if int(yr) >= self.market_entry_year and int(yr) < self.market_exit_year:
                ...
                # microsegment applies to new structure type
                    ...
                    if yr != self.handyvars.aeo_years[0] and stock_total[yr] != 0:
                        new_stock_add_frac = (
                    ...
                    competed_frac = new_stock_add_frac + captured_base_replace_frac
                    ...
                    # Handle case with lifetime or retro. rate distributions
                        ...
                        if all(captured_base_replace_frac <= 1):
                            ...
                            captured_base_replace_frac[
                                numpy.where(captured_base_replace_frac > 1)
                            ] = 1
                            competed_frac = captured_base_replace_frac
            ...
            stock_compete_meas[yr] = stock_total[yr] * competed_captured_eff_frac
            ...
            # Subsequent year in modeling time horizon
                ...
                if (
                    ...
                    and (int(yr) >= self.market_entry_year)
                    and (int(yr) < self.market_exit_year)
                ):
                ...
                # All other cases
                    ...
                    if (
                        "existing" in mskeys
                        and yr != self.handyvars.aeo_years[0]
                        and (stock_total[str(int(yr) - 1)]) != 0
                    ):
                        stock_adj_frac = stock_total[yr] / stock_total[str(int(yr) - 1)]
                    else:
                        stock_adj_frac = 1
            ...
            if int(yr) <= self.market_entry_year:
                ...
                tsv_energy_eff, tsv_ecost_eff, tsv_carb_eff = [
                    tsv_adj[x]["efficient"][yr] for x in ["energy", "cost", "carbon"]
                ]
            else:
                ...
                # Update relative performance
                rel_perf_capt = rel_perf[yr] * base_turnover_wt + rel_perf_capt * (
                    1 - base_turnover_wt
                )
                # Update time-sensitive efficiency scaling factors
                tsv_energy_eff, tsv_ecost_eff, tsv_carb_eff = [
            ...
            if self.measure_type == "add-on":
                ...
                stock_compete_cost_eff[yr] = (
                    ...
                    + (stock_compete[yr] - stock_compete_meas[yr]) * cost_base[yr]
                )
                # Total-efficient stock cost (add-on measure)
                stock_total_cost_eff[yr] = (
                    stock_total_meas[yr] * (cost_meas + cost_base[yr])
            ...
            carb_compete_cost[yr] = carb_compete[yr] * self.handyvars.ccosts[yr]
    ...
    def create_keychain(self, mseg_type):
        ...
        if any([x in ht_cl_euses for x in self.end_use[mseg_type]]):
            ...
            eu_non_hc = list(eu[numpy.where([x not in ht_cl_euses for x in eu])])
    ...
    def add_keyvals(self, dict1, dict2):
        ...
        """
        for (k, i), (k2, i2) in zip(sorted(dict1.items()), sorted(dict2.items())):
            if k == k2:
                if isinstance(i, dict):
    ...
    def div_keyvals_float(self, dict1, reduce_num):
        ...
        for (k, i) in dict1.items():
            if isinstance(i, dict):
                ...
                # Handle zero values
                if (type(reduce_num) == numpy.ndarray and all(reduce_num) != 0) or (
                    type(reduce_num) != numpy.ndarray and reduce_num != 0
                ):
    ...
    def fill_perf_dict(
        self, perf_dict, eplus_perf_array, vintage_weights, base_cols, eplus_bldg_types
    ):
            ...
            if isinstance(item, dict):
                ...
                if key in handydicts.structure_type.keys():
                    ...
                    if len(updated_perf_array) == 0 or (
                        key == "new"
                        and len(numpy.unique(updated_perf_array["template"])) != 1
                        or key == "retrofit"
                ...
                updated_perf_array_m, updated_perf_array_b = [
                    updated_perf_array[updated_perf_array["measure"] != "none"],
                    updated_perf_array[updated_perf_array["measure"] == "none"],
                ]
                ...
                for ind in range(0, len(updated_perf_array_m)):
                    row_m, row_b = [
                        updated_perf_array_m[ind],
                        updated_perf_array_b[ind],
                    ]
                    # Loop through remaining columns with consumption data
                    for n in eplus_header:
                        if row_m[n].dtype.char != "S" and row_m[n].dtype.char != "U":
                            # Find appropriate building type to weight
                            # consumption data points by
                            eplus_bldg_type_wt_row_m, eplus_bldg_type_wt_row_b = [
    ...
    def create_perf_dict(self, msegs):
        ...
        perf_dict_empty["primary"] = self.create_nested_dict(msegs, "primary")

        # Create secondary dict structure from baseline market properties
        # (if needed)
        if isinstance(self.end_use, dict):
            perf_dict_empty["secondary"] = self.create_nested_dict(msegs, "secondary")
    ...
    def create_nested_dict(self, msegs, mseg_type):
        ...
        for kc in dict_keys:
            ...
            for ind, key in enumerate(kc):
                ...
                if key in dict_check.keys():
                    ...
                    if key in ["heating", "cooling"]:
                        dict_check = dict_check[self.technology_type[mseg_type]]
                else:
                    break
            ...
            if any([x in self.technology[mseg_type] for x in dict_check.keys()]):
    ...
    def build_array(self, eplus_coltyp, files_to_build):
        ...
        for ind, f in enumerate(files_to_build):
            ...
            eplus_file = numpy.genfromtxt(
                f, names=True, dtype=eplus_coltyp, delimiter=",", missing_values=""
            )
            ...
            if ind == 0:
                ...
                eplus_perf_array = numpy.concatenate((eplus_perf_array, eplus_file))
...
class MeasurePackage(Measure):
    ...
    def __init__(self, measure_list_package, p, bens, handyvars):
        ...
        # Check to ensure that measure name is proper length for plotting
        if len(self.name) > 40:
            raise ValueError("ECM '" + self.name + "' name must be <= 40 characters")
        self.benefits = bens
        self.remove = False
        self.energy_outputs = {"site_energy": False, "captured_energy_ss": False}
        ...
        if all([x.energy_outputs["site_energy"] is True for x in self.contributing_ECMs]):
            ...
            raise ValueError(
                ...
                "across contributing ECMs for package: '"
                + str(self.name)
            ...
            for adopt_scheme in self.handyvars.adopt_schemes:
                ...
                for x in ["baseline", "efficient", "savings"]:
                    self.merge_out_break(
                        self.markets[adopt_scheme]["mseg_out_break"][x],
                        m.markets[adopt_scheme]["mseg_out_break"][x],
                    )
    ...
    def merge_contrib_msegs(
        self, msegs_pkg, msegs_meas, cm_key, meas_typ, adopt_scheme, mseg_out_break_adj
    ):
                ...
                if k == "energy":
                    ...
                    total_energy_orig_save = {
                        yr: (
                            ...
                            - copy.deepcopy(msegs_meas[k]["total"]["efficient"][yr])
                        )
                        for yr in self.handyvars.aeo_years
                    }
                    # Scale down contributing microsegment energy based on
                    # number of overlapping measures
                    self.div_keyvals_float(msegs_meas[k], len(overlap_meas))
                    ...
                    mseg_out_break_adj["savings"][out_cz][out_bldg][out_eu] = {
                        yr: mseg_out_break_adj["savings"][out_cz][out_bldg][out_eu][yr]
                        - (
    ...
    def update_dict(self, dict1, dict2):
        ...
        if len(dict1.keys()) != 0 and all(["," not in x for x in dict1.keys()]):
            ...
            for x in ["energy", "carbon"]:
                for cs in ["competed", "total"]:
                    ...
                    msegs_meas[x][cs]["efficient"] = {
                        key: 0
                        if (
                            not isinstance(eff[key], numpy.ndarray)
                            and eff[key] > 0
                            and (base[key] - eff[key]) * energy_ben > eff[key]
                        )
                        or (
                    ...
                    msegs_meas["cost"][x][cs]["efficient"] = {
                        key: 0
                        if (
                            not isinstance(eff_c[key], numpy.ndarray)
                            and eff_c[key] > 0
                            and (base_c[key] - eff_c[key]) * energy_ben > eff_c[key]
                        )
                        or (
...
def prepare_packages(
    packages, meas_update_objs, meas_summary, handyvars, handyfiles, base_dir, opts
):
...
def main(base_dir):
    ...
    if opts.web is True:
        # Get the ECM name and ECM JSON content from the PostgreSQL database
        query = "SELECT name, definition FROM " + db["schema"] + ".ecm WHERE id = %s"
        cursor.execute(query, opts.id)
        content = cursor.fetchone()

        # Insert the ECM name and JSON content into the appropriate lists
        meas_toprep_indiv.append(content[1])
        meas_toprep_indiv_names = [content[0]]
    else:
        ...
        for mi in meas_toprep_indiv_names:
            with open(path.join(base_dir, handyfiles.indiv_ecms, mi), "r") as jsf:
                try:
                    ...
                    # requiring further preparation if: a) measure name is not
                    ...
                    if (
                        all([meas_dict["name"] != y["name"] for y in meas_summary])
                        or all(
                            [
                        ...
                        or (
                            opts is not None
                            and opts.site_energy is True
                            and all(
                                [
                                    y["energy_outputs"]["site_energy"] is False
                                    for y in meas_summary
                                    if y["name"] == meas_dict["name"]
                                ]
                            )
                        )
                        or (
                            opts is not None
                            and opts.captured_energy is True
                            and all(
                                [
                                    y["energy_outputs"]["captured_energy_ss"] is False
                                    for y in meas_summary
                                    if y["name"] == meas_dict["name"]
                                ]
                            )
                        ...
                        or (
                            opts is None
                            or opts.site_energy is False
                            and all(
                                [
                                    y["energy_outputs"]["site_energy"] is True
                                    for y in meas_summary
                                    if y["name"] == meas_dict["name"]
                                ]
                            )
                        ...
                        or (
                            opts is None
                            or opts.captured_energy is False
                            and all(
                                [
                                    y["energy_outputs"]["captured_energy_ss"] is True
                                    for y in meas_summary
                                    if y["name"] == meas_dict["name"]
                                ]
                            )
    ...
    if len(meas_toprep_indiv) > 0 or len(meas_toprep_package) > 0:
        ...
        with open(path.join(base_dir, *handyfiles.msegs_in), "r") as msi:
            try:
                ...
                raise ValueError(
                    "Error reading in '" + handyfiles.msegs_in + "': " + str(e)
                ) from None
        ...
        with open(path.join(base_dir, *handyfiles.msegs_cpl_in), "r") as bjs:
            try:
                ...
                raise ValueError(
                    "Error reading in '" + handyfiles.msegs_cpl_in + "': " + str(e)
                ) from None
        ...
        with open(path.join(base_dir, *handyfiles.cost_convert_in), "r") as cc:
            try:
                ...
                raise ValueError(
                    "Error reading in '" + handyfiles.cost_convert_in + "': " + str(e)
                ) from None
        ...
        with open(path.join(base_dir, *handyfiles.tsv_data), "r") as tsv:
            try:
                tsv_data = json.load(tsv)
            except ValueError as e:
                raise ValueError(
                    "Error reading in '" + handyfiles.tsv_data + "': " + str(e)
                ) from None
        ...
        try:
            ...
            try:
                ...
                raise ValueError(
                    "Error reading in '" + handyfiles.run_setup + "': " + str(e)
                ) from None
            am.close()
        except FileNotFoundError:
        ...
        meas_prepped_compete, meas_prepped_summary = split_clean_data(meas_prepped_objs)
        ...
        # Export data based on whether running locally or in the web app
        if opts.web:
            # Write measure data to a PostgreSQL database
            for ind, m in enumerate(meas_prepped_objs):
                # Specify calculation method string  # CHECK - method strings
                if opts.site_energy is True:
                    sql_calc_mtd = "site"
                elif opts.captured_energy is True:
                    sql_calc_mtd = "captured"
                else:
                    sql_calc_mtd = "fossil_equivalent"

                sql_comp = meas_prepped_compete[ind]
                sql_uncomp = meas_prepped_summary[ind]

                psgUpsertTable(opts.id, sql_calc_mtd, sql_uncomp, sql_comp)
        else:
...
def psgUpsertEcms():
    """Add new ECMs and update existing ECMs in the ecm table

    Note:
        This does not remove ecms from the database that no longer exist
        in the ecm_definitions directory.
    """
    try:
        ecm_definitions = []
        ...
        for ecm in ecm_definitions:
            ...
            if (
                ...
                updated_at = parse(
                    definition["_updated_by"][-1]["timestamp"], tzinfos=tzinfos
                ...
                updated_at = parse(
                    definition["_updated_by"]["timestamp"], tzinfos=tzinfos
                ).strftime("%Y-%m-%d %H:%M:%S %z")
        ...
        connection.commit()
        print("Table ecm populated successfully in PostgreSQL")

    except (Exception, psycopg2.Error) as error:
        print("Error while populating ecms", error)
...
#     except (Exception, psycopg2.Error) as error:
#         print("Error while truncating PostgreSQL table", error)


def psgUpsertTable(ecm_id, calc_mtd, uncomp, comp):
    """CHECK - add docstring"""
    try:
        query = (
            "INSERT INTO "
            + db["schema"]
            + ".ecm_data (ecm_id, energy_calc_method, uncomp_data, comp_data) "
            "VALUES (%s,%s,%s,%s) "
            "ON CONFLICT ON CONSTRAINT "
            + db["schema"]
            + "_ecm_data_ecm_id_energy_calc_method_uniq "
            "DO UPDATE SET uncomp_data = EXCLUDED.uncomp_data, "
            "              comp_data = EXCLUDED.comp_data"
        )

        cursor.execute(query, (ecm_id, calc_mtd, json.dumps(uncomp), json.dumps(comp)))
        connection.commit()
        ...
        print(
            ...
            + ", "
            + calc_mtd
            + ") inserted successfully into ecm_data table",  # CHECK - TEMPORARY
        )

    except (Exception, psycopg2.Error) as error:
        print("Failed to insert " + ecm_id + " into ecm_data table", error)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--id", help="ECM id number for the ECM to be processed")
    ...
    # Configuration for --web argument to send data to PostgreSQL database
    if opts is not None and opts.web is True:
        try:
            ...
            # Instantiates ecm_data table in the PostgreSQL database for
            # results data if the table does not already exist
            ddl_template = Template(open("./DDL.sql", "r").read())
            ddl = ddl_template.substitute({"schema": db["schema"]})
            cursor.execute(ddl)
            connection.commit()

            # Insert and update ECMs
            psgUpsertEcms()

            # Run through all three energy accounting/calculation methods
            opts.site_energy = False
            opts.captured_energy = False
            main(base_dir)  # fossil fuel equivalent (primary energy)
            opts.site_energy = True
            opts.captured_energy = False
            main(base_dir)  # site energy
            opts.site_energy = False
            opts.captured_energy = True
            main(base_dir)  # captured energy (primary energy)

            # Close database connection
            cursor.close()
            connection.close()
            print("PostgreSQL connection is closed")  # CHECK - TEMPORARY??
