...
def main(base_dir):
    ...
    if options.web is True:
        ...
        for id_num in options.ids:
            ...
            uncomp_data, meas_name = psgSelectTable_uncomp(id_num, options.tsv, energy_out)
    ...
    for m in measures_objlist:
        if options.web is True:
            meas_comp_data = psgSelectTable_comp(m.name, options.tsv, energy_out)
        else:
    ...
    if options.web is True:
        ...
        # Record analysis results by inserting into analysis_data table
        # or updating the corresponding row if it already exists
        query = ("INSERT INTO " + db["schema"] + ".analysis_data "
                 "(analysis_id, tsv_view, ecm_results) VALUES (%s, %s, %s) "
                 "ON CONFLICT (analysis_id, tsv_view) "
                 "DO UPDATE SET ecm_results = EXCLUDED.ecm_results "
                 "WHERE " + db["schema"] + ".analysis_data.analysis_id = EXCLUDED.analysis_id "
                 "AND " + db["schema"] + ".analysis_data.tsv_view = EXCLUDED.tsv_view")
        cursor.execute(query, (options.analysis_id, options.tsv, json.dumps(a_run.output_ecms)))
        connection.commit()
    ...
    if options.web is not True and (
        regions == "AIA" or (
...
def psgSelectTable_uncomp(ecm_id, tsv_config, energy_calc_method):
    """Return uncomp_data and ecm_name; add comments inside function"""
    try:
        # if tsv_config:
        query = (
            "SELECT name, uncomp_data FROM "
            + db["schema"] + ".ecm_data_view "
            "WHERE ecm_id = %s AND tsv_view = %s AND energy_calc_method = %s")
        cursor.execute(query, (ecm_id, tsv_config, energy_calc_method))
        # else:
        #     query = (
        #         "SELECT name, uncomp_data FROM "
        #         + db["schema"] + ".ecm_data_view "
        #         "WHERE ecm_id = %s AND tsv_view IS NULL AND energy_calc_method = %s")
...
def psgSelectTable_comp(ecm_name, tsv_config, energy_calc_method):
    """Return comp_data; add comments inside function"""
    try:
        # if tsv_config:
        query = (
            "SELECT comp_data FROM "
            + db["schema"] + ".ecm_data_view "
            "WHERE name = %s AND tsv_view = %s AND energy_calc_method = %s")
        cursor.execute(query, (ecm_name, tsv_config, energy_calc_method))
        # else:
        #     query = (
        #         "SELECT comp_data FROM "
        #         + db["schema"] + ".ecm_data_view "
        #         "WHERE name = %s AND tsv_view IS NULL AND energy_calc_method = %s")
...
if __name__ == '__main__':
    ...
    parser.add_argument("--ids", nargs="*", type=str,
                        help=("Specify id numbers of ECMs to include in the " +
                              "analysis, separated by spaces"))
    ...
    parser.add_argument("--analysis_id",
                        help="Specify id number for the analysis itself")
    ...
    if options is not None and options.web is True:
        ...
        # Determine if analysis has TSV ECMs
        query = ("SELECT has_tsv FROM "
                 + db["schema"]
                 + ".analysis WHERE id = %s")
        cursor.execute(query, (options.analysis_id,))
        response = cursor.fetchone()
        options.has_tsv = response[0]  # Acting as a backdoor global variable

        # Generate results
        options.tsv = ''  # Set TSV status, backdoor global variable
        main(base_dir)  # Run annual results
        # If TSV ECMs, also generate results for the TSV perspectives
        if options.has_tsv:
            # Specify the TSV perspectives to auto-calculate
            tsv_views = ['summer_peak', 'summer_off_peak',
                         'winter_peak', 'winter_off_peak']

            options.site_energy = True  # Reset to use only site energy
            for tsv_view in tsv_views:
                options.tsv = tsv_view  # Acting as a backdoor global variable
                main(base_dir)
