"""Extract test data from UpdateMeasuresTest to pickle file.

This script dynamically loads the original ecm_prep_test.py file,
executes the UpdateMeasuresTest.setUpClass() method, and serializes
all class attributes to a pickle file for use in the pytest version.

Usage:
    python tests/ecm_prep_test/dump_update_measures_test_data.py
"""
...
def main():
    print("=" * 80)
    print("UpdateMeasuresTest Data Extraction")
    print("=" * 80)
    ...
    UpdateMeasuresTest = orig_tests.UpdateMeasuresTest
    print("   [OK] Module loaded")
    
    # Execute setUpClass to initialize all test data
    print("\n[2/4] Executing UpdateMeasuresTest.setUpClass()...")
    print("   (This is the largest test class - may take a while...)")
    
    try:
        UpdateMeasuresTest.setUpClass()
    ...
    for attr_name in dir(UpdateMeasuresTest):
        if not attr_name.startswith('_') and attr_name not in ['setUpClass', 'tearDownClass']:
            try:
                attr_value = getattr(UpdateMeasuresTest, attr_name)
                # Skip methods
    ...
    key_attrs = ['opts_aia', 'opts_emm', 'opts_health', 'aia_measures',
                 'package_ecms_file', 'handyvars_aia', 'handyfiles_aia']
    ...
    summary_file = output_dir / "update_measures_test_data_summary.txt"
    
    with open(summary_file, 'w') as f:
        f.write("UpdateMeasuresTest Data Summary\n")
        f.write("=" * 80 + "\n\n")
    ...
    print(f"\nYou can now run: pytest tests/ecm_prep_test/test_update_measures.py")
