...
def main():
    ...
    print("-" * 115)
    print(
        f"{'Class Name':<35} {'Line Range':<18} {'Old Lines':<12} "
        f"{'New Lines':<12} {'Line Change':<12} {'Status'}"
    )
    print("-" * 115)
    ...
    for cls in classes:
        ...
        print(
            f"{cls['name']:<35} {line_range:<18} {old_lines_str:<12} "
            f"{new_line_info:<12} {line_change_str:<12} {status}"
        )
    ...
    if converted_count > 0:
        overall_change = ((total_new_lines - total_original_lines_converted) /
                          total_original_lines_converted) * 100
        print(
            f"Progress: {converted_count}/{len(classes)} classes converted "
            f"({converted_count*100//len(classes)}%)"
        )
        print(
            f"Converted so far: {total_original_lines_converted:,} original -> "
            f"{total_new_lines:,} new (change: {overall_change:+.1f}%)"
        )
