...
class FixECMMoveConflictsTest(CommonUnitTest):
    ...
    def test_move_conflicts_out_of_range_entry(self, err_input, ok_input):
        ...
        expect_keep = ["(Prospective) Silica Nanoparticle Liquid-applied Insulation"]
...
class ECMListKeywordSelectionUpdatingTest(CommonUnitTest):
    ...
    @patch("scout.run_setup.user_input_ecm_kw", side_effect=[["ENERGY STAR"], ["Prospective"]])
    ...
    @patch(
        "scout.run_setup.user_input_ecm_kw",
        side_effect=[["Prospective", "ENERGY STAR", "Novel"], ["Low-cost"]],
    )
    @patch(
        "scout.run_setup.fix_ecm_move_conflicts",
        side_effect=[
            ["Novel Electric Clothes Dryer, Low-cost"],
            ["low-cost energy star refrigerator v.3.0"],
        ],
...
class UserBaselineMarketSelectionsTest(unittest.TestCase):
    ...
    def test_user_baseline_climate_zone_selections(self, user_txt):
        ...
        self.assertEqual(
            run_setup.user_input_baseline_market_filters(baseline_mkt), expected_output
        )
    ...
    def test_user_baseline_all_selected(self, user_txt):
        ...
        for mkt in baseline_mkt:
            self.assertFalse(run_setup.user_input_baseline_market_filters(mkt))
...
class ECMJSONEvaluationTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.ecm4 = {
            ...
            "climate_zone": "all",
            "bldg_type": ["assembly", "education", "small office", "large office", "lodging"],
            "structure_type": "all",
            "end_use": "lighting",
            "fuel_type": "electricity",
    ...
    def test_single_climate_zone_user_selection(self):
        ...
        for idx, ecm_obj in enumerate(self.ecms):
            self.assertIs(
                run_setup.evaluate_ecm_json(ecm_obj, filter_str, filter_cat), expected_response[idx]
            )
...
class ECMListMarketSelectionUpdatingTest(CommonUnitTest):
    ...
    def test_ecm_list_baseline_market_updating(self, mock_listdir, mock_fopen):
        ...
        handlers = (
            mock_open(read_data=self.ecm1).return_value,
        ...
        expect_inactive = ["Commercial Gas Boiler, 90.1 c. 2013", "ENERGY STAR Gas Boiler v. 3.0"]
        ...
        active, inactive = run_setup.ecm_list_market_update(
            Path("whatever"), self.active_list, self.inactive_list, filters, market_cat
        )
