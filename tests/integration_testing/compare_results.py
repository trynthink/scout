...
class ScoutCompare:
    ...
    def compare_dict_keys(
        self, dict1: dict, dict2: dict, paths: list, path: str = "", key_diffs: pd.DataFrame = None
    ):
        ...
        for key in keys1 & keys2:
            if isinstance(dict1[key], dict) and isinstance(dict2[key], dict):
                key_diffs = self.compare_dict_keys(
                    dict1[key], dict2[key], paths, path=f"{path}: {key}", key_diffs=key_diffs
                )
    ...
    def compare_jsons(
        self, json1_path: Path, json2_path: Path, percent_threshold: float, output_dir: Path = None
    ):
    ...
    def compare_summary_reports(
        self, report1_path: Path, report2_path: Path, output_dir: Path = None
    ):
...
def main():
    ...
    parser.add_argument(
        "--summary-baseline", type=Path, help="Path to the baseline summary report (Excel file)"
    )
    parser.add_argument(
        "--summary-new", type=Path, help="Path to the new summary report (Excel file)"
    )
    ...
    parser.add_argument(
        "--threshold", type=float, default=10, help="Threshold for percent difference"
    )
    ...
    if args.base_dir and args.new_dir:
        ...
        compare.compare_jsons(
            agg_json_base, agg_json_new, percent_threshold=args.threshold, output_dir=new_dir
        )
        ecm_json_base = base_dir / "ecm_results.json"
        ecm_json_new = new_dir / "ecm_results.json"
        compare.compare_jsons(
            ecm_json_base, ecm_json_new, percent_threshold=args.threshold, output_dir=new_dir
        )
        ...
        if args.json_baseline and args.json_new:
            compare.compare_jsons(
                args.json_baseline, args.json_new, percent_threshold=args.threshold
            )
