...
SCHEMA_CONFIGS = {
    ...
    "cpl": {
        ...
        "compressed": True,
        "filter_errors": ["consumer choice"],  # Filter consumer choice regex errors
...
def data(data_path, schema_config):
    """Load and return the JSON data (handles both compressed and uncompressed)."""
    schema_name, config = schema_config
...
def test_schema_data_validity(schema, data, data_path, schema_config):
    ...
    This unified test validates both microsegment and cost/performance/lifetime
    data files against their respective schemas. It provides comprehensive error
    messages including:
    ...
    Tested schemas:
    - mseg: Microsegment stock and energy data (mseg_res_com_cz.json)
    - cpl: Cost, performance, and lifetime data (cpl_res_com_cz.gz)
    """
    schema_name, config = schema_config
        ...
        for e in errors:
            ...
            if config["filter_errors"]:
                ...
                for filter_term in config["filter_errors"]:
                    if filter_term in e.message and "does not match any of the regexes" in e.message:
                        skip_error = True
                        break
                if skip_error:
                    continue
            ...
            # Build error message parts with JSON filename prominently displayed
            msg_parts = [
                f"ERROR: {e.message}",
...
def test_schema_is_valid_json(schema_path, schema_config):
    ...
    Verifies that schema files can be parsed as JSON without errors.
    """
    schema_name, config = schema_config
    try:
