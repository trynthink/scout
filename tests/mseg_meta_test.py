...
class YearRangeExtractionFromStructuredArraysTest(unittest.TestCase):
    ...
    aeo_structured_arrays = [
        ...
        np.array(
            [
                (1, 2005, 2040, 3, 4.5, 3000, 2, 3, b"ELEC_RAD"),
    ...
    column_names = [["Year"], ["START_EQUIP_YR", "END_EQUIP_YR"]]
...
class YearRangeExtractionFromArrayColumnHeadingsTest(unittest.TestCase):
    ...
    def test_year_min_max_extraction_from_dtype_function(self):
        min_out, max_out = mm.dtype_ripper(
            self.example_dtype, self.min_yrs_input_list, self.max_yrs_input_list
        )
...
class FileProcessorCombinedFunctionTest(unittest.TestCase):
    ...
    def test_combined_data_processor_function(self):
        ...
        a, b = mm.file_processor(
            "ktek.csv",
            dummy_file_import_function,
            "",
            self.files_to_check,
            self.min_yrs_input_list,
            self.max_yrs_input_list,
...
class JSONProcessorTest(unittest.TestCase):
    ...
    @patch("builtins.open", new_callable=mock_open, read_data=json.dumps(example_dict))
    def test_json_structure_year_extraction(self, mocked_json):
        ...
        a, b = mm.json_processor(mocked_json, self.min_yrs_input_list, self.max_yrs_input_list)
        # Compare the minimum year and maximum year lists to those
        # output by the function
        self.assertEqual(self.expected_min_years, a)
        self.assertEqual(self.expected_max_years, b)
