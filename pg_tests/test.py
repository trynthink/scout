...
class PostgreSQLTest(unittest.TestCase):
    ...
    def test_local_environment_variables(self):
    ...
    def test_populated_ecms(self):
        """Ensure that the ecms table is populated and the row count
        ...
        # Make sure some ECMs are present in the local file system
        ...
        # Make sure that the unique set of ECM names matches
        # the number of files present in the local file system
        ...
        # Make sure the number of ECMs in the database matches
        # the number of unique ECMs in the local file system
    ...
    def test_populated_ecm_data(self):
        """Ensure that the ecm_data table is populated and that all rows
        have populated uncomp andcomp values"""
        query = "SELECT count(*) FROM " + self.schema + ".ecm_data"
        self.cursor.execute(query)
        response = self.cursor.fetchone()
        self.assertGreater(response[0], 0)

        # Make sure none of the uncompeted and competed fields are empty dicts
        query = (
            "SELECT count(*) FROM "
            + self.schema
    ...
    def test_populated_analysis(self):
        """Ensure that the analysis table is populated and that all rows
        have populated ecm_results"""
        ...
        query = (
            "SELECT count(*) FROM "
            + self.schema
            + ".analysis_data WHERE ecm_results <> '{}'"
        )
        self.cursor.execute(query)
        response = self.cursor.fetchone()
        self.assertGreater(response[0], 0)
    ...
    def test_analysis_view(self):
        # Check that the view is working as expected to create
        # an ecm_ids jsonb field
