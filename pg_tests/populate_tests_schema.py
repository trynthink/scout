...
# Use DDL to create tables, relationships, etc. in database
cursor.execute(ddl)
...
for ecm in ecm_definitions:
    ...
    cursor.execute(
        ...
        "(name, definition, created_at, updated_at, is_package, control_status, type, status) "
        "VALUES (%s, %s, %s, %s, %s, '', '', '')",
        (name, json.dumps(definition), added_at, updated_at, False),
    )
...
for result in competition_results:
    ...
    # Extract calculation method from ECM uncompeted data and assign
    # calculation method string using the strings allowed by the DDL
    if uncomp_data['usr_opts']['site_energy']:
        calc_method = 'site'
    elif uncomp_data['usr_opts']['captured_energy']:
        calc_method = 'captured'
    else:
        calc_method = 'fossil_equivalent'
    ...
    if name in ecm_ids:
        ...
        cursor.execute(
            ...
            "VALUES (%s, %s, %s, %s)",
            (ecm_id, calc_method, json.dumps(uncomp_data), json.dumps(comp_data)),
        )
    else:
...
cursor.execute(
    ...
    "(name, energy_calc_method, control_status, status) "
    "VALUES (%s, %s, '', '')",
    ("Test Analysis", "fossil_equivalent"),
)

# Add analysis results
cursor.execute(
    "INSERT INTO " + schema + ".analysis_data "
    "(analysis_id, ecm_results) "
    "VALUES (%s, %s)",
    (1, json.dumps(ecm_results))
)
