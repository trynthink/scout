...
class MakeImmutableMixin:
    ...
    def __setattr__(self, name, value):
        if not hasattr(self, '_immutable') or not self._immutable or name == '_immutable':
            ...
            raise AttributeError(f"Cannot modify '{name}': {self.__class__.__name__} is immutable after initialization.")
...
class UsefulVars(MakeImmutableMixin, object):
    ...
    #         super().__setattr__(name, value)
    #     else:
    #         raise AttributeError(f"Cannot modify '{name}': UsefulVars is immutable after initialization.")
