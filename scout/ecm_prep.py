...
from scout.ecm_prep_utils import ECMUtils
...
def main(opts: argparse.NameSpace):  # noqa: F821
    ...
    # ---- Import Measures ----
    # Import JSON data, filter as needed
    ...
    meas_toprep_package_init = ECMUtils.downselect_packages(meas_toprep_package_init,
                                                            opts.ecm_packages)
    meas_summary = ECMUtils.get_existing_measure_summary_data(handyfiles)
    meas_shapes = ECMUtils.get_measure_savings_shapes(opts, handyfiles)
    ...
    # Get package environment separation info
    ctrb_ms_pkg_all = []
    meas_summary_env_cf = []
    meas_shapes_env_cf = []
    if opts.pkg_env_sep is True:
        ctrb_ms_pkg_all = [[p["name"], p["contributing_ECMs"]] for p in meas_toprep_package_init]
        for p in meas_toprep_package_init:
            ctrb_ms_pkg_all.append([p["name"], p["contributing_ECMs"]])
        meas_summary_env_cf, \
            meas_shapes_env_cf, \
            meas_summary = ECMUtils.get_pkg_env_sep_info(handyfiles, meas_summary)

    # Import all individual measure JSONs
    meas_toprep_indiv, \
        meas_toprep_indiv_nopkg, \
        pkg_copy_flag = ECMUtils.get_indiv_measures(
            opts,
            handyfiles,
            handyvars,
            meas_summary,
    ...
    meas_toprep_package_init, _ = ECMUtils.filter_invalid_packages(meas_toprep_package_init,
                                                                   ecm_names,
                                                                   opts)

    # Write initial and update data for run_setup.json
    run_setup = ECMUtils.initialize_run_setup(handyfiles)
    run_setup = ECMUtils.update_run_setup(run_setup, opts, meas_toprep_package_init)
    meas_toprep_package, ctrb_ms_pkg_prep = ECMUtils.get_measure_packages(opts,
                                                                          handyfiles,
                                                                          meas_toprep_package_init,
                                                                          meas_prepped_pkgs,
                                                                          meas_toprep_indiv_nopkg,
    ...
    if len(meas_toprep_indiv) > 0 or len(meas_toprep_package) > 0:
        ...
        if opts is not None and opts.pkg_env_sep and meas_summary_env_cf:
