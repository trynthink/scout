...
class ECMPrepHelper:
    ...
    def initialize_run_setup(input_files: UsefulInputFiles) -> dict:
        ...
        try:
            ...
            try:
                ...
                raise ValueError(f"Error reading in '{input_files.run_setup}': {str(e)}") from None
            am.close()
            # Initialize all measures as inactive
            run_setup = ECMPrepHelper.update_active_measures(
                run_setup, to_inactive=run_setup["active"]
            )
    ...
    @staticmethod
    def update_active_measures(
        run_setup: dict, to_active: list = [], to_inactive: list = [], to_skipped: list = []
    ) -> dict:
            ...
            for bldg_key, bldg_val in geo_val.items():
                ...
                for fuel_key, fuel_val in bldg_val.items():
                    ...
                    for eu in ("heating", "secondary heating", "cooling"):
                        ...
                        summed = {
                            yr: float(sum(ed.get(yr, 0.0) for _, ed in comp_energy_pairs))
                            for yr in years
                        }
                        demand[new_name] = {"stock": "NA", "energy": summed}
    ...
    def retrieve_valid_ecms(
        ...
        contributing_ecms = {ecm for pkg in packages for ecm in pkg["contributing_ECMs"]}
        opts.ecm_files.extend([ecm for ecm in contributing_ecms if ecm not in opts.ecm_files])
        valid_ecms = [
            x
            for x in handyfiles.indiv_ecms.iterdir()
            if x.suffix == ".json" and "package_ecms" not in x.name and x.stem in opts.ecm_files
        ]

        return valid_ecms

    @staticmethod
    def filter_invalid_packages(
        packages: list[dict], ecms: list, opts: argparse.Namespace
        ...
        invalid_pkgs = [
            pkg["name"] for pkg in packages if not set(pkg["contributing_ECMs"]).issubset(set(ecms))
        ]
        ...
        if invalid_pkgs:
            ...
            msg = (
                f"WARNING: Package(s) in package_ecms.json {package_opt_txt}have contributing"
    ...
    def tsv_cost_carb_yrmap(tsv_data, aeo_years):
        ...
        tsv_yr_map = {
            yr_tsv: [str(x) for x in range(int(yr_tsv), int(tsv_yrs[ind + 1]))]
            if (ind + 1) < len(tsv_yrs)
            else [str(x) for x in range(int(yr_tsv), int(aeo_years[-1]) + 1)]
        ...
        if aeo_years[0] not in tsv_yr_map[tsv_yrs[0]]:
            yrs_to_prepend = range(int(aeo_years[0]), min([int(x) for x in tsv_yr_map[tsv_yrs[0]]]))
            tsv_yr_map[tsv_yrs[0]] = [str(x) for x in yrs_to_prepend] + tsv_yr_map[tsv_yrs[0]]
    ...
    def split_clean_data(meas_prepped_objs, full_dat_out):
        ...
        for m in meas_prepped_objs:
            ...
            for adopt_scheme in m.handyvars.adopt_schemes_prep:
                ...
                if not isinstance(m, MeasurePackage):
                    del m.markets[adopt_scheme]["mseg_adjust"]["paired heat/cool mseg adjustments"]
                ...
                if full_dat_out[adopt_scheme]:
                    ...
                    if len(m.eff_fs_splt[adopt_scheme].keys()) != 0:
                        fs_splits_dict[adopt_scheme] = m.eff_fs_splt[adopt_scheme]
                    # If applicable, add sector shape data
                    if (
                        m.sector_shapes is not None
                        and len(m.sector_shapes[adopt_scheme].keys()) != 0
                    ):
                        shapes_dict["name"] = m.name
                        shapes_dict[adopt_scheme] = m.sector_shapes[adopt_scheme]
                else:
            ...
            if isinstance(m, MeasurePackage):
                m.contributing_ECMs = [x.name for x in m.contributing_ECMs]
                del m.htcl_overlaps
        ...
        return meas_prepped_compete, meas_prepped_summary, meas_prepped_shapes, meas_eff_fs_splt
...
class Measure(object):
    ...
    def __init__(self, base_dir, handyvars, handyfiles, opts_dict, **kwargs):
        ...
        if (
            self.usr_opts["tsv_metrics"] is not False
            and (self.fuel_type not in ["electricity", ["electricity"]])
            and self.fuel_switch_to != "electricity"
        ):
            raise ValueError(
                "Non-electric fuel found for measure '"
                + self.name
        ...
        for _tsv_attr in ("tsv_hourly_lafs", "tsv_hourly_price", "tsv_hourly_emissions"):
            if hasattr(handyvars, _tsv_attr):
                setattr(self.handyvars, _tsv_attr, copy.deepcopy(getattr(handyvars, _tsv_attr)))
        ...
        try:
            ...
            # Extend retrofit rate point value across all years
                ...
                self.retro_rate = {yr: self.retro_rate for yr in self.handyvars.aeo_years}
            # Accommodate retrofit rate input as a probability distribution
            elif type(self.retro_rate) is list and isinstance(self.retro_rate[0], str):
                # Sample measure retrofit rate values
                self.retro_rate = {
                    yr: self.rand_list_gen(self.retro_rate, self.handyvars.nsamples)
        ...
        self.hp_convert_flag = (
            self.tech_switch_to not in [None, "NA", "same"] and "HP" in self.tech_switch_to
        )
        ...
        if (
            isinstance(self.technology, list)
            and all([x in self.handyvars.demand_tech for x in self.technology])
        ) or self.technology in self.handyvars.demand_tech:
        ...
        if self.handyvars.panel_shares is not None and (
            self.technology_type == "demand"
            or (
                self.technology_type != "demand"
                and not all(
        ...
        self.target = any(
            [x in self.name for x in ["Prosp", "Emerging", "Target", "Brk", "Break", "RDO"]]
        )
        ...
        if self.usr_opts["floor_start"] is not None and (
            self.min_eff_elec_flag is not None
            or (self.usr_opts["add_typ_eff"] is not False and "Analogue" in self.name)
        ):
        ...
        try:
            ...
            has_ann_shp = (
                "shape" in self.tsv_features.keys()
                and "custom_annual_savings" in self.tsv_features["shape"].keys()
            )
            ...
            if (
                self.fuel_type not in ["electricity", ["electricity"]]
            ) and self.fuel_switch_to != "electricity":
                raise ValueError(
                    "Non-electric fuel found for measure '"
                    + self.name
            ...
            if has_ann_shp:
                ...
                logger.info("Retrieving custom savings shape data for measure " + self.name + "...")
                # Determine the CSV file name
                csv_shape_file_name = self.tsv_features["shape"]["custom_annual_savings"]
                ...
                try:
                    self.tsv_features["shape"]["custom_annual_savings"] = numpy.genfromtxt(
                ...
                css_dat = self.tsv_features["shape"]["custom_annual_savings"]
                ...
                for eu in euses:
                    ...
                    css_dat_eu = css_dat[numpy.in1d(css_dat["End_Use"], eu)]
                    ...
                    bldg_types = numpy.unique(css_dat_eu["Building_Type"])
                    czones = numpy.unique(css_dat_eu["Climate_Zone"])
                    ...
                    for bd in bldg_types:
                        ...
                        for cz in czones:
                            ...
                            sys_v = numpy.unique(css_dat_eu_bldg_cz["Net_Load_Version"])
                            # If "Net_Load_Version" column is blank, set unique
                            # net load versions to 1
                            if len(sys_v) == 0 or (len(sys_v) == 1 and sys_v[0] == -1):
                                sys_v = [1]
                            for sv in sys_v:
                                # Restrict data further to current net load
                                # shape version
                                css_dat_eu_bldg_cz_nlv = css_dat_eu_bldg_cz[
                                    numpy.in1d(css_dat_eu_bldg_cz["Net_Load_Version"], sv)
                                ]
                                ...
                                # not, throw error
                                if not all([len(x) == 8760 for x in [eff_l, base_l]]):
                                    raise ValueError(
                                ...
                                # efficient case)
                                    ...
                                    if ann_load != 0:
                                        ...
                                        base_l_frac[numpy.isnan(base_l_frac)] = 0
                                    else:
                ...
                self.tsv_features["shape"]["custom_annual_savings"] = css_dict
        ...
        for adopt_scheme in handyvars.adopt_schemes_prep:
            ...
            if self.usr_opts["fugitive_emissions"] is not False:
                ...
                self.markets[adopt_scheme]["master_mseg"]["fugitive emissions"] = {
                    "methane": {
                        "total": {"baseline": _cp(init_meth), "efficient": _cp(init_meth)},
                        "competed": {"baseline": _cp(init_meth), "efficient": _cp(init_meth)},
                    },
                    "refrigerants": {
                        "total": {"baseline": _cp(init_refr), "efficient": _cp(init_refr)},
                        "competed": {"baseline": _cp(init_refr), "efficient": _cp(init_refr)},
                    },
                }
            ...
            self.markets[adopt_scheme]["mseg_out_break"] = {
                key: {"baseline": _obi(), "efficient": _obi(), "savings": _obi()}
                for key in ["energy", "carbon", "energy cost"]
            }
            # Add stock breakouts
            self.markets[adopt_scheme]["mseg_out_break"]["stock"] = {
                key: _obi() for key in ["baseline", "efficient"]
            }
            # Add stock cost (capital investment) breakouts
            if self.usr_opts["cap_invest"]:
                self.markets[adopt_scheme]["mseg_out_break"]["capital cost"] = {
                    key: _obi() for key in ["baseline", "efficient", "savings"]
                }
            # Initialize breakouts of efficient energy captured by measure
            # if user does not suppress reporting of this variable
            if self.usr_opts["no_eff_capt"] is not True:
                self.markets[adopt_scheme]["mseg_out_break"]["energy"]["efficient-captured"] = (
                    _obi()
                )

    def fill_mkts(
        self, msegs, msegs_cpl, convert_data, tsv_data_init, opts, ctrb_ms_pkg_prep, tsv_data_nonfs
    ):
        ...
        if opts.sect_shapes is True:
            calc_sect_shapes = (
                (self.technology_type["primary"] != "demand" or not is_in_package)
                and (not agen_ref)
                and (
                    "electricity" in self.fuel_type["primary"]
                    or self.fuel_switch_to == "electricity"
                )
                and (all([x not in self.name for x in ["Gas", "Oil", "N-Elec", "Fossil"]]))
            )
        else:
        ...
        if calc_sect_shapes is True:
            ...
            self.sector_shapes = {a_s: {} for a_s in self.handyvars.adopt_schemes_run}
            ...
            for a_s in self.handyvars.adopt_schemes_run:
                self.sector_shapes[a_s] = {
                    reg: {
                        yr: {
                            "baseline": [0 for x in range(8760)],
                            "efficient": [0 for x in range(8760)],
                        }
        ...
        if any(
            [
                x in ["single family home", "mobile home", "multi family home"]
        ...
        if not dmd_meas and (
            len(self.end_use["primary"]) > 1
            and any([x in self.end_use["primary"] for x in ["heating", "cooling"]])
        ):
            ...
            if dmd_meas:
                ...
                try:
                    ...
                    self.linked_htcl_tover_anchor_tech = [
                        x
                        for x in linked_htcl_tover_anchor_tech_list
                        if x in self.technology["primary"]
                    ][0]
                except IndexError:
                    ...
                    try:
                        ...
                        if self.linked_htcl_tover_anchor_eu == "heating":
                            ...
                            link_tech_candidates = [
                                x[1]
                                for x in linked_htcl_tover_linked_tech_list.items()
                                if self.linked_htcl_tover_anchor_tech == x[0]
                            ][0]
                        else:
                            link_tech_candidates = [
                                x[0]
                                for x in linked_htcl_tover_linked_tech_list.items()
                                if self.linked_htcl_tover_anchor_tech in x[1]
                            ]
                        # Select first technology in retrieved list that is present in measure def.
                        self.linked_htcl_tover_linked_tech = [
                            x for x in link_tech_candidates if x in self.technology["primary"]
                        ][0]
                    except IndexError:
                        ...
                        raise ValueError(
                            ...
                            "are included in the following list: " + str(link_tech_candidates)
                        )
            ...
            if self.hp_convert_flag:
                try:
                    ...
                    add_cool_anchor_tech_list = [
                        x for x in self.handyvars.htcl_anchor_tech_opts[bldg_sect]["cooling"]
                    ]
                    self.add_cool_anchor_tech = [
                        x for x in add_cool_anchor_tech_list if x in self.technology["primary"]
                    ][0]
                except IndexError:
                    ...
                    raise ValueError(
                        ...
                        "measure '" + self.name + "'. Check measure "
                        "'technology' attribute to ensure one or more items "
                        "are included in the following list: " + str(add_cool_anchor_tech_list)
                    )
        ...
        if self.handyvars.panel_shares is not None:
            ...
            if self.linked_htcl_tover is not None:
                anchor_techs = self.handyvars.htcl_anchor_tech_opts[bldg_sect][
                    self.linked_htcl_tover_anchor_eu
                ]
            else:
                anchor_techs = None
        ...
        if self.linked_htcl_tover:
            ...
            ms_iterable_comes_first = [
                x
                for x in ms_iterable
                if self.linked_htcl_tover_anchor_eu in x
                and any([y in x for y in anchor_tech_w_alts])
            ]
            ...
            ms_iterable_comes_last = [
                x
                for x in ms_iterable
                if self.linked_htcl_tover_anchor_eu not in x
                or all([y not in x for y in anchor_tech_w_alts])
            ]
            ...
            if len(ms_iterable) != iter_len_check:
                raise ValueError(
                    "Microsegments list for measure '"
                    + self.name
        ...
        if self.end_use["secondary"] is not None:
            ms_iterable_second, ms_lists_second = self.create_keychain("secondary")
            ms_iterable.extend(ms_iterable_second)
        elif (
            "lighting" in self.end_use["primary"]
            and (not opts or opts.no_scnd_lgt is not True)
            and any([x not in self.end_use["primary"] for x in ["heating", "cooling"]])
            and any(
                [
            ...
            self.energy_efficiency["secondary"] = "Missing (secondary lighting)"
            # Set secondary energy efficiency units to "relative
            # savings"
            self.energy_efficiency_units["secondary"] = "relative savings (constant)"
            # Set secondary fuel type to include all heating/cooling
            # fuels
            self.fuel_type["secondary"] = ["electricity", "natural gas", "distillate"]
        ...
        if "demand" in self.technology_type["primary"] or (
            all(
                [
                    x not in ["single family home", "mobile home", "multi family home"]
                    for x in self.bldg_type
                ]
            )
            and all([x in self.handyvars.com_eqp_eus_nostk for x in self.end_use["primary"]])
        ):
        ...
        for ind, mskeys in enumerate(ms_iterable):
            ...
            if (
                "demand" not in mskeys
                and "secondary heating" in mskeys
                and "heating" in self.end_use["primary"]
                and all(
                    [
                        x
                        not in self.handyvars.in_all_map["technology"]["residential"]["supply"][
                            mskeys[3]
                        ]["heating"]
            ...
            if self.tech_switch_to not in [None, "NA", "same"] and (
                ...
                if bldg_sect == "residential":
                    ...
                    # HVAC
                    if mskeys[4] in ["heating", "cooling"]:
                        if "ASHP" in self.tech_switch_to and mskeys[4] == "heating":
                            mskeys_swtch_tech = "rooftop_ASHP-heat"
                        elif "ASHP" in self.tech_switch_to and mskeys[4] == "cooling":
                            mskeys_swtch_tech = "rooftop_ASHP-cool"
                        elif "GSHP" in self.tech_switch_to and mskeys[4] == "heating":
                            mskeys_swtch_tech = "comm_GSHP-heat"
                        elif "GSHP" in self.tech_switch_to and mskeys[4] == "cooling":
            ...
            (
                ...
                rmv_lnkd_tech_costs,
            ) = self.set_lnkd_cost_exclude(mskeys)
            ...
            if bldg_sect in self.retro_rate.keys():
                ...
                # HVAC or envelope microsegments
                    ...
                    # Envelope microsegment ("demand" tech. type)
                        ...
                        try:
                            retro_rate_mseg = self.retro_rate[bldg_sect][mskeys[-2]]
                        # Windows require special handling b/c windows
                        # microsegment tech. is broken into conduction/solar
                        except KeyError:
                            retro_rate_mseg = self.retro_rate[bldg_sect]["windows"]
                # All other microsegments – set early retrofit rate to zero
                else:
                    retro_rate_mseg = {yr: 0 for yr in self.handyvars.aeo_years}
            ...
            if (
                (perf_meas is _sentinel)
                or (ms_iterable[ind][0] != ms_iterable[ind - 1][0])
                or isinstance(self.energy_efficiency, dict)
            ):
                perf_meas = self.energy_efficiency
            if (
                (perf_units is _sentinel)
                or (ms_iterable[ind][0] != ms_iterable[ind - 1][0])
            ...
            if mskeys[0] == "secondary":
                ...
                # microsegments
                mkt_scale_frac, mkt_scale_frac_source = (None for n in range(2))
            else:
                ...
                if (reg_adj_flag and (ms_iterable[ind][1] != ms_iterable[ind - 1][1])) or (
                    ...
                    cost_units, cost_meas = [self.cost_units, self.installed_cost]
                ...
                # reset cost/cost units to those of original measure
                    ...
                    (sqft_subst == 1 or "$/ft^2 floor" in self.cost_units)
                    or (cost_meas is _sentinel)
                    or (ms_iterable[ind][0] != ms_iterable[ind - 1][0])
                    or (ms_iterable[ind][4] != ms_iterable[ind - 1][4])
                    or (ms_iterable[ind][-1] != ms_iterable[ind - 1][-1])
                    or any([isinstance(x, dict) for x in [self.installed_cost, self.cost_units]])
                ):
                    cost_units, cost_meas = [self.cost_units, self.installed_cost]
                # Set lifetime attribute to initial value
                if (life_meas is _sentinel) or isinstance(self.product_lifetime, dict):
                    life_meas = self.product_lifetime
                # Set market scaling attributes to initial values
                if (mkt_scale_frac is _sentinel) or isinstance(self.market_scaling_fractions, dict):
            ...
            if (
                opts.grid_decarb is not False
                and all(
                    [
                        x is not None
                        for x in [self.handyvars.carb_int_nonfs, self.handyvars.ecosts_nonfs]
                    ]
                )
                and (
                    self.fuel_switch_to is None
                    or (self.fuel_switch_to == "electricity" and "electricity" in mskeys)
                )
            ):
            ...
            if self.fuel_switch_to is None:
                ...
                if opts is not None and opts.site_energy is True:
                    ...
                    try:
                        intensity_carb_base, intensity_carb_meas = [
                            {
                                yr: carb_int_dat[bldg_sect][ftkey][mskeys[1]][yr]
                                for yr in self.handyvars.aeo_years
                            }
                            for n in range(2)
                        ]
                    ...
                    # to match the user's site energy setting
                        ...
                        intensity_carb_base, intensity_carb_meas = [
                            {
                                yr: carb_int_dat[bldg_sect][ftkey][yr]
                ...
                # Case where user has not flagged site energy outputs
                    ...
                    # user's source energy setting
                        ...
                        intensity_carb_base, intensity_carb_meas = (
                            carb_int_dat[bldg_sect][ftkey] for n in range(2)
                        )
            else:
                ...
                if opts is not None and opts.site_energy is True:
                    ...
                    try:
                        ...
                        intensity_carb_base = carb_int_dat[bldg_sect][ftkey][mskeys[1]]
                    except KeyError:
                    ...
                    try:
                        ...
                        intensity_carb_meas = carb_int_dat[bldg_sect][self.fuel_switch_to][
                            mskeys[1]
                        ]
                    except KeyError:
                        # Measure fuel intensity not broken by region
                        intensity_carb_meas = {
                            yr: carb_int_dat[bldg_sect][self.fuel_switch_to][yr]
                ...
                # Case where user has not flagged site energy outputs
                    ...
                    try:
                        ...
                        # Base fuel intensity not broken by region
                        intensity_carb_base = carb_int_dat[bldg_sect][ftkey]
                    try:
                        # Measure fuel intensity broken by region
                        intensity_carb_meas = {
                            yr: carb_int_dat[bldg_sect][self.fuel_switch_to][mskeys[1]][yr]
                            / self.handyvars.ss_conv[self.fuel_switch_to][yr]
                            for yr in self.handyvars.aeo_years
                        }
                    except KeyError:
            ...
            if self.fuel_switch_to is None:
                ...
                if opts is not None and opts.site_energy is True:
                    ...
                    try:
                        cost_energy_base, cost_energy_meas = (
                            cost_dat[bldg_sect][ftkey][mskeys[1]] for n in range(2)
                        )
                    # National fuel costs
                    except KeyError:
                        cost_energy_base, cost_energy_meas = [
                            {
                                yr: cost_dat[bldg_sect][ftkey][yr]
                                * self.handyvars.ss_conv[ftkey][yr]
                                for yr in self.handyvars.aeo_years
                            }
                            for n in range(2)
                ...
                # Case where user has not flagged site energy outputs
                    ...
                    try:
                        cost_energy_base, cost_energy_meas = [
                            {
                                yr: cost_dat[bldg_sect][ftkey][mskeys[1]][yr]
                                / self.handyvars.ss_conv[ftkey][yr]
                                for yr in self.handyvars.aeo_years
                            }
                            for n in range(2)
                    ...
                    # National fuel costs
                        ...
                        cost_energy_base, cost_energy_meas = (
                            cost_dat[bldg_sect][ftkey] for n in range(2)
                        )
            else:
                # Case where use has flagged site energy outputs
                if opts is not None and opts.site_energy is True:
                    try:
                        ...
                        cost_energy_base = cost_dat[bldg_sect][ftkey][mskeys[1]]
                    except KeyError:
                        # Base fuel cost not broken out by region
                        cost_energy_base = {
                            yr: cost_dat[bldg_sect][ftkey][yr] * self.handyvars.ss_conv[ftkey][yr]
                            for yr in self.handyvars.aeo_years
                        }
                    try:
                        # Measure fuel cost broken out by region
                        cost_energy_meas = cost_dat[bldg_sect][self.fuel_switch_to][mskeys[1]]
                    except KeyError:
                        # Measure fuel cost not broken out by region
                        cost_energy_meas = {
                            yr: cost_dat[bldg_sect][self.fuel_switch_to][yr]
                            * self.handyvars.ss_conv[self.fuel_switch_to][yr]
                            for yr in self.handyvars.aeo_years
                        }
                # Case where user has not flagged site energy outputs
                    ...
                    try:
                        ...
                        # Base fuel cost not broken out by region
                        cost_energy_base = cost_dat[bldg_sect][ftkey]
                    try:
                        # Measure fuel cost broken out by region
                        cost_energy_meas = {
                            yr: cost_dat[bldg_sect][self.fuel_switch_to][mskeys[1]][yr]
                            / self.handyvars.ss_conv[self.fuel_switch_to][yr]
                            for yr in self.handyvars.aeo_years
                        }
                    except KeyError:
                        # Measure fuel cost not broken out by region
                        cost_energy_meas = cost_dat[bldg_sect][self.fuel_switch_to]
            ...
            if opts.health_costs is not False and (
                "PHC" in self.name
                and ("electricity" in mskeys or self.fuel_switch_to == "electricity")
            ):
                # Set row/column key information for the public health
                # cost scenario suggested by the measure name
                row_key = [x[1] for x in self.handyvars.health_scn_names if x[0] in self.name][0]
                col_key = [x[2] for x in self.handyvars.health_scn_names if x[0] in self.name][0]
                ...
                phc_dat = {
                    yr: (
                        (
                            self.handyvars.health_scn_data[
                                numpy.in1d(self.handyvars.health_scn_data["Category"], row_key)
                                & numpy.in1d(
                                    self.handyvars.health_scn_data["EMM_Region"], mskeys[1]
                                )
                            ][col_key]
                        )[0]
                        / 100
                ...
                if self.fuel_switch_to == "electricity":
                    # Update measure
                    cost_energy_meas = {
                        yr: (val + phc_dat[yr]) for yr, val in cost_energy_meas.items()
                    }
                else:
                    # Update baseline
                    cost_energy_base = {
                        yr: (val + phc_dat[yr]) for yr, val in cost_energy_base.items()
                    }
                    # Update measure
                    cost_energy_meas = {
                        yr: (val + phc_dat[yr]) for yr, val in cost_energy_meas.items()
            ...
            new_constr = {"annual new": {}, "total new": {}, "total": {}, "new fraction": {}}
            ...
            for i in range(0, len(mskeys)):
                ...
                if (i == 1) and self.handyvars.region_cpl_mapping:
                    reg_cpl_map = [
                        x[0] for x in self.handyvars.region_cpl_mapping.items() if mskeys[1] in x[1]
                    ][0]
                    # Mapping should yield single string for census division
                    if not isinstance(reg_cpl_map, str):
                        raise ValueError(
                            "State " + mskeys[1] + " could not be mapped to a census "
                ...
                if (
                    isinstance(base_cpl, dict)
                    and (
                        (key_item in base_cpl.keys())
                        or (reg_cpl_map and reg_cpl_map in base_cpl.keys())
                    )
                    or key_item in ["primary", "secondary", "new", "existing", None]
                ):
                    ...
                    # categories)
                    if key_item not in ["primary", "secondary", "new", "existing"]:
                        ...
                        if reg_cpl_map:  # Handle region mapping
                            ...
                            # applicable
                            if base_cpl_swtch:
                                base_cpl_swtch = base_cpl_swtch[reg_cpl_map]
                        else:
                            ...
                            if base_cpl_swtch and key_item_swtch is not None:
                                try:
                                    base_cpl_swtch = base_cpl_swtch[key_item_swtch]
                                except KeyError:
                                    raise KeyError(
                    ...
                    if any(
                        [
                            type(x) is dict and "undefined" in x.keys()
                            for x in [perf_meas, perf_units, cost_meas, cost_units, life_meas]
                        ]
                    ):
                    ...
                    if any(
                        [
                            (type(x) is dict or type(x) is list)
                            for x in [
                                perf_meas,
                                perf_units,
                                cost_meas,
                        ...
                        # full set of climate breakouts
                            ...
                            alt_break_keys = self.handyvars.alt_attr_brk_map["levels"]
                        ...
                        # full set of building breakouts
                            ...
                            # inputs by end use.)
                            bldg_typs_for_brks = [
                                x for x in self.bldg_type if x not in self.end_use[mskeys[0]]
                            ]
                        ...
                        # full set of technology breakouts
                            ...
                            alt_break_keys = ""
                            err_message = "technologies the measure applies to: "
                        # full set of vintage breakouts
                        elif i == (len(mskeys) - 1):
                            break_keys = self.structure_type
                            alt_break_keys = ""
                            err_message = "building vintages the measure applies to: "
                        else:
                            break_keys = ""
                            alt_break_keys = ""
                            err_message = ""
                        ...
                        if (
                            isinstance(perf_meas, dict)
                            and break_keys
                            and all([x in perf_meas.keys() for x in break_keys])
                        ):
                        ...
                        # and performance data use alternate regional breakout
                            ...
                            alt_key_reg_typ = [
                                x
                                for x in self.handyvars.alt_attr_brk_map.keys()
                                if any([x in y for y in perf_meas.keys()])
                            ]
                            ...
                            if len(alt_key_reg_typ) > 0:
                                ...
                                if sorted(perf_meas.keys()) == sorted(
                                    self.handyvars.alt_attr_brk_map[alt_key_reg_typ][
                                        alt_key_reg_typ
                                    ]
                                ):
                                    ...
                                    perf_meas = [
                                        dict(perf_meas),
                                        self.handyvars.alt_attr_brk_map[alt_key_reg_typ][mskeys[1]],
                                    ]
                                # If unexpected keys are present, yield error
                                else:
                                    raise KeyError(
                                        self.name + " energy performance (energy_"
                                        "efficiency) must be broken out "
                                        "by ALL " + err_message + str(break_keys) + " OR alternate "
                        ...
                        # current mseg
                        elif isinstance(perf_meas, list) and isinstance(perf_meas[0], dict):
                            ...
                            for k in perf_meas[0].keys():
                                if (
                                    isinstance(perf_meas[0][k], dict)
                                    and break_keys
                                    and all([x in perf_meas[0][k].keys() for x in break_keys])
                                ):
                                    perf_meas[0][k] = perf_meas[0][k][key_item]
                            ...
                            # region
                            if all([not isinstance(x, dict) for x in perf_meas[0].values()]):
                                perf_meas = sum(
                                    [x * y for x, y in zip(perf_meas[0].values(), perf_meas[1])]
                                )
                        # If none of the above cases holds, yield error
                            ...
                            raise KeyError(
                                self.name + " energy performance (energy_efficiency) "
                                "must be broken out "
                        ...
                        # Case where data are broken out directly by mseg info.
                        if (
                            isinstance(cost_meas, dict)
                            and break_keys
                            and all([x in cost_meas.keys() for x in break_keys])
                        ):
                        ...
                        # and cost data use alternate regional breakout
                            ...
                            if len(alt_key_reg_typ) > 0:
                                ...
                                # If unexpected keys are present, yield error
                                    ...
                                    raise KeyError(
                                        self.name + " installed cost (installed_"
                        ...
                        # further work to finalize as a single value for the
                        # current mseg
                        elif isinstance(cost_meas, list) and isinstance(cost_meas[0], dict):
                            # Check the first element of the list for
                            ...
                            for k in cost_meas[0].keys():
                                if (
                                    isinstance(cost_meas[0][k], dict)
                                    and break_keys
                                    and all([x in cost_meas[0][k].keys() for x in break_keys])
                                ):
                                    cost_meas[0][k] = cost_meas[0][k][key_item]
                            ...
                            if all([not isinstance(x, dict) for x in cost_meas[0].values()]):
                                cost_meas = sum(
                                    [x * y for x, y in zip(cost_meas[0].values(), cost_meas[1])]
                                )
                            ...
                            if alt_break_keys:
                                ...
                                raise KeyError(
                                    self.name + " installed cost (installed_cost) must "
                                    "be broken out "
                                    "by ALL " + err_message + str(break_keys)
                                )
                        ...
                        if (
                            ...
                            and break_keys
                            and all([x in perf_units.keys() for x in break_keys])
                        ):
                            ...
                            raise KeyError(
                                self.name + " energy performance units ("
                        ...
                        if (
                            ...
                            and break_keys
                            and all([x in cost_units.keys() for x in break_keys])
                        ):
                            ...
                            raise KeyError(
                                self.name + " installed cost units (installed_cost_"
                                "units) must be broken out "
                                "by ALL " + err_message + str(break_keys)
                            )
                        # Lifetime data
                        if (
                            isinstance(life_meas, dict)
                            and break_keys
                            and all([x in life_meas.keys() for x in break_keys])
                        ):
                            ...
                            raise KeyError(
                                self.name + " lifetime (product_lifetime) must be "
                                "broken out "
                                "by ALL " + err_message + str(break_keys)
                            )
                        ...
                        if (
                            ...
                            and break_keys
                            and all([x in mkt_scale_frac.keys() for x in break_keys])
                        ):
                            mkt_scale_frac = mkt_scale_frac[key_item]
                        # Case where region is being looped through in the mseg
                        # and scaling data use alternate regional breakout
                        elif isinstance(mkt_scale_frac, dict) and alt_break_keys:
                            # Determine the alternate regions by which the
                            ...
                            alt_key_reg_typ = [
                                x
                                for x in self.handyvars.alt_attr_brk_map.keys()
                                if any([x in y for y in mkt_scale_frac.keys()])
                            ]
                            # If the alternate regional breakout is supported,
                            ...
                            if len(alt_key_reg_typ) > 0:
                                ...
                                # If unexpected keys are present, yield error
                                    ...
                                    raise KeyError(
                                        self.name + " market scaling fractions (mkt_"
                        ...
                        # further work to finalize as a single value for the
                        # current mseg
                        elif isinstance(mkt_scale_frac, list) and isinstance(
                            mkt_scale_frac[0], dict
                        ):
                            # Check the first element of the list for
                            ...
                            for k in mkt_scale_frac[0].keys():
                                if (
                                    isinstance(mkt_scale_frac[0][k], dict)
                                    and break_keys
                                    and all([x in mkt_scale_frac[0][k].keys() for x in break_keys])
                                ):
                                    mkt_scale_frac[0][k] = mkt_scale_frac[0][k][key_item]
                            ...
                            # that region
                            if all([not isinstance(x, dict) for x in mkt_scale_frac[0].values()]):
                                mkt_scale_frac = sum(
                                    [
                                        x * y
                                        for x, y in zip(
                                            mkt_scale_frac[0].values(), mkt_scale_frac[1]
                                        )
                                    ]
                                )
                        elif isinstance(mkt_scale_frac, dict) and any(
                            [x in mkt_scale_frac.keys() for x in break_keys]
                        ):
                            raise KeyError(
                                self.name + " market scaling fractions (market_"
                                "scaling_fractions) must be "
                                "broken out by ALL " + err_message + str(break_keys)
                            )
                        # Market scaling fraction source
                        if (
                            isinstance(mkt_scale_frac_source, dict)
                            and break_keys
                            and all([x in mkt_scale_frac_source.keys() for x in break_keys])
                        ):
                            mkt_scale_frac_source = mkt_scale_frac_source[key_item]
                        elif isinstance(mkt_scale_frac_source, dict) and any(
                            [x in mkt_scale_frac_source.keys() for x in break_keys]
                        ):
                            raise KeyError(
                                self.name + " market scaling fraction source (market_"
            ...
            elif ("demand" in mskeys and self.name not in ctrb_ms_pkg_prep) and (
                (
                    (opts.tsv_metrics is not False or opts.sect_shapes is True)
                    or self.tsv_features is not None
                )
                and (
                    mskeys[4] in ["heating", "secondary heating"]
                    and mskeys[-2] in self.handyvars.env_heat_ls_scrn
                )
            ):
                continue
            ...
            # Otherwise update all stock/energy/cost information for each year
                ...
                if (
                    (self.handyvars.hp_rates and "demand" not in mskeys)
                    and ("stove (wood)" not in self.technology["primary"])
                    and (mskeys[-2] is None or "HP" not in mskeys[-2])
                    and (
                        any(
                            [
                                mskeys[4] == x
                        ...
                        or (
                            self.linked_htcl_tover and self.linked_htcl_tover_anchor_eu == "heating"
                        )
                    )
                ):
                    hp_rate_flag = True
                ...
                if hp_rate_flag:
                    ...
                    # replacement measure
                        ...
                        if any(
                            [
                                x in self.technology["primary"]
                                for x in self.handyvars.resist_ht_wh_tech
                            ]
                        ) or (
                            "electricity" in mskeys
                            and any([x in mskeys for x in ["cooking", "drying"]])
                        ):
                ...
                if comp_tag_exog or comp_tag_htcl_pair:
                    ...
                    try:
                        contrib_mseg_key[-2] = (
                            contrib_mseg_key[-2] + "-" + comp_tag_exog + comp_tag_htcl_pair
                        )
                ...
                if hp_rate_flag:
                    ...
                    if "gh" in opts.exog_hp_rates[0]:
                        ...
                        reg = [
                            r[0]
                            for r in self.handyvars.hp_rates_reg_map.items()
                            if mskeys[1] in r[1]
                        ][0]
                        ...
                        hp_rate_dat = self.handyvars.hp_rates["data (by scenario)"][
                            opts.exog_hp_rates[0]
                        ][reg][bldg_sect]
                        ...
                        try:
                            hp_rate = hp_rate_dat[mskeys[3]][hp_eu_key][mskeys[-2]][mskeys[-1]]
                        except KeyError:
                            try:
                                hp_rate = hp_rate_dat[mskeys[3]][hp_eu_key]["all"][mskeys[-1]]
                            except KeyError:
                                # Handle switch from commercial heating in RTUs vs.
                                # other technologies
                                if (
                                    hp_eu_key == "heating"
                                    and bldg_sect == "commercial"
                                    and any(
                                        [
                                            mskeys[-2] in x
                                            for x in [
                                    ...
                                    if mskeys[-2] in self.handyvars.com_RTU_fs_tech:
                                        tech_key = "RTUs"
                                    else:
                                        tech_key = "all other"
                                    # Try resultant tech. key
                                    try:
                                        hp_rate = hp_rate_dat[mskeys[3]][hp_eu_key][tech_key][
                                            mskeys[-1]
                                        ]
                                    except KeyError:
                                        hp_rate = None
                                # Residential secondary heating
                                    ...
                                    mskeys[4] == "secondary heating" and bldg_sect == "residential"
                                ):
                                ...
                                # mseg that does have a rate; set flag
                                    ...
                                    self.linked_htcl_tover
                                    and hp_eu_key != self.linked_htcl_tover_anchor_eu
                                ):
                                    hp_rate = "linked"
                                else:
                                    hp_rate = None
                    ...
                    # competed), region, building type, fuel type, end use, and vintage
                        ...
                        hp_rate_in = self.handyvars.hp_rates["data (by scenario)"][
                            opts.exog_hp_rates[0]
                        ]
                        ...
                        try:
                            hp_rate = {
                                out_typ: hp_rate_in[out_typ][mskeys[1]]
                                for out_typ in hp_rate_in.keys()
                            }
                            try:
                                ...
                                fuel_detail, fuel_simple = [
                                    ...
                                    for y in [
                                        self.handyvars.detailed_fuel_map,
                ...
                if (
                    isinstance(mkt_scale_frac_source, dict)
                    and "title" in mkt_scale_frac_source.keys()
                ):
                    ...
                    if sbmkt_derive_invalid == 1 or (
                        sbmkt_source_invalid == 1 and sbmkt_url_invalid == 1
                    ):
                ...
                if isinstance(perf_meas, list) and isinstance(perf_meas[0], str):
                    # Sample measure performance values
                    perf_meas = self.rand_list_gen(perf_meas, self.handyvars.nsamples)
                    ...
                    if (
                        ...
                        and type(perf_units) is not list
                        and any(perf_meas < 0) is True
                    ):
                ...
                if isinstance(cost_meas, list) and isinstance(cost_meas[0], str):
                    # Sample measure cost values
                    cost_meas = self.rand_list_gen(cost_meas, self.handyvars.nsamples)
                    # Set any measure cost values less than zero to zero
                    if any(cost_meas < 0) is True:
                        cost_meas[numpy.where(cost_meas < 0)] = 0
                if isinstance(life_meas, list) and isinstance(life_meas[0], str):
                    # Sample measure lifetime values
                    life_meas = self.rand_list_gen(life_meas, self.handyvars.nsamples)
                    # Set any measure lifetime values in list less than zero
                    # to 1
                    if any(life_meas < 0) is True:
                        life_meas[numpy.where(life_meas < 0)] = 1
                elif (
                    isinstance(life_meas, float)
                    or isinstance(life_meas, int)
                ...
                (
                    ...
                    perf_base,
                    life_base,
                ) = ({yr: None for yr in self.handyvars.aeo_years} for n in range(3))
                ...
                if mskeys[0] == "primary":
                    try:
                        ...
                        try:
                            ...
                            perf_base = base_cpl["performance"]["typical"][mskeys[-1]]
                            # Try to pull 'best' tier performance data alongside 'typical'.
                            # If it doesn't work, set typical and best to the same levels.
                            try:
                                perf_base_best = base_cpl["performance"]["best"][mskeys[-1]]
                            except KeyError:
                                perf_base_best = base_cpl["performance"]["typical"][mskeys[-1]]
                        except KeyError:
                            # Typical performance level
                        ...
                        if "incentives" in base_cpl["installed cost"].keys():
                            ...
                            cost_base_init = base_cpl["installed cost"]["before incentives"]
                            ...
                            if (
                                opts.incentive_restrictions != "all"
                                and (self.measure_type != "add-on" or ref_addon)
                                and "relative savings" not in perf_units_incents
                            ):
                                # Pull out baseline incentives data
                                cost_incentives = base_cpl["installed cost"]["incentives"][
                                    "by performance tier"
                                ]
                                ...
                                i_units_base = base_cpl["installed cost"]["incentives"][
                                    "performance units"
                                ]
                                ...
                                if base_cpl_swtch:
                                    ...
                                    cost_incentives_meas = base_cpl_swtch["installed cost"][
                                        "incentives"
                                    ]["by performance tier"]
                                    # Further restrict incentives by new or
                                    # existing, if applicable
                                    if mskeys_swtch[-1] in cost_incentives_meas.keys():
                                        cost_incentives_meas = cost_incentives_meas[mskeys[-1]]
                                    # Set performance units for measure mseg
                                    # incentives
                                    i_units_meas = base_cpl_swtch["installed cost"]["incentives"][
                                        "performance units"
                                    ]
                                else:
                                    cost_incentives_meas, i_units_meas = ("" for n in range(2))
                            else:
                            ...
                            # No incentives data
                            cost_incentives, cost_incentives_meas, i_units_base, i_units_meas = (
                                "" for n in range(4)
                            )
                        ...
                        if (
                            self.handyvars.incentives is not None
                            and len(self.handyvars.incentives) != 0
                        ):
                            ...
                            incent_mod_base = [
                                x
                                for x in incent_mod
                                if all(f == mskeys[3] for f in [x[5], x[6]])  # fuel
                                and (x[4] == "all" or x[4] == mskeys[-2])
                            ]  # tech
                            ...
                            if base_cpl_swtch:
                                ...
                                incent_mod_swtch = [
                                    ...
                                    if (
                                        (
                                            (
                                                (self.fuel_switch_to is None)  # no fuel switch
                                                or (
                                                    self.backup_fuel_fraction
                                                    is None  # no backup or backup ok
                                                    or self.backup_fuel_fraction is not None
                                                    and x[7] == "yes"
                                                )
                                            )
                                            and (x[5] == mskeys_swtch[3] and x[6] == mskeys[3])
                                        )  # fuel
                                        and (x[4] == "all" or x[4] == mskeys_swtch[-2])
                                    )
                                ]  # tech
                        ...
                        # year if necessary
                        if type(base_cpl["lifetime"]["average"]) in [float, int]:
                            life_base = {yr: life_base for yr in self.handyvars.aeo_years}
                        ...
                        if any(
                                ...
                                (
                                    (
                                        (
                                            "lighting" in mskeys
                                            and (
                                                isinstance(x[1], float) and round(x[1]) in [0, 999]
                                            )
                                        )
                                        or x[1] in [0, "NA", 999]
                                    )
                            ...
                            mx_cb = round(max([x[1] for x in cost_base.items() if x[1] != "NA"]))
                            if mx_cb not in [0, 999]:
                                cost_base = {yr: mx_cb for yr in self.handyvars.aeo_years}
                            else:
                                raise ValueError
                        ...
                        if any([z[1] in [0, "NA"] for z in life_base.items()]):
                            ...
                            # full time horizon
                            mx_lf = round(max([x[1] for x in life_base.items() if x[1] != "NA"]))
                            if mx_lf not in [0, 999]:
                                life_base = {yr: mx_lf for yr in self.handyvars.aeo_years}
                            else:
                                raise ValueError
                        ...
                        if (
                            not (
                                perf_units == "relative savings (constant)"
                                or (
                                    isinstance(perf_units, list)
                                    and perf_units[0] == "relative savings (dynamic)"
                                )
                            )
                            and (perf_units != perf_base_units)
                        ) and (perf_base_units in self.handyvars.tech_units_rmv):
                        ...
                        # units (for example, AFUE to COP)
                            ...
                            and (perf_units != perf_base_units)
                        ) and (perf_units in self.handyvars.tech_units_map.keys()):
                            convert_fact = self.handyvars.tech_units_map[perf_units][
                                perf_base_units
                            ]
                        ...
                        if (
                            not isinstance(perf_units, list)
                        ...
                        if bldg_sect == "residential" and (
                            mskeys[-1] == "existing"
                            and (mskeys[-2] is not None and "HP" in mskeys[-2])
                        ):
                            # Multiply heat pump baseline cost units by 2
                            # to account for EIA heat pump cost handling
                            cost_base = {yr: cost_base[yr] * 2 for yr in self.handyvars.aeo_years}
                        ...
                        # baseline segment.
                        if (
                            self.measure_type == "add-on" or "secondary heating" in mskeys
                        ) and perf_units == "relative savings (constant)":
                            # Set baseline cost to zero with appropriate units
                            # for the building sector of interest
                            cost_base = {yr: 0 for yr in self.handyvars.aeo_years}
                            if sqft_subst == 1:
                            ...
                            try:
                                if any(
                                    [
                                        x[1] in [0, "NA"]
                                        for x in base_cpl["lifetime"]["average"].items()
                                    ]
                                ):
                                    raise ValueError
                                life_base = base_cpl["lifetime"]["average"]
                            except (TypeError, ValueError):
                                life_base = {yr: 10 for yr in self.handyvars.aeo_years}
                        ...
                        # available, set the baseline lifetime to 10 years.
                            ...
                            # performance/units
                            perf_base, perf_base_best = (
                                {yr: perf_meas for yr in self.handyvars.aeo_years} for n in range(2)
                            )
                            perf_base_units = perf_units
                            ...
                            cost_base = {
                                yr: cost_meas if not isinstance(cost_meas, dict) else cost_meas[yr]
                                for yr in self.handyvars.aeo_years
                            }
                    ...
                    cost_base, perf_base, perf_base_best, life_base = [
                        {yr: x for yr in self.handyvars.aeo_years}
                        for x in [cost_meas, perf_meas, perf_meas, 10]
                    ]
                ...
                if any([x in cost_units for x in ["$/kBtu/h heating", "$/kBtu/h cooling"]]):
                    if mskeys[4] in ["heating", "cooling"] or (
                        mskeys[4] == "ventilation"
                        and any([x in ms_lists[3] for x in ["heating", "cooling"]])
                    ):
                        # Separate measure and baseline cost units into
                        # the year vs. everything else
                        cost_meas_units_unpack, cost_base_units_unpack = [
                            re.search(r"(\d*)(.*)", x) for x in [cost_units, cost_base_units]
                        ]
                        # Measure units preserve measure cost year but
                        # otherwise switch to baseline cost units
                        cost_units = cost_meas_units_unpack.group(1) + cost_base_units_unpack.group(
                            2
                        )
                    else:
                        raise ValueError(
                ...
                if mskeys[0] == "primary" and not no_stk_mseg and cost_base_units != cost_units:
                    ...
                    # years
                        ...
                        for yr in self.handyvars.aeo_years:
                            cost_meas[yr], cost_units, cost_base_units = self.convert_costs(
                                convert_data,
                                bldg_sect,
                                mskeys,
                                cost_meas[yr],
                ...
                if bldg_sect == "residential" and (
                    sqft_subst == 1 or (sqft_subst != 1 and "$/ft^2 floor" in cost_units)
                ):
                    ...
                    if sf_to_house_key not in self.handyvars.sf_to_house.keys():
                        ...
                        self.handyvars.sf_to_house[sf_to_house_key] = {
                            ...
                            / (mseg_sqft_stock["total square footage"][yr] * 1000000)
                            for yr in self.handyvars.aeo_years
                        }
                    ...
                    try:
                        cost_meas = {
                            yr: cost_meas[yr] / self.handyvars.sf_to_house[sf_to_house_key][yr]
                            for yr in self.handyvars.aeo_years
                        }
                    except (IndexError, TypeError):
                        cost_meas = {
                            yr: cost_meas / self.handyvars.sf_to_house[sf_to_house_key][yr]
                            for yr in self.handyvars.aeo_years
                        }
                    ...
                    if sqft_subst == 1:
                        cost_base = {
                            yr: cost_base[yr] / self.handyvars.sf_to_house[sf_to_house_key][yr]
                            for yr in self.handyvars.aeo_years
                        }
                ...
                if opts.pkg_env_sep is True and "(CF)" in self.name:
                    ...
                    cost_meas = {yr: cost_base[yr] for yr in self.handyvars.aeo_years}
                ...
                # which are defined and assessed like normal measures)
                    ...
                    perf_meas = {yr: perf_base[yr] for yr in self.handyvars.aeo_years}
                    # For cost, ensure that measure is not listed as add-on;
                    # if so, added cost is zero
                    if self.measure_type == "add-on":
                        cost_meas = {yr: 0 for yr in self.handyvars.aeo_years}
                    else:
                        cost_meas = {yr: cost_base[yr] for yr in self.handyvars.aeo_years}
                ...
                # explicit check of baseline units needed in this case)
                    ...
                    or perf_base_units == perf_units
                ) and (mskeys[0] == "secondary" or no_stk_mseg or cost_base_units == cost_units):
                    ...
                    if perf_units == "relative savings (constant)" or (
                        ...
                        if (
                            not isinstance(perf_meas, numpy.ndarray)
                            and perf_meas == "Missing (secondary lighting)"
                        ):
                            ...
                            for yr in self.handyvars.aeo_years:
                                ...
                                if isinstance(perf_units, list):
                                    ...
                                    if perf_units[1] < int(self.handyvars.aeo_years[0]):
                                        perf_units[1] = int(self.handyvars.aeo_years[0])
                                    try:
                                        if (
                                            perf_base_units
                                            not in self.handyvars.inverted_relperf_list
                                        ):
                                            perf_meas = 1 - (
                                                perf_base[yr]
                                                / (
                                    ...
                                    if isinstance(perf_meas, numpy.ndarray):
                                        if any(perf_meas > 1):
                                            ...
                                            perf_meas[numpy.where(perf_meas < 0)] = 0
                                    elif not isinstance(perf_meas, numpy.ndarray) and perf_meas > 1:
                                        perf_meas = 1
                                    elif (
                                        not isinstance(perf_meas, numpy.ndarray)
                                        and perf_meas < 0
                                        and perf_meas_orig > 0
                                    ):
                                        perf_meas = 0
                                # Calculate relative performance
                                rel_perf[yr] = 1 - perf_meas
                    elif perf_units not in self.handyvars.inverted_relperf_list:
                        for yr in self.handyvars.aeo_years:
                            ...
                            with numpy.errstate(all="ignore"):
                                ...
                                if (
                                    not isinstance(rel_perf[yr], numpy.ndarray)
                                    and (not numpy.isfinite(rel_perf[yr]))
                                ) or (
                                    isinstance(rel_perf[yr], numpy.ndarray)
                                    and any([not numpy.isfinite(x) for x in rel_perf[yr]])
                                ):
                                    rel_perf[yr] = 1
                    else:
                        for yr in self.handyvars.aeo_years:
                    ...
                    if (
                        mskeys[4] == "lighting"
                        and mskeys[0] == "primary"
                        and light_scnd_autoperf is True
                    ):
                    ...
                    if (
                        opts.rp_persist is True
                        and not self.target
                        and (opts.add_typ_eff is not True or not agen_ref)
                    ):
                        # Escalate relative performance
                        rel_perf = {
                            yr: rel_perf[str(self.market_entry_year)]
                            for yr in self.handyvars.aeo_years
                        }
                        ...
                        cost_chg = {
                            yr: cost_base[yr] / cost_base[str(self.market_entry_year)]
                            if (
                                numpy.isfinite(cost_base[str(self.market_entry_year)])
                                and cost_base[str(self.market_entry_year)] != 0
                            )
                            else 1
                            for yr in self.handyvars.aeo_years
                        }
                        ...
                        try:
                            cost_meas = {
                                yr: cost_meas * cost_chg[yr] for yr in self.handyvars.aeo_years
                            }
                        except TypeError:
                            cost_meas = {
                                yr: cost_meas[str(self.market_entry_year)] * cost_chg[yr]
                                for yr in self.handyvars.aeo_years
                            }
                    else:
                        ...
                        if not isinstance(cost_meas, dict):
                            cost_meas = {yr: cost_meas for yr in self.handyvars.aeo_years}
                    # If incentives data were found, apply them to
                    # baseline and measures tech. costs on the basis of their
                    # performance levels
                    if cost_incentives or any(
                        [len(x) != 0 for x in [incent_mod_base, incent_mod_swtch]]
                    ):
                        ...
                        for ind_u, i_unit in enumerate([i_units_base, i_units_meas]):
                            ...
                            # conversion
                                ...
                                and i_unit in self.handyvars.tech_units_map.keys()
                            ):
                                # Find units conversion factor
                                i_unit_cnv[ind_u] = self.handyvars.tech_units_map[i_unit][
                                    i_unit_chk[ind_u]
                                ]
                        ...
                        if all([x is not None for x in [cnv_b, cnv_m]]):
                            ...
                            if bldg_sect == "residential" and mskeys[-1] == "existing":
                                # Baseline mseg incentives multiplier
                                if mskeys[-2] is not None and "HP" in mskeys[-2]:
                                    if mskeys[4] == "heating":
                                        mlt_b = 2
                                    else:
                                        mlt_b = 0
                                ...
                                if (
                                    mskeys_swtch
                                    and mskeys_swtch[-2] is not None
                                    and "HP" in mskeys_swtch[-2]
                                ):
                ...
                if (
                    bldg_sect == "residential"
                    and opts.elec_upgrade_costs != "ignore"
                    and "existing" in mskeys
                    and (
                        self.fuel_switch_to == "electricity"
                        or self.tech_switch_to not in [None, "NA", "same"]
                    )
                    and (
                        not self.linked_htcl_tover
                        or (
                            self.linked_htcl_tover and mskeys[4] == self.linked_htcl_tover_anchor_eu
                        )
                    )
                ):
                    cost_meas, elec_infr_flag = self.add_elec_infr_costs(
                        cost_meas, mskeys, elec_infr_flag, opts
                    )
                ...
                if bldg_sect == "residential":
                    ...
                    for yr in self.handyvars.aeo_years:
                        ...
                        new_constr["annual new"][yr] = mseg_sqft_stock["new homes"][yr]
                        new_constr["total"][yr] = mseg_sqft_stock["total homes"][yr]
                    ...
                    # set choice coefficients to a default value for these msegs
                        ...
                        isinstance(base_cpl, dict) and "consumer choice" not in base_cpl.keys()
                    ):
                    ...
                    # Primary mseg with choice coefficient data
                        ...
                        choice_dict = base_cpl["consumer choice"]["competed market share"][
                            "parameters"
                        ]
                        ...
                        if all([x in choice_dict.keys() for x in ["typical", "best"]]) and all(
                            ...
                            raise ValueError(
                                "ECM '" + self.name + "' missing valid consumer "
                            ...
                            and (
                                any(
                                    [
                                        any(
                                            [
                                                not isinstance(x[1], float)
                                                for x in choice_dict["typical"][coef].items()
                                            ]
                                        )
                                        for coef in ["b1", "b2"]
                                    ]
                                ...
                                and any(
                                    [
                                        any(
                                            [
                                                not isinstance(x[1], float)
                                                for x in choice_dict[coef].items()
                                            ]
                                        )
                                        for coef in ["b1", "b2"]
                                    ]
                                ...
                                for yr in self.handyvars.aeo_years:
                                    ...
                                    if (
                                        ...
                                        and (
                                            perf_base_units
                                            not in self.handyvars.inverted_relperf_list
                                            and perf_meas_yr >= perf_base_best[yr]
                                        )
                                        or (
                                            perf_base_units in self.handyvars.inverted_relperf_list
                                            and perf_meas_yr <= perf_base_best[yr]
                                        )
                                    ) or ("relative savings" in perf_units and perf_meas_yr > 0.1):
                                        choice_key[yr] = "best"
                            # Finalize coefficients
                            choice_params = {
                                "b1": {
                                    yr: base_cpl["consumer choice"]["competed market share"][
                                        "parameters"
                                    ][choice_key[yr]][coef_names[0]][yr]
                                    for yr in self.handyvars.aeo_years
                                },
                                "b2": {
                                    yr: base_cpl["consumer choice"]["competed market share"][
                                        "parameters"
                                    ][choice_key[yr]][coef_names[1]][yr]
                                    for yr in self.handyvars.aeo_years
                                },
                            }
                ...
                if mskeys[2] != "unspecified":
                    ...
                    new_constr["new fraction"] = {yr: 0 for yr in self.handyvars.aeo_years}
                ...
                if mskeys[-1] == "new":
                    new_existing_frac = {
                        key: val for key, val in new_constr["new fraction"].items()
                    }
                else:
                    new_existing_frac = {
                ...
                if (
                    self.handyvars.panel_shares is not None
                    and all([y in mskeys for y in ["single family home", "existing"]])
                    and any([y in mskeys for y in ["heating", "secondary heating", "cooling"]])
                ):
                    ...
                    if mskeys[1] in self.handyvars.panel_shares.keys():
                        ...
                        reg_shr = [
                            x[0]
                            for x in self.handyvars.region_cpl_mapping.items()
                            if mskeys[1] in x[1]
                        ][0]
                    ...
                    if any([x in mskeys for x in ["electricity", "natural gas", "distillate"]]):
                        ...
                        if (
                        ...
                        ) and self.linked_htcl_tover_anchor_tech:
                            # Segment is linked to gas heating tech.; pull gas fuel
                            if any(
                                [
                                    x in self.linked_htcl_tover_anchor_tech
                                    for x in ["NG", "gas", "natural gas"]
                                ]
                            ):
                            ...
                            # pull first non-electric fuel in measure definition
                                ...
                                fuel_shr = [
                                    x for x in self.fuel_type["primary"] if x != "electricity"
                                ][0]
                    ...
                    if "no panel" in mskeys[-2]:  # Sub-segment for homes not requiring upgrade
                        ...
                        adj_stk, adj_energy = [
                            self.handyvars.panel_shares[reg_shr][x][fuel_shr]["BAU w/ HPWH"][
                                "no panel"
                            ]
                            for x in ["stock", "energy"]
                        ]
                ...
                if (
                    mskeys[4] == "lighting"
                    and (not opts or opts.no_scnd_lgt is not True)
                    and self.end_use["secondary"] is not None
                ):
                ...
                if mskeys[0] == "secondary":
                    ...
                    if sf_to_house_key and sf_to_house_key in self.handyvars.sf_to_house.keys():
                        add_stock = {
                            key: val
                            ...
                            * 1000000
                            for key, val in mseg_sqft_stock["total square footage"].items()
                    ...
                    add_stock = {
                        ...
                        for key, val in mseg["stock"].items()
                        if key in self.handyvars.aeo_years
                    }
                else:  # If no stock data exist, set stock to zero
                    add_stock = {key: 0 for key in self.handyvars.aeo_years}
                ...
                if bldg_sect == "residential" and (mskeys[-2] is not None and "HP" in mskeys[-2]):
                    add_stock = {yr: add_stock[yr] * 2 for yr in self.handyvars.aeo_years}
                ...
                if energy_total_scnd is True:
                    energy_total_scnd = self.find_scnd_overlp(
                        new_existing_frac,
                        site_source_conv_base,
                        reduce(operator.getitem, mskeys[1:5], msegs),
                        energy_tot=dict.fromkeys(self.handyvars.aeo_years, 0),
                    )
                # Total carbon emissions
                add_carb = {
                    key: val * intensity_carb_base[key]
                ...
                if (
                    opts.fugitive_emissions is not False
                    and opts.fugitive_emissions[0] in ["1", "3"]
                    and (opts.alt_regions != "State" and mskeys[3] == "natural gas")
                ):
                    ...
                    try:
                        reg_weight_row = [
                            r_i
                            for r_i, r in enumerate(self.handyvars.fugitive_emissions_map)
                            if r[0] == mskeys[1]
                        ][0]
                    except IndexError:
                        raise ValueError(
                            "No fugitive emissions state-level \
                            mapping data found for region '"
                            + mskeys[1]
                            + "'"
                        )
                    reg_weight = self.handyvars.fugitive_emissions_map[reg_weight_row]
                    # Apply mapping fractions to develop methane leakage rate
                    try:
                        lkg_rate = sum(
                            [
                                self.handyvars.fug_emissions["methane"]["total_leakage_rate"][
                                    lkg_rate_scenario
                                ][state]
                                * reg_weight[state]
                                for state in self.handyvars.fug_emissions["methane"][
                                    "total_leakage_rate"
                    ...
                    if self.fuel_switch_to is not None:
                        ...
                        lkg_fmeth_base, lkg_fmeth_meas = (copy.deepcopy(lkg_rate) for n in range(2))
                # State region setting requires no further mapping
                elif (
                    opts.fugitive_emissions is not False
                    and opts.fugitive_emissions[0] in ["1", "3"]
                    and mskeys[3] == "natural gas"
                ):
                    # Directly pull methane leakage rate for current state
                    lkg_rate = self.handyvars.fug_emissions["methane"]["total_leakage_rate"][
                        lkg_rate_scenario
                    ][mskeys[1]]
                    # Handle case where measure is switching away from
                    # a baseline case with methane leakage to a non-gas tech.
                    ...
                    lkg_rate, lkg_fmeth_base, lkg_fmeth_meas = (None for n in range(3))
                ...
                if opts.fugitive_emissions is not False and opts.fugitive_emissions[0] in [
                    ...
                    if mskeys[4] in ["heating", "cooling", "secondary heating", "water heating"]:
                        # Flag for HP measure (requires special handling)
                        hp_flag = self.hp_convert_flag or any(
                            [x in self.name for x in ["HP", "heat pump", "Heat Pump"]]
                        )

                        # Measure is HP for heating/cooling
                        if hp_flag and mskeys[4] in ["heating", "cooling", "secondary heating"]:
                            # Set measure refrigerant data to that of
                            # a GSHP or ASHP based on measure name
                            if any([x in self.name for x in ["GSHP", "Ground", "ground"]]):
                                if bldg_sect == "residential":
                            ...
                            if (
                                ...
                                if mskeys[-2] is not None and "HP" in mskeys[-2]:
                                    if mskeys[4] == "cooling":
                                        ...
                                        zero_b_r_flag, zero_m_r_flag = (True for n in range(2))
                        ...
                        # its name)
                            ...
                            tech_name_chk_b, tech_name_chk_e = (mskeys[-2] for n in range(2))
                            # Given like-for-like HP replacement, anchor
                            # baseline and measure refrigerant emissions
                            # on the cooling end use and set flag to zero
                            # out baseline and measure refrigerant
                    ...
                    # for commercial
                        ...
                        # data in the refigerants input file
                            ...
                            if mskeys[-2] == "Commercial Supermarket Display Cases":
                                tech_name_chk_b, tech_name_chk_e = (
                                    "Centralized refrigeration" for n in range(2)
                                )
                            elif mskeys[-2] in [
                                ...
                                raise ValueError(
                                    ...
                                    "technology encountered for segment " + mskeys
                                )
                        zero_b_r_flag, zero_m_r_flag = (False for n in range(2))
                    else:
                        tech_name_chk_b, tech_name_chk_e = (mskeys[-2] for n in range(2))
                    ...
                    try:
                        f_refr_b = self.handyvars.fug_emissions["refrigerants"][
                    ...
                    try:
                        f_refr_e = self.handyvars.fug_emissions["refrigerants"][
                            "refrigerant_data_by_tech_type"
                ...
                if any([x is not None for x in [f_refr_b, f_refr_e]]):
                    f_refr = {
                        "baseline": [f_refr_b, zero_b_r_flag],
                ...
                if (
                    ...
                    add_fmeth = {
                        ...
                        * mmbtu_to_mcf
                        * lkg_rate
                        * methane_gram_per_mcf
                        * methane_100yr_GWP
                        * mmt_conv
                        for key, val in add_energy.items()
                    }
                elif opts.fugitive_emissions is not False and opts.fugitive_emissions[0] in [
                    "1",
                    "3",
                ]:
                ...
                if (
                    opts.tsv_metrics is not False
                    or self.tsv_features is not None
                    or (calc_sect_shapes is True)
                ) and (
                    mskeys[0] == "primary"
                    and ((mskeys[3] == "electricity") or (self.fuel_switch_to == "electricity"))
                ):
                    ...
                    if (
                        opts.grid_decarb is not False
                        and tsv_data_nonfs is not None
                        and (
                            self.fuel_switch_to is None
                            or (self.fuel_switch_to == "electricity" and "electricity" in mskeys)
                        )
                    ...
                    tsv_scale_fracs, tsv_shapes = self.gen_tsv_facts(
                        tsv_data, mskeys, bldg_sect, convert_data, opts, cost_energy_meas
                    )
                else:
                    tsv_scale_fracs = {
                ...
                if self.fuel_switch_to == "electricity" and "electricity" not in mskeys:
                ...
                for adopt_scheme in self.handyvars.adopt_schemes_prep:
                    ...
                    [
                        ...
                        add_fmeth_compete,
                        add_frefr_compete,
                        add_stock_compete_meas,
                        add_energy_compete_eff,
                        add_carb_compete_eff,
                        add_fmeth_compete_eff,
                        add_frefr_compete_eff,
                        add_stock_cost,
                        add_energy_cost,
                        add_carb_cost,
                        add_stock_cost_meas,
                        add_energy_cost_eff,
                        add_carb_cost_eff,
                        add_stock_cost_compete,
                        add_energy_cost_compete,
                        add_carb_cost_compete,
                        add_stock_cost_compete_meas,
                        add_energy_cost_compete_eff,
                    ...
                    add_dict = {
                        ...
                        "carbon": {
                            "total": {"baseline": add_carb_total, "efficient": add_carb_total_eff},
                            "competed": {
                    ...
                    if sqft_subst != 1 and (
                        rmv_lnkd_tech_costs
                        or rmv_scnd_hvac_stkcosts
                        or (
                            len(ms_lists[3]) > 1
                            and (
                                ("heating" in ms_lists[3] and "heating" not in mskeys)
                                or (
                                    "heating" not in ms_lists[3]
                                    and "cooling" in ms_lists[3]
                                    and "cooling" not in mskeys
                                )
                            )
                        )
                    ):
                    ...
                    add_dict["lifetime"] = {
                        "baseline": {
                            yr: life_base[yr] * add_dict["stock"]["total"]["all"][yr]
                            for yr in self.handyvars.aeo_years
                        },
                        "measure": life_meas,
                    }
                    ...
                    if opts.cap_invest:
                        ...
                        brk_stk_costs[0:2] = [
                            add_dict["cost"]["stock"]["total"][x] for x in ["baseline", "efficient"]
                        ]
                    ...
                    if self.handyvars.full_dat_out[adopt_scheme]:
                        ...
                        self.breakout_mseg(
                            mskeys, contrib_mseg_key, adopt_scheme, opts, brk_in_dat, brk_stk_costs
                        )
                    ...
                    # information)
                    if (
                        contrib_mseg_key_str
                        not in self.markets[adopt_scheme]["mseg_adjust"][
                            "contributing mseg keys and values"
                        ].keys()
                    ):
                    ...
                    if self.handyvars.full_dat_out[adopt_scheme] or self.name in ctrb_ms_pkg_prep:
                        ...
                        if all(
                            [
                                round(x[1], 3)
                                == round(mkt_scale_frac_fin[self.handyvars.aeo_years[0]], 3)
                                for x in mkt_scale_frac_fin.items()
                            ]
                        ):
                            mkt_scale_frac_fin = mkt_scale_frac_fin[self.handyvars.aeo_years[0]]

                        # Record the sub-market scaling fraction associated w/
                        # the current contributing microsegment
                        self.markets[adopt_scheme]["mseg_adjust"][
                            "contributing mseg keys and values"
                        ][contrib_mseg_key_str]["sub-market scaling"] = mkt_scale_frac_fin
                    ...
                    # key chains
                    self.markets[adopt_scheme]["master_mseg"] = self.add_keyvals(
                        self.markets[adopt_scheme]["master_mseg"], add_dict
                    )
                    # Add capacity factor information to contributing microsegment data
                    self.markets[adopt_scheme]["mseg_adjust"]["capacity factor"][
                        contrib_mseg_key_str
                    ] = stk_cap_fact
        ...
        if len(suppress_incent) > 0:
            warnings.warn(
                "WARNING: Incentives found for measure '"
                + self.name
            ...
            for adopt_scheme in self.handyvars.adopt_schemes_prep:
                ...
                if self.handyvars.full_dat_out[adopt_scheme] or self.name in ctrb_ms_pkg_prep:
                    for key in contrib_msegs.keys():
                        contrib_msegs[key]["lifetime"]["baseline"] = {
                            yr: (
                                contrib_msegs[key]["lifetime"]["baseline"][yr]
                                / contrib_msegs[key]["stock"]["total"]["all"][yr]
                            )
                            if contrib_msegs[key]["stock"]["total"]["all"][yr] != 0
                            else 10
                            for yr in self.handyvars.aeo_years
                        }
        ...
        if opts is not None and opts.verbose is True:
            ...
            if valid_keys_stk_energy == valid_keys:
                ...
                bstk_msg = (
                    "\n"
                    + " - "
                    + str(round((1 - (valid_keys_stk_energy / valid_keys)) * 100))
            ...
            if valid_keys_cpl == valid_keys_stk_energy:
                ...
                bcpl_msg = (
                    "\n"
                    + " - "
                    + str(round((1 - (valid_keys_cpl / valid_keys_stk_energy)) * 100))
                    + " % of baseline technologies were missing valid"
    ...
    def scrub_panel_info(self, key_in):
        ...
        if key_in is not None and (any([x in key_in for x in self.handyvars.alt_panel_names])):
    ...
    def use_deflt_res_choice(self, mskeys, consume_warn, opts):
        ...
        if mskeys[4] not in consume_warn:
            ...
            fmt.verboseprint(
                opts.verbose,
                "WARNING: ECM '" + self.name + "' missing "
                "valid consumer choice data for segment '"
                + str(mskeys)
    ...
    def fix_lgt_perf_vals(self, perf_in, mskeys):
        ...
        if any(
            [
                (
                    ("lighting" in mskeys and (isinstance(y[1], float) and round(y[1]) in [0, 999]))
                    or y[1] in [0, "NA", 999]
                )
                for y in perf_in.items()
            ]
        ):
        ...
        for seg in seg_loop:
                    ...
                    (
                        ...
                        and (
                            (
                                ...
                                seg in ["federal", "all"]
                                and opts.incentive_restrictions == "no federal electric"
                            )
                            or
                            # Case 2: electric segment, non-fed. $, user suppressed non-fed elec. $
                            (
                                seg in ["non-federal", "all"]
                                and opts.incentive_restrictions == "no sub-federal electric"
                            )
                            or
                            # Case 3: electric segment, all electric $ suppressed
                            opts.incentive_restrictions == "no electric"
                        )
                    )
                    or (
                        "electricity" not in keys
                        and (
                            # Case 4: non-elec. segment, fed. $, user suppressed fed. non-elec. $
                            (
                                seg in ["federal", "all"]
                                and opts.incentive_restrictions == "no federal fossil"
                            )
                            or (
                                # Case 5: non-elec. segment, non-fed. $, user suppressed non-fed. non-elec. $
                                seg in ["non-federal", "all"]
                                and opts.incentive_restrictions == "no sub-federal fossil"
                            )
                            # Case 6: non-elec. segment, all non-elec. $ suppressed
                            or opts.incentive_restrictions == "no fossil"
                        )
                    )
                )
            ...
            for yr_str in self.handyvars.aeo_years:
                ...
                if cost_incentives:
                    ...
                    if perf_base_units not in self.handyvars.inverted_relperf_list:
                        ...
                        if not cost_incentives_meas:
                            ...
                            base_val_yr = [
                                x[1] * mlt_b * cnv_b
                                for x in cost_incentives_yr
                                if perf_base[yr_str] >= x[0]
                            ]
                            meas_val_yr = [
                                x[1] * mlt_m * cnv_m
                                for x in cost_incentives_meas_yr
                                if perf_meas_yr >= x[0]
                            ]
                    else:
                    ...
                    base_val_yr_max_init, meas_val_yr_max_init = [
                        max(x) if (suppress is False and len(x) >= 1) else False
                        for suppress, x in zip(
                            [suppress_base_incentive, suppress_measure_incentive],
                            [base_val_yr, meas_val_yr],
                        )
                    ]
                ...
                rmv_base, ext_base, rpl_base = [
                    [
                        x
                        for x in incent_mod_base
                        if (
                            y in x[8]
                            and (seg == "all" or x[9] == seg)
                            and ((not x[-3] or x[-3] <= yr_int) and (not x[-2] or x[-2] >= yr_int))
                        )
                    ]
                    for y in ["remove", "extend", "replace"]
                ]
                rmv_swtch, ext_swtch, rpl_swtch = [
                    [
                        x
                        for x in incent_mod_swtch
                        if (
                            y in x[8]
                            and (seg == "all" or x[9] == seg)
                            and ((not x[-3] or x[-3] <= yr_int) and (not x[-2] or x[-2] >= yr_int))
                        ...
                        if len(rpl) != 0:
                            ...
                            for r in rpl:
                                ...
                                if (
                                    ...
                                    and rebate_units in cost_base_units
                                ):
                                    rebate_lev = r[-5]
                                # Rebate presence and units do not match base/measure cost units
                                elif r[-5] != 0 and (
                                    not isinstance(rebate_units, str)
                                    or rebate_units not in cost_base_units
                                ):
                                    raise ValueError(
                                        "Rebate units "
                                        + rebate_units
                                        + " for incentives row "
                                        + str(r)
                                        + " are inconsistent with mseg units "
                                        + cost_base_units
                                    )
                                # Rebate not present
                                else:
                                ...
                                if not suppress_ira and isinstance(perf_thres_val, str):
                                    ...
                                    # and some HP incentives are broken out by cold/warm climate)
                                        ...
                                        if "climates" in perf_thres_val:
                                            ...
                                            warm_reg = (
                                                mskeys[1]
                                                in self.handyvars.warm_cold_regs["warm climates"]
                                                or self.backup_fuel_fraction is not None
                                            )
                                ...
                                # If units don't match, append incentive value of zero and continue
                                    ...
                                    fmt.verboseprint(
                                        ...
                                        "Incentive units of "
                                        + perf_thres_units
                                        + " do not match required units of "
                                        + units
                                        + " for measure "
                                        + self.name
                                        + " and microsegment "
                        ...
                        # provided that existing incentives are available to extend
                            ...
                            if val_adjs_i == 0:
                                ...
                                if len(base_val_yrs_max) != 0:
                                    ...
                                    base_val_yr_max = (
                                        max_to_extend + max_to_extend * ext_incr_pct
                                    ) * appl_frac
                                else:
    ...
    def gen_tsv_facts(self, tsv_data, mskeys, bldg_sect, cost_conv, opts, cost_energy_meas):
        ...
        try:
            ...
            if mskeys[4] == "other":
                ...
                if (
                    self.handyvars.tsv_hourly_lafs is not None
                    and eu
                    not in self.handyvars.tsv_hourly_lafs[mskeys[1]][bldg_sect][mskeys[2]].keys()
                ):
        ...
        if (
            self.handyvars.tsv_hourly_lafs is not None
            and eu not in self.handyvars.tsv_hourly_lafs[mskeys[1]][bldg_sect][mskeys[2]].keys()
        ):
            ...
            if opts.alt_regions == "EMM":
                ash_czone_wts = [
                    [x["ASHRAE"], x[mskeys[1]]]
                    for x in self.handyvars.ash_emm_map
                    if (numpy.isfinite(x[mskeys[1]]) and x[mskeys[1]] != 0)
                ]
            ...
            if bldg_sect == "commercial":
                eplus_bldg_wts = cost_conv["building type conversions"]["conversion data"]["value"][
                    "commercial"
                ][mskeys[2]]
            ...
            if round(sum([x[1] for x in eplus_bldg_wts.items()]), 2) != 1:
                raise ValueError(
                    ...
                    "region " + mskeys[1] + " and building type " + mskeys[2] + " do not sum to 1"
                )
            ...
            if opts.sect_shapes is True and opts.tsv_metrics is False and self.tsv_features is None:
                cost_fact_hourly, carbon_fact_hourly, cost_yr_map, carb_yr_map = (
                    None for n in range(4)
                )
            else:
                ...
                if self.handyvars.tsv_hourly_price[mskeys[1]] is None:
                    ...
                    for yr in tsv_data["price"]["electricity price shapes"].keys():
                        ...
                        if yr in self.handyvars.aeo_years:
                            cost_fact_hourly[yr], self.handyvars.tsv_hourly_price[mskeys[1]][yr] = (
                                [
                                    (
                                        (0.59 + 0.41 * x)
                                        if (cost_energy_meas[yr] * (0.59 + 0.41 * x) >= 0)
                                        else 0
                                    )
                                    for x in tsv_data["price"]["electricity price shapes"][yr][
                                        mskeys[1]
                                    ]
                                ]
                                for n in range(2)
                ...
                if self.handyvars.tsv_hourly_emissions[mskeys[1]] is None:
                    carbon_fact_hourly, self.handyvars.tsv_hourly_emissions[mskeys[1]] = (
                        {
                            yr: tsv_data["emissions"]["average carbon emissions rates"][yr][
                                mskeys[1]
                            ]
                            for yr in tsv_data["emissions"]["average carbon emissions rates"].keys()
                        }
                        for n in range(2)
                    )
                else:
            ...
            self.handyvars.tsv_hourly_lafs[mskeys[1]][bldg_sect][mskeys[2]][eu] = {
                "annual adjustment fractions": copy.deepcopy(updated_tsv_fracs),
    ...
    def apply_tsv(
        ...
        if opts.sect_shapes is True:
            energy_base_shape, energy_eff_shape = ([0 for x in range(8760)] for n in range(2))
        ...
        if opts.tsv_metrics is not False or self.tsv_features is not None:
            ...
            # AEO years)
            cost_scale_base_aeo, cost_scale_eff_aeo, carb_scale_base_aeo, carb_scale_eff_aeo = (
                {yr: 0 for yr in self.handyvars.aeo_years} for n in range(4)
            )
        ...
        if self.tsv_features is not None:
            if type(self.tsv_features) is dict and all(
                [x not in self.handyvars.tsv_feature_types for x in self.tsv_features.keys()]
            ):
                # By EMM region, building type, and end use
                try:
                    tsv_adjustments = self.tsv_features[mskeys[1]][mskeys[2]][mskeys[4]]
                # By EMM region and building type
                    ...
                    # By EMM region and end use
                        ...
                        try:
                            tsv_adjustments = self.tsv_features[mskeys[1]][mskeys[4]]
                        # By building type and end use
                        except KeyError:
                            try:
                                tsv_adjustments = self.tsv_features[mskeys[2]][mskeys[4]]
                            # By EMM region
                            except KeyError:
                                try:
                                ...
                                # By building type
                                    ...
                                    # By end use
                                        ...
                                        try:
                                            tsv_adjustments = self.tsv_features[mskeys[4]]
                                        # Not broken out by any of these
                                        except KeyError:
                                            raise KeyError(
                                                "Unexpected breakout of "
                                                "tsv_features parameter for"
                                                "measure " + self.name + ". If this parameter is "
        ...
        for bldg in eplus_bldg_wts.keys():
            ...
            try:
                load_fact_bldg_key = [
                    x
                    for x in load_fact.keys()
                    if (
                        bldg in load_fact[x]["represented building types"]
                        or load_fact[x]["represented building types"] == "all"
                    )
                ][0]
            except IndexError:
                ...
                if bldg in ["RetailStandalone", "MediumOfficeDetailed", "LargeOfficeDetailed"]:
                    # Account for possible use of StandAlone naming in
                ...
                load_fact_bldg_key = [
                    ...
                    for x in load_fact.keys()
                    if (
                        bldg_adj in load_fact[x]["represented building types"]
                        or load_fact[x]["represented building types"] == "all"
                    )
                ][0]
            ...
            if ash_cz_wts and all(
                ...
                # renormalize weights
                ash_cz_wts = [
                    [x[0], x[1]] for x in ash_cz_wts if x[0] in self.handyvars.tsv_climate_regions
                ]
                # Ensure that ASHRAE climate zone weightings sum to 1
                ash_cz_renorm = sum([x[1] for x in ash_cz_wts])
                if round(ash_cz_renorm, 2) != 1:
                    ash_cz_wts = [[x[0], (x[1] / ash_cz_renorm)] for x in ash_cz_wts]
            else:
                ash_cz_wts = [[None, 1]]
            ...
            for cz in ash_cz_wts:
                ...
                if cz[0] and isinstance(self.handyvars.cz_emm_map[cz[0]], int):
                    ...
                    mult_sysshp_key_save = [
                        y[0]
                        for y in self.handyvars.cz_emm_map[cz[0]].items()
                        if self.handyvars.emm_name_num_map[mskeys[1]] in y[1][1]
                    ][0]
                ...
                # by EMM region
                try:
                    base_load_hourly = load_fact[load_fact_bldg_key]["load shape"][mskeys[1]]
                except (KeyError, TypeError):
                    base_load_hourly = load_fact[load_fact_bldg_key]["load shape"]
                ...
                for a in [x for x in sorted(tsv_adjustments.keys())]:
                    ...
                    try:
                        ...
                        start_stop_dy = [tsv_adjustments[a][x] for x in ["start_day", "stop_day"]]
                        # Generate a list of all days in which the
                        # time-varying efficiency impact applies
                        try:
                            applicable_days = list(range(start_stop_dy[0] - 1, start_stop_dy[1]))
                        # Error sets applicable day range to the full year
                            ...
                            try:
                                ...
                                applicable_days = list(range(start_1 - 1, stop_1)) + list(
                    ...
                    try:
                        ...
                        start_stop_hr = [tsv_adjustments[a][x] for x in ["start_hour", "stop_hour"]]
                        ...
                        try:
                            ...
                            if start_stop_hr[0] <= start_stop_hr[1]:
                                applicable_hrs = list(range(start_stop_hr[0] - 1, start_stop_hr[1]))
                            else:
                                applicable_hrs = list(range(0, start_stop_hr[1])) + list(
                                    range(start_stop_hr[0], 24)
                                )
                    ...
                    enumerate_list = list(enumerate(itertools.product(range(365), range(24))))
                    ...
                    if "shed" in a:
                        ...
                        try:
                            rel_save_tsv = tsv_adjustments[a]["relative energy change fraction"]
                        except KeyError:
                            rel_save_tsv = 0
                        # Reflect the shed impacts on efficient load shape
                        # across all relevant hours of the year
                        eff_load_hourly = [
                            base_load_hourly[i] * (1 - rel_save_tsv)
                            if (x in applicable_days and y in applicable_hrs)
                            else eff_load_hourly[i]
                            for i, (x, y) in enumerate_list
                        ]
                    ...
                    # another time period
                        ...
                        if len(applicable_hrs) == 24:
                            ...
                            eff_load_hourly = [
                                base_load_hourly[i + offset_hrs]
                                if ((i + offset_hrs) <= 8759 and x in applicable_days)
                                else base_load_hourly[(y + offset_hrs) - 24]
                                for i, (x, y) in enumerate_list
                            ]
                        ...
                        # shifting, shift the load in accordance with range
                            ...
                            if new_start <= new_end:
                                ...
                                hrs_to_shift_to = list(range(new_start, 24)) + list(
                                    range(0, new_end + 1)
                                )
                            ...
                            eff_load_hourly = [
                                (
                                    base_load_hourly[i]
                                    + (base_load_hourly[i + offset_hrs] * rel_save_tsv)
                                )
                                if (
                                    ((i + offset_hrs) <= 8759 and x in applicable_days)
                                    and y in hrs_to_shift_to
                                    and y not in applicable_hrs
                                )
                                else (
                                    base_load_hourly[i] * (1 - rel_save_tsv)
                                    + (base_load_hourly[i + offset_hrs] * rel_save_tsv)
                                ...
                                if (
                                    ((i + offset_hrs) <= 8759 and x in applicable_days)
                                    and y in hrs_to_shift_to
                                    and y in applicable_hrs
                                )
                                else (
                                    base_load_hourly[i]
                                    + base_load_hourly[(y + offset_hrs) - 24] * rel_save_tsv
                                )
                                if (
                                    y in hrs_to_shift_to
                                    and y not in applicable_hrs
                                    and x in applicable_days
                                )
                                else (
                                    base_load_hourly[i] * (1 - rel_save_tsv)
                                    + base_load_hourly[(y + offset_hrs) - 24] * rel_save_tsv
                                ...
                                else eff_load_hourly[i] * (1 - rel_save_tsv)
                                if (y in applicable_hrs and x in applicable_days)
                                else eff_load_hourly[i]
                                for i, (x, y) in enumerate_list
                            ]
                    ...
                    # day or across all 8760 hours of the year
                        ...
                        if (
                            "custom_daily_savings" in tsv_adjustments[a].keys()
                            and tsv_adjustments[a]["custom_daily_savings"] is not None
                        ):
                            # Set the custom daily savings shape (each element
                            # of the list represents hourly savings fraction)
                            custom_save_shape = tsv_adjustments[a]["custom_daily_savings"]
                            # Reflect custom load savings in efficient load
                            # shape
                            eff_load_hourly = [
                                base_load_hourly[i] * (1 - custom_save_shape[y])
                                if (x in applicable_days)
                                else eff_load_hourly[i]
                                for i, (x, y) in enumerate_list
                            ]
                        ...
                        # savings fractions for all 8760 hours of the year
                            ...
                            "custom_annual_savings" in tsv_adjustments[a].keys()
                            and tsv_adjustments[a]["custom_annual_savings"] is not None
                        ):
                            ...
                            try:
                                custom_hr_save_shape = tsv_adjustments[a]["custom_annual_savings"][
                                    eu
                                ][load_fact_bldg_key][load_fact_climate_key][mult_sysshp_key_save]
                            except KeyError:
                                ...
                                try:
                                    ...
                                    ][eu][load_fact_bldg_key][load_fact_climate_key]["set 1"]
                                except KeyError:
                            ...
                            # the baseline load.
                            if (
                                len(custom_hr_save_shape) == 0
                                or sum(custom_hr_save_shape["CSV base frac. annual"]) == 0
                            ):
                                ...
                                meas_base_adj = [
                                    (
                                        custom_hr_save_shape["CSV base frac. annual"][x]
                                        / base_load_hourly[x]
                                    )
                                    if (
                                        numpy.isfinite(base_load_hourly[x])
                                        and base_load_hourly[x] != 0
                                    )
                                    else 1
                                    for x in range(8760)
                                ]
                                ...
                                hr_chg = custom_hr_save_shape["CSV relative change"]
                                # Apply hourly baseline adjustment and relative
                                # load change to derive efficient shape
                                eff_load_hourly = [
                                    (base_load_hourly[x] * meas_base_adj[x] * hr_chg[x])
                                    for x in range(8760)
                                ]
                                # Ensure all efficient load fractions are
                                # greater than zero
                                eff_load_hourly = [
                                    eff_load_hourly[x] if eff_load_hourly[x] >= 0 else 0
                                    for x in range(8760)
                                ]
                        else:
                ...
                if opts.tsv_metrics is not False:
                    ...
                    if calc == "sum" or calc == "avg":
                        tsv_metrics_days = self.handyvars.tsv_metrics_data["season days"][days][
                            season
                        ]
                    # Maximum calc type (pertains only to a peak day)
                    else:
                        tsv_metrics_days = [self.handyvars.tsv_metrics_data["peak days"][season]]
                    ...
                    if calc in ["sum", "avg"]:
                        ...
                        # Peak hours only
                            ...
                            tsv_metrics_hrs = self.handyvars.tsv_metrics_data["system load hours"][
                                season
                            ]["2050"][tsvmets_reg]["peak range"]
                        # Take hours only
                        else:
                    ...
                    # Maximum calc type (pertains only to a max/min hour)
                        ...
                        if hours == "all" or hours == "peak":
                            ...
                            # than multiple sets
                            tsv_metrics_hrs = [
                                self.handyvars.tsv_metrics_data["system load hours"][season][
                                    "2050"
                                ][tsvmets_reg]["max"]
                            ]
                        # Min take hour
                    ...
                    if calc == "avg":
                        ...
                        if output == "power":
                            avg_len = len(tsv_metrics_days) * len(tsv_metrics_hrs)
                    ...
                    base_load_hourly, eff_load_hourly = [
                        [
                            (x[i] / avg_len)
                            if ((d + 1) in tsv_metrics_days and (h + 1) in tsv_metrics_hrs)
                            else 0
                            for i, (d, h) in self.handyvars.tsv_metrics_data["hourly index"]
                        ]
                        for x in [base_load_hourly, eff_load_hourly]
                    ]
                ...
                energy_scale_base += numpy.sum([x * emm_adj_wt for x in base_load_hourly])
                energy_scale_eff += numpy.sum([x * emm_adj_wt for x in eff_load_hourly])
        ...
        if opts.tsv_metrics is not False or self.tsv_features is not None:
            ...
            for yr in cost_scale_base.keys():
                cost_scale_base[yr] += numpy.sum(
                    [x * y for x, y in zip(base_load_hourly, cost_fact_hourly[yr])]
                )
                cost_scale_eff[yr] += numpy.sum(
                    [x * y for x, y in zip(eff_load_hourly, cost_fact_hourly[yr])]
                )
            ...
            for yr in carb_scale_base.keys():
                carb_scale_base[yr] += numpy.sum(
                    [x * y for x, y in zip(base_load_hourly, carbon_fact_hourly[yr])]
                )
                carb_scale_eff[yr] += numpy.sum(
                    [x * y for x, y in zip(eff_load_hourly, carbon_fact_hourly[yr])]
                )
            ...
            for yr in self.handyvars.aeo_years:
                ...
                # current AEO year
                tsv_yr_cost = [x[0] for x in cost_yr_map.items() if yr in x[1]][0]
                # Find year in the emissions scaling factors data that maps to
                # the current AEO year
                tsv_yr_carb = [x[0] for x in carb_yr_map.items() if yr in x[1]][0]
                # Finalize the cost/emissions scaling factors data for the
                # current AEO year

                # APPROACH 1: TAKE THE DATA AS IS
                (
                    cost_scale_base_aeo[yr],
                    cost_scale_eff_aeo[yr],
                    carb_scale_base_aeo[yr],
                    carb_scale_eff_aeo[yr],
                ) = [
        ...
        if (
            self.fuel_switch_to == "electricity"
            and "electricity" not in mskeys
            and opts.sect_shapes is True
        ):
        ...
        # if sector-level load shape information is desired by the user
        if opts.sect_shapes is True:
            updated_tsv_shapes = {"baseline": energy_base_shape, "efficient": energy_eff_shape}
        else:
            updated_tsv_shapes = None
        ...
        updated_tsv_fracs = {
            "energy": {"baseline": energy_scale_base, "efficient": energy_scale_eff},
            "cost": {"baseline": cost_scale_base_aeo, "efficient": cost_scale_eff_aeo},
            "carbon": {"baseline": carb_scale_base_aeo, "efficient": carb_scale_eff_aeo},
        }
    ...
    def convert_costs(
        ...
        cost_meas_units_unpack, cost_base_units_unpack = [
            re.search(r"(\d*)(.*)", x) for x in [cost_meas_units, cost_base_units]
        ]
        # Establish measure and baseline cost year
        cost_meas_yr, cost_base_yr = (
            cost_meas_units_unpack.group(1),
            cost_base_units_unpack.group(1),
        )
        ...
        cost_meas_noyr, cost_base_noyr = (
            cost_meas_units_unpack.group(2),
        ...
        res_sf_unit = (
            bldg_sect == "residential"
            and "$/ft^2 floor" in cost_meas_noyr
            and "$/unit" in cost_base_noyr
        )
        ...
        if cost_meas_noyr != cost_base_noyr and res_sf_unit is False:
            ...
            if top_key == "heating and cooling":
                ...
                if htcl_key == "supply":
                    ...
                    supply_dat = convert_data["cost unit conversions"][top_key][htcl_key]
                    ...
                    try:
                        if mskeys[4] == "secondary heating":
                            supply_key = [
                                x
                                for x in supply_dat.keys()
                                if "heating" in x
                                and cost_meas_noyr
                                in [supply_dat[x]["revised units"], supply_dat[x]["original units"]]
                            ][0]
                    ...
                    convert_units_data = [
                        convert_data["cost unit conversions"][top_key][htcl_key][x]
                        for x in demand_keys[demand_key]["conversion stages"]
                    ]
            else:
                # Retrieve conversion data for costs outside of the
                # heating/cooling cases
                convert_units_data = [convert_data["cost unit conversions"][top_key]]
            ...
            for cval in convert_units_data:
                ...
                if isinstance(cval["conversion factor"]["value"], dict):
                    ...
                    cval_bldgtyp = cval["conversion factor"]["value"][bldg_sect]
                    # Case where conversion data is further nested by
                    # Scout building type and EnergyPlus building type
                    if isinstance(cval_bldgtyp, dict) and isinstance(cval_bldgtyp[mskeys[2]], dict):
                        ...
                        if invert_cval is False:
                            ...
                            cval_bldgtyp = [1 / x for x in cval_bldgtyp[mskeys[2]].values()]
                    ...
                    if invert_cval is False:
                        ...
                        convert_units *= 1 / cval["conversion factor"]["value"]
        else:
            convert_units = 1
            new_units = None
        ...
        if new_units and isinstance(convert_units_data, list):
            cost_meas_units_fin = cost_base_yr + convert_units_data[-1][new_units]
        # Cost year/unit adjustments involving a single stage of conversion
        elif new_units:
            cost_meas_units_fin = cost_base_yr + convert_units_data[new_units]
            ...
            if verbose:
                ...
                if not isinstance(cost_meas, numpy.ndarray):
                    ...
                    user_message = (
                        ...
                        + self.name
                        + "' cost converted from "
                        + str(numpy.mean(cost_meas))
                        + " "
                        + cost_meas_units
                        + " to "
                        + str(round(numpy.mean(cost_meas_fin), 3))
                        + " "
                        + cost_meas_units_fin
                    )
                ...
                # components)
                if cost_meas_noyr in self.handyvars.cconv_bybldg_units or isinstance(
        ...
        # Case where cost conversion has not succeeded
            ...
            raise ValueError(
                ...
                + self.name
                + "' cost units '"
                + str(cost_meas_units_fin)
                + "' not equal to base units '"
    ...
    def partition_microsegment(
        ...
        (
            ...
            stock_total_hp_convert,
            energy_total_eff,
            carb_total_eff,
            stock_compete,
            stock_compete_meas,
            stock_compete_sbmkt,
            stock_comp_hp_convert,
            stock_comp_hp_remain,
            energy_compete,
            carb_compete,
            energy_compete_sbmkt,
            carb_compete_sbmkt,
            energy_compete_eff,
            carb_compete_eff,
            stock_total_cost,
            energy_total_cost,
            carb_total_cost,
            stock_total_cost_eff,
            energy_total_eff_cost,
            carb_total_eff_cost,
            stock_compete_cost,
            energy_compete_cost,
            carb_compete_cost,
            stock_compete_cost_eff,
            energy_compete_cost_eff,
            carb_compete_cost_eff,
            mkt_scale_frac_fin,
        ) = ({} for n in range(35))
        ...
        if (
            opts.fugitive_emissions is not False
            and opts.fugitive_emissions[0] in ["1", "3"]
            and any([x[1] != 0 for x in fmeth_total_init.items()])
        ):
        ...
        elif opts.fugitive_emissions is not False and opts.fugitive_emissions[0] in ["1", "3"]:
        ...
        if (
            opts.fugitive_emissions is not False
            and opts.fugitive_emissions[0] in ["2", "3"]
            and f_refr is not None
        ):
            # Initialize fugitive refrigerant dicts for further calculations
            frefr_total, frefr_total_eff, frefr_compete, frefr_compete_eff = ({} for n in range(4))
        ...
        elif opts.fugitive_emissions is not False and opts.fugitive_emissions[0] in ["2", "3"]:
            # Set fugitive refrigerants dicts to all zeros
            frefr_total, frefr_total_eff, frefr_compete, frefr_compete_eff = (
                dict.fromkeys(_aeo_years, 0) for n in range(4)
            )
        ...
        (
            fs_stk_eff_remain,
            fs_stk_cost_eff_remain,
            fs_energy_eff_remain_base,
            fs_carb_eff_remain_base,
            fs_energy_cost_eff_remain_base,
            fs_energy_eff_remain_switch,
            fs_carb_eff_remain_switch,
        ...
        if (
            isinstance(self.backup_fuel_fraction, pd.DataFrame)
            and "heating" in mskeys
            and (self.fuel_switch_to is not None and self.fuel_switch_to not in mskeys)
        ):
            remain_fuel_frac = self.backup_fuel_fraction[
                self.backup_fuel_fraction["state"] == mskeys[1]
            ]["remain_frac"].iloc[0]
        else:
            remain_fuel_frac = 0
        ...
        if energy_total_scnd is not False or mskeys[0] == "secondary":
            ...
            if secnd_mseg_adjkey not in secnd_adj_stk["original energy (total)"].keys():
                ...
                # information
                secnd_adj_stk["adjusted energy (competed and captured)"][secnd_mseg_adjkey] = (
                    dict.fromkeys(self.handyvars.aeo_years, 0)
                )
                ...
                secnd_adj_mktshr["adjusted energy (total captured)"][secnd_mseg_adjkey] = (
                    dict.fromkeys(self.handyvars.aeo_years, 0)
                )
            ...
            linked_htcl_tover_anchor_tech, total_hp_converts = (None for n in range(2))
        ...
        # primary mseg are flagged as being linked to those of another mseg
            ...
            (
                ...
                total_htcl_lnk_adj,
                compete_htcl_lnk,
                compete_htcl_lnk_adj,
                cum_capt_htcl_lnk,
                cum_comp_htcl_lnk,
                total_hp_converts,
            ) = [
                self.markets[adopt_scheme]["mseg_adjust"]["paired heat/cool mseg adjustments"][x]
                for x in [
                    "original total stock",
                    "adjusted total stock",
                    "original competed stock",
            ...
            if initiate_linked_dat and linked_htcl_tover_anchor_tech:
                ...
                ) = (dict.fromkeys(self.handyvars.aeo_years, 0) for n in range(6))
            ...
            # unlinked turnover calculations for the mseg
                ...
                if hp_rate is not None and (
                    self.fuel_switch_to == "electricity"
                    or (self.tech_switch_to not in [None, "NA"])
                ):
                    null_val = 0
                else:
                    null_val = 1
                if not initiate_linked_dat:
                    ...
                    if adopt_scheme == "Technical potential" and any(
                        ...
                        lnkd_cost_adj_fact = {
                            yr: (
                                total_htcl_lnk[htcl_lnk_adjkey][yr]
                                / (stock_total_init[yr] * mkt_scale_frac)
                            )
                            if (
                                (stock_total_init[yr] * mkt_scale_frac) != 0
                                and (
                                    ...
                                    and (
                                        ...
                                        and (
                                            self.linked_htcl_tover_linked_tech == "all"
                                            or self.linked_htcl_tover_linked_tech in mskeys[-2]
                                        )
                                    )
                                    ...
                                    and not any(
                                        [
                                            mskeys[-2] is not None and x in mskeys[-2]
                    ...
                    diffuse_frac_linked = {
                        yr: (
                            total_htcl_lnk_adj[htcl_lnk_adjkey][yr]
                            / total_htcl_lnk[htcl_lnk_adjkey][yr]
                        )
                        if total_htcl_lnk[htcl_lnk_adjkey][yr] != 0
                        else null_val
                        for yr in self.handyvars.aeo_years
                    }
                    ...
                    comp_frac_diffuse_linked = {
                        ...
                        / total_htcl_lnk_adj[htcl_lnk_adjkey][yr]
                        if total_htcl_lnk_adj[htcl_lnk_adjkey][yr] != 0
                        else 0
                        for yr in self.handyvars.aeo_years
                    }
                    ...
                    cum_frac_linked = {
                        "original total stock": total_htcl_lnk[htcl_lnk_adjkey],
                        "adjusted total stock": total_htcl_lnk_adj[htcl_lnk_adjkey],
                        "original competed stock": compete_htcl_lnk[htcl_lnk_adjkey],
                        "adjusted competed stock": compete_htcl_lnk_adj[htcl_lnk_adjkey],
                        "cumulative captured stock": cum_capt_htcl_lnk[htcl_lnk_adjkey],
                        "cumulative competed stock": cum_comp_htcl_lnk[htcl_lnk_adjkey],
                    }
                else:
                    ...
                    diffuse_frac_linked = {yr: null_val for yr in _aeo_years}
            ...
            if (
                self.hp_convert_flag
                and "heating" in mskeys
                and htcl_lnk_adjkey not in total_hp_converts.keys()
            ):
            ...
            # number of additional cooling units to represent in below calcs.
                ...
                self.hp_convert_flag
                and ("cooling" in mskeys and mkt_scale_frac == 0)
                and (self.add_cool_anchor_tech == mskeys[-2])
            ):
                ...
                if htcl_lnk_adjkey in total_hp_converts.keys():
                    new_cool_units = {
                        comp_tot_key: {
                            yr: total_hp_converts[htcl_lnk_adjkey][comp_tot_key][yr]
                            for yr in self.handyvars.aeo_years
                        }
                        for comp_tot_key in ["total", "competed"]
                    }
                else:
                    raise ValueError(
        ...
        # building type.
        if (
            (bldg_sect == "commercial" and mskeys[2] != "unspecified")
            and sqft_subst != 1
            and (mskeys[4] not in self.handyvars.com_eqp_eus_nostk)
        ):
        ...
        try:
            ...
            if "fraction_" in list(self.diffusion.keys())[0]:
                try:
                    ...
                    if (df["diff"] > 1).any():
                        warn_list.append(
                            "WARNING: Some declared diffusion fractions are greater"
                    ...
                    if (df["diff"] < 0).any():
                        warn_list.append(
                            "WARNING: Some declared diffusion fractions are smaller"
                    ...
                    if df["diff"].isna().values.any():
                        warn_list.append(
                            "WARNING: Not enough data were provided for first and"
                    ...
                    df = df[
                        ...
                        & (df.index.year < (int(self.handyvars.aeo_years[-1]) + 1))
                    ]
                    fractions = df["diff"].to_list()
                    # for year in range_years:
                    for i in range(0, len(self.handyvars.aeo_years)):
                        years_diff_fraction_dictionary[str(self.handyvars.aeo_years[i])] = (
                            fractions[i]
                        )
        ...
        if self.handyvars.low_volume_rate is not None and len(self.handyvars.low_volume_rate) != 0:
            ...
            alt_rates_init = [
                seg
                for seg in self.handyvars.low_volume_rate
                if (
                    (seg[0:3] == [mskeys[1], mskeys[2], mskeys[-1]])  # reg/bldg/vint
                    and (seg[5] == fuel_for_rate)  # fuel (switched to, if applicable)
                    and (seg[3] == "all" or seg[3] == mskeys[4])  # end use
                    and (seg[4] == "all" or seg[4] == tech_for_rate)
                )
            ]  # tech (switched to, if appl.)
        else:
            alt_rates_init = []
        ...
        for yr in _aeo_years:
            ...
            if int(yr) >= self.market_entry_year and int(yr) < self.market_exit_year:
                measure_on_mkt = True
                measure_exited_mkt = ""
            ...
            try:
                tsv_ecost_base, tsv_carb_base = [
                    tsv_adj_init[x]["baseline"][yr] for x in ["cost", "carbon"]
                ]
            except TypeError:
                tsv_ecost_base, tsv_carb_base = [
                    tsv_adj_init[x]["baseline"] for x in ["cost", "carbon"]
                ]
            ...
            if measure_on_mkt:
                ...
                try:
                    tsv_ecost_eff, tsv_carb_eff = [
                        tsv_adj_init[x]["efficient"][yr] for x in ["cost", "carbon"]
                    ]
                except TypeError:
                    tsv_ecost_eff, tsv_carb_eff = [
                        tsv_adj_init[x]["efficient"] for x in ["cost", "carbon"]
                    ]
            else:
            ...
            if mskeys[0] == "primary" and not comp_frac_diffuse_linked:
                ...
                if mskeys[-1] == "new":
                    ...
                    if yr != self.handyvars.aeo_years[0] and stock_total_sbmkt[yr] != 0:
                        # Handle case where data for previous year are
                        ...
                        try:
                            ...
                            if stock_total_sbmkt[yr] >= stock_total_sbmkt[str(int(yr) - 1)]:
                                new_frac = (
                                    stock_total_sbmkt[yr] - stock_total_sbmkt[str(int(yr) - 1)]
                    ...
                    if prev_capt_turnover:
                        ...
                        if prev_capt_yr >= int(self.handyvars.aeo_years[0]):
                            ...
                            if (
                                str(prev_capt_yr) != self.handyvars.aeo_years[0]
                            ) and stock_total_sbmkt[yr] != 0:
                                ...
                                if (
                                    ...
                                    repl_frac = (
                                        stock_total_sbmkt[str(prev_capt_yr)] / stock_total_sbmkt[yr]
                                    )
                                retro_frac = 0
            ...
            elif mskeys[0] == "primary" and adopt_scheme == "Technical potential":
                comp_frac_sbmkt = 1
            ...
            if (
                not diffuse_frac_linked
                and hp_rate
                and mskeys[0] == "primary"
                and yr in hp_rate.keys()
            ):
                ...
                # Otherwise, stock is converted according to external rate
                    ...
                    if stock_total_hp_convert_frac != 1:
                        # Frac. total stock that converted to HP in year
                        annual_hp_convert_frac = (new_frac + repl_frac) * hp_rate_yr + retro_convert
                        # Frac. total stock that remains with base fuel in year
                        annual_hp_remain_frac = (new_frac + repl_frac) * (
                            1 - hp_rate_yr
                        ) + retro_remain
                    else:
                        annual_hp_convert_frac = new_frac + repl_frac + retro_convert
                        annual_hp_remain_frac = 0

                # Find the annual stock that is converted to the HP measure
                stock_comp_hp_convert[yr] = annual_hp_convert_frac * stock_total_sbmkt[yr]
                # Find the annual stock competed but remains /w base fuel
                stock_comp_hp_remain[yr] = annual_hp_remain_frac * stock_total_sbmkt[yr]
                ...
                # If stock is not already all captured, add competed stock
                    ...
                    stock_total_hp_convert[yr] = (
                        stock_total_hp_convert[str(int(yr) - 1)] + stock_comp_hp_convert[yr]
                    )
                ...
                if stock_total_sbmkt[yr] != 0:
                    stock_total_hp_convert_frac = stock_total_hp_convert[yr] / stock_total_sbmkt[yr]
                else:
                    stock_total_hp_convert_frac = 0
                ...
                if self.fuel_switch_to == "electricity" or (
                    ...
                    if stock_total_hp_convert[yr] != 0:
                        comp_frac_diffuse = stock_comp_hp_convert[yr] / stock_total_hp_convert[yr]
                    else:
                        comp_frac_diffuse = 0
                ...
                # or baseline resistance heating tech.)
                    ...
                    if (stock_total_sbmkt[yr] - stock_total_hp_convert[yr]) != 0:
                        comp_frac_diffuse = stock_comp_hp_remain[yr] / (
                            stock_total_sbmkt[yr] - stock_total_hp_convert[yr]
                        )
                    else:
                        comp_frac_diffuse = 0
            ...
            # rates of another mseg that have already been determined
                ...
                and hp_rate
                and mskeys[0] == "primary"
                and all([x in hp_rate.keys() for x in ["total", "competed"]])
            ):
            ...
            if f_refr_assess:
                ...
                if f_refr["baseline"][0] is not None:
                    ...
                    if yr == self.handyvars.aeo_years[0]:
                        ...
                        if isinstance(f_refr["baseline"][0]["typ_refrigerant"], dict):
                            r_key_b_init = f_refr["baseline"][0]["typ_refrigerant"][
                                [y for y in f_refr["baseline"][0]["typ_refrigerant"].keys()][0]
                            ]
                        else:
                            r_key_b_init = f_refr["baseline"][0]["typ_refrigerant"]
                        # Use key above to pull installed base refrigerant GWP
                        try:
                            base_gwp_yr_init = self.handyvars.fug_emissions["refrigerants"][
                                "refrigerant_GWP100"
                            ][r_key_b_init]
                        except KeyError:
                            raise ValueError(
                    ...
                    try:
                        base_gwp_yr = self.handyvars.fug_emissions["refrigerants"][
                            "refrigerant_GWP100"
                        ][r_key_b]
                    except KeyError:
                        raise ValueError(
                ...
                if f_refr["efficient"][0] is not None:
                    ...
                    if opts.fugitive_emissions[1] == "3":
                        ...
                        try:
                            ...
                            if self.low_gwp_refrigerant in [None, "default"] or type(
                                self.low_gwp_refrigerant
                            ) not in [str, dict]:
                                low_gwp_usr = ""
                            else:
                    ...
                    try:
                        meas_gwp_yr = self.handyvars.fug_emissions["refrigerants"][
                            "refrigerant_GWP100"
                        ][r_key_e]
                    except KeyError:
                        raise ValueError(
                ...
                if base_gwp_yr:
                    ...
                    base_f_r_unit_yr, base_f_r_unit_capt = (0 for n in range(2))
                ...
                base_f_r_unit_yr, base_f_r_unit_overall, base_f_r_unit_capt = ("" for n in range(3))
            ...
            energy_total[yr] = energy_total_sbmkt[yr] * diffuse_frac * tsv_energy_base
            carb_total[yr] = carb_total_sbmkt[yr] * diffuse_frac * tsv_carb_base
            ...
            if f_meth_assess:
                fmeth_total[yr] = fmeth_total_sbmkt[yr] * diffuse_frac * tsv_energy_base
            if f_refr_assess:
            ...
            if (
                calc_sect_shapes is True
                and self.handyvars.full_dat_out[adopt_scheme]
                and yr in self.handyvars.aeo_years_summary
            ) and tsv_shapes is not None:
                self.sector_shapes[adopt_scheme][mskeys[1]][yr]["baseline"] = [
                    self.sector_shapes[adopt_scheme][mskeys[1]][yr]["baseline"][x]
                    + tsv_shapes["baseline"][x] * (energy_total[yr] / tsv_energy_base)
                    for x in range(8760)
                ]
            elif (
                (
            ...
            # any sub-market scaling
            if f_meth_assess:
                fmeth_compete_sbmkt[yr] = fmeth_total_sbmkt[yr] * comp_frac_sbmkt

            # Final competed stock, energy, and carbon markets after accounting
            # for any diffusion/conversion dynamics that restrict a measure's
            # access to it's full baseline market (after sub-mkt scaling), as
            # well as any adjustments to account for time-sensitive effects
            stock_compete[yr] = stock_total_sbmkt[yr] * diffuse_frac * comp_frac_diffuse
            energy_compete[yr] = (
                energy_total_sbmkt[yr] * diffuse_frac * comp_frac_diffuse * tsv_energy_base
            )
            carb_compete[yr] = (
                carb_total_sbmkt[yr] * diffuse_frac * comp_frac_diffuse * tsv_carb_base
            )
            ...
            if f_meth_assess:
                fmeth_compete[yr] = (
                    fmeth_total_sbmkt[yr] * diffuse_frac * comp_frac_diffuse * tsv_energy_base
                )
            if f_refr_assess:
                # Check for flag to force baseline refrig. emissions to zero
                if f_refr["baseline"][1] is True:
            ...
            stock_compete_meas[yr] = stock_total_sbmkt[yr] * diffuse_frac * comp_frac_diffuse_meas
            ...
            if cum_frac_linked and cum_frac_linked != 0:
                stk_tot_sbmkt, stk_tot_diff, stk_cmp_sbmkt, stk_cmp_diff, stk_cum_m, stk_cum_cmp = [
            ...
            if mskeys[0] == "primary" and yr != self.handyvars.aeo_years[0]:
                ...
                if (not isinstance(meas_cum_frac, numpy.ndarray) and meas_cum_frac != 1) or (
                    isinstance(meas_cum_frac, numpy.ndarray) and any(meas_cum_frac != 1)
                ):
                ...
                # if not already 100%
                if (not isinstance(comp_cum_frac, numpy.ndarray) and comp_cum_frac != 1) or (
                    isinstance(comp_cum_frac, numpy.ndarray) and any(comp_cum_frac != 1)
                ):
                    try:
                ...
                if not isinstance(meas_cum_frac, numpy.ndarray) and meas_cum_frac > 1:
                    meas_cum_frac = 1
                elif isinstance(meas_cum_frac, numpy.ndarray) and any(meas_cum_frac > 1):
                    meas_cum_frac[numpy.where(meas_cum_frac > 1)] = 1
                # Cumulative competed-captured fraction
                if not isinstance(comp_cum_frac, numpy.ndarray) and comp_cum_frac > 1:
                    comp_cum_frac = 1
                elif isinstance(comp_cum_frac, numpy.ndarray) and any(comp_cum_frac > 1):
                ...
                if "existing" in mskeys and adopt_scheme != "Technical potential":
            ...
            # Subsequent year in modeling time horizon
                ...
                if adopt_scheme == "Technical potential" and (measure_on_mkt or measure_exited_mkt):
                ...
                # Non-technical potential case
                    ...
                    try:
                        ...
                        stock_total_meas[yr] = (
                            stock_total_meas[str(int(yr) - 1)] + stock_compete_meas[yr]
                        )
                        stock_comp_cum_sbmkt[yr] = (
                            stock_comp_cum_sbmkt[str(int(yr) - 1)] + stock_compete_sbmkt[yr]
                        )
                    except KeyError:
                        stock_total_meas[yr] = stock_compete_meas[yr]
                        stock_comp_cum_sbmkt[yr] = stock_compete_sbmkt[yr]
            ...
            if (
                isinstance(stock_total_meas[yr], numpy.ndarray)
                and any(stock_total_meas[yr] > stock_total[yr]) is True
            ):
                stock_total_meas[yr][numpy.where(stock_total_meas[yr] > stock_total[yr])] = (
                    stock_total[yr]
                )
            elif (
                isinstance(stock_total_meas[yr], numpy.ndarray)
                and any(stock_total_meas[yr] < 0) is True
            ):
                stock_total_meas[yr][numpy.where(stock_total_meas[yr] < 0)] = 0
            # Handle case where stock captured by measure is point val
            elif (
                not isinstance(stock_total_meas[yr], numpy.ndarray)
                and stock_total_meas[yr] > stock_total[yr]
            ):
                stock_total_meas[yr] = stock_total[yr]
            elif not isinstance(stock_total_meas[yr], numpy.ndarray) and stock_total_meas[yr] < 0:
                stock_total_meas[yr] = 0
            ...
            if (
                ...
                compete_htcl_lnk[htcl_lnk_adjkey][yr] += stock_compete_sbmkt[yr]
                compete_htcl_lnk_adj[htcl_lnk_adjkey][yr] += stock_compete[yr]
                cum_capt_htcl_lnk[htcl_lnk_adjkey][yr] += stock_total_meas[yr]
                cum_comp_htcl_lnk[htcl_lnk_adjkey][yr] += stock_comp_cum_sbmkt[yr]
            ...
            if "heating" in mskeys and all(
                [x for x in [self.linked_htcl_tover, self.hp_convert_flag, total_hp_converts]]
            ):
                total_hp_converts[htcl_lnk_adjkey]["competed"][yr] += stock_compete_meas[yr]
                total_hp_converts[htcl_lnk_adjkey]["total"][yr] += stock_total_meas[yr]
            ...
            if int(yr) <= self.market_entry_year:
                ...
                rel_perf_overall, rel_perf_capt = (copy.deepcopy(rel_perf[yr]) for n in range(2))
            ...
            # measure stock across all previous years
                ...
                if (
                    not isinstance(stock_total_meas[yr], numpy.ndarray)
                    and stock_total_meas[yr] != 0
                ) or (
                    isinstance(stock_total_meas[yr], numpy.ndarray)
                    and all(stock_total_meas[yr]) != 0
                ):
                    to_m_stk = stock_compete_meas[yr] / stock_total_meas[yr]
                else:
                    to_m_stk = 0
                # Ensure the measure turnover weight never exceeds 1
                if (not isinstance(to_m_stk, numpy.ndarray)) and to_m_stk > 1:
                    to_m_stk = 1
                elif (isinstance(to_m_stk, numpy.ndarray)) and any(to_m_stk > 1):
                    to_m_stk[numpy.where(to_m_stk > 1)] = 1
                # Update overall measure stock RP
                rel_perf_overall = rel_perf[yr] * to_m_stk + rel_perf_capt * (1 - to_m_stk)
                # Update overall and previously captured measure unit
                # refrigerant emissions, if needed
                if f_refr_assess:
                    rel_frefr_overall = rel_frefr_yr * to_m_stk + rel_frefr_capt * (1 - to_m_stk)
            ...
            energy_tot_uncomp_meas_mkt = (
                energy_total_sbmkt[yr] * diffuse_frac * (1 - comp_frac_diffuse) * meas_cum_frac
            )
            ...
            energy_compete_eff[yr] = energy_tot_comp_meas + energy_tot_comp_base
            # Total-efficient energy
            energy_total_eff[yr] = (
                energy_compete_eff[yr] + energy_tot_uncomp_meas + energy_tot_uncomp_base
            )
            # Total-efficient-captured energy (if not suppressed by user)
            if eff_capt:
                energy_total_eff_capt[yr] = energy_tot_comp_meas + energy_tot_uncomp_meas
            ...
            # adoption scenarios where full data reporting is flagged.)
            if (
                (calc_sect_shapes is True and self.handyvars.full_dat_out[adopt_scheme])
                and yr in self.handyvars.aeo_years_summary
                and tsv_shapes is not None
            ):
                ...
                if self.fuel_switch_to == "electricity" and "electricity" not in mskeys:
                    self.sector_shapes[adopt_scheme][mskeys[1]][yr]["efficient"] = [
                        self.sector_shapes[adopt_scheme][mskeys[1]][yr]["efficient"][x]
                        + ((energy_tot_comp_meas + energy_tot_uncomp_meas) / tsv_energy_eff)
                    ...
                    self.sector_shapes[adopt_scheme][mskeys[1]][yr]["efficient"] = [
                        ...
                        + ((energy_tot_comp_meas + energy_tot_uncomp_meas) / tsv_energy_eff)
                        * tsv_shapes["efficient"][x]
                        + ((energy_tot_comp_base + energy_tot_uncomp_base) / tsv_energy_base)
            ...
            # electricity)
            try:
                intensity_carb_ratio = intensity_carb_meas[yr] / intensity_carb_base[yr]
            except ZeroDivisionError:
            ...
            carb_tot_comp_meas_mkt = carb_total_sbmkt[yr] * diffuse_frac * comp_frac_diffuse_meas
            carb_tot_comp_meas = (
            ...
            ) * (1 - remain_fuel_frac) + carb_tot_comp_meas_mkt * tsv_carb_base * remain_fuel_frac
            ...
            carb_tot_uncomp_meas_mkt = (
                carb_total_sbmkt[yr] * diffuse_frac * (1 - comp_frac_diffuse) * meas_cum_frac
            )
            carb_tot_uncomp_meas = (
                ...
                * (site_source_conv_meas[yr] / site_source_conv_base[yr])
                * intensity_carb_ratio
            ) * (1 - remain_fuel_frac) + carb_tot_uncomp_meas_mkt * tsv_carb_base * remain_fuel_frac
            ...
            # Total-efficient energy
            carb_total_eff[yr] = carb_compete_eff[yr] + carb_tot_uncomp_meas + carb_tot_uncomp_base
            ...
            if f_meth_assess:
                ...
                fmeth_tot_comp_meas_mkt = (
                    fmeth_total_sbmkt[yr] * diffuse_frac * comp_frac_diffuse_meas
                )
                fmeth_tot_comp_meas = (
                    fmeth_tot_comp_meas_mkt
                    * tsv_energy_eff
                    ...
                    * lkg_fmeth_ratio
                    ...
                    1 - remain_fuel_frac
                ) + fmeth_tot_comp_meas_mkt * tsv_energy_base * remain_fuel_frac
                # Competed fugitive methane not captured by measure
                fmeth_tot_comp_base = (
                    fmeth_total_sbmkt[yr]
                    * diffuse_frac
                    * tsv_energy_base
                    * (comp_frac_diffuse - comp_frac_diffuse_meas)
                )
                # Uncompeted fugitive methane captured by measure
                fmeth_tot_uncomp_meas_mkt = (
                    fmeth_total_sbmkt[yr] * diffuse_frac * (1 - comp_frac_diffuse) * meas_cum_frac
                )
                fmeth_tot_uncomp_meas = (
                    ...
                    * lkg_fmeth_ratio
                    ...
                    1 - remain_fuel_frac
                ) + fmeth_tot_uncomp_meas_mkt * tsv_energy_base * remain_fuel_frac
                # Uncompeted fugitive methane not captured by measure
                fmeth_tot_uncomp_base = (
                    fmeth_total_sbmkt[yr]
                    * diffuse_frac
                    * tsv_energy_base
                    * (1 - comp_frac_diffuse)
                ...
                fmeth_compete_eff[yr] = fmeth_tot_comp_meas + fmeth_tot_comp_base
                # Total-efficient fugitive methane
                fmeth_total_eff[yr] = (
                    fmeth_compete_eff[yr] + fmeth_tot_uncomp_meas + fmeth_tot_uncomp_base
                )
            # Refrigerants
            if f_refr_assess:
                # Check for flag to force measure refrig. emissions to zero
                if f_refr["efficient"][1] is True:
                    frefr_tot_comp_meas, frefr_tot_uncomp_meas = (0 for n in range(2))
                else:
                    # # Competed fugitive refrigerants captured by measure
                    frefr_tot_comp_meas_mkt = (
                        stock_total_sbmkt[yr]
                        * stk_serv_cap_cnv
                        * diffuse_frac
                    ...
                    frefr_tot_uncomp_meas = (
                        frefr_tot_uncomp_meas_mkt * rel_frefr_capt * (1 - remain_fuel_frac)
                    ) + frefr_tot_uncomp_meas_mkt * remain_fuel_frac
                # Check for flag to force baseline refrig. emissions to zero
                if f_refr["baseline"][1] is True:
                    frefr_tot_comp_base, frefr_tot_uncomp_base = (0 for n in range(2))
                else:
                ...
                frefr_compete_eff[yr] = frefr_tot_comp_meas + frefr_tot_comp_base
                # Total-efficient fugitive refrigerants
                frefr_total_eff[yr] = (
                    frefr_compete_eff[yr] + frefr_tot_uncomp_meas + frefr_tot_uncomp_base
                )
                # Reset previously captured baseline refrigerant emissions for
                # next year to that of overall baseline stock in current yr.
                base_f_r_unit_capt, rel_frefr_capt = [base_f_r_unit_overall, rel_frefr_overall]
            ...
            if self.handyvars.low_volume_rate is not None and len(alt_rates_init) != 0:
                ...
                alt_rates = [
                    seg
                    for seg in alt_rates_init
                    if ((not seg[-3] or seg[-3] <= int(yr)) and (not seg[-2] or seg[-2] >= int(yr)))
                ]
                ...
                if len(alt_rates) != 0:
                    ...
                    for alt_rate in alt_rates:
                        ...
                        cost_energy_meas_adj_apply_cum += (
                            cost_energy_meas[yr] - vol_reduce
                        ) * apply_frac
            ...
            energy_cost_tot_comp_meas = (
                ...
                * rel_perf[yr]
                * (site_source_conv_meas[yr] / site_source_conv_base[yr])
                * cost_energy_meas_yr_adj
            ) * (
                1 - remain_fuel_frac
            ) + energy_cost_tot_comp_meas_mkt * tsv_ecost_base * cost_energy_base[
                yr
            ] * remain_fuel_frac
            ...
            energy_cost_tot_uncomp_meas = (
                ...
                * cost_energy_meas_yr_adj
                ...
                1 - remain_fuel_frac
            ) + energy_cost_tot_uncomp_meas_mkt * tsv_ecost_base * cost_energy_base[
                yr
            ] * remain_fuel_frac
            ...
            carb_compete_cost[yr] = (
                carb_compete[yr] * lnkd_ecarb_cost_adj_fact[yr] * self.handyvars.ccosts[yr]
            )
            # Total baseline carbon cost
            carb_total_cost[yr] = (
                carb_total[yr] * lnkd_ecarb_cost_adj_fact[yr] * self.handyvars.ccosts[yr]
            )
            ...
            if new_cool_units:
                ...
                elif all([not x for x in [uec_eff_yr, ci_eff_yr, uecst_eff_yr]]):
                    uec_eff_yr, ci_eff_yr, uecst_eff_yr = (0 for n in range(3))
                    if f_refr_assess:
                        f_r_unit = 0
                ...
                if stock_total_meas[yr] != 0:
                    uec_eff_yr_tot, ci_eff_yr_tot, uecst_eff_yr_tot = [
                        ((energy_tot_comp_meas + energy_tot_uncomp_meas) / stock_total_meas[yr]),
                        ((carb_tot_comp_meas + carb_tot_uncomp_meas) / stock_total_meas[yr]),
                        (
                            (energy_cost_tot_comp_meas + energy_cost_tot_uncomp_meas)
                            / stock_total_meas[yr]
                        ),
                    ]
                    # Unit metric for refrigerants, if needed
                ...
                elif all([not x for x in [uec_eff_yr_tot, ci_eff_yr_tot, uecst_eff_yr_tot]]):
                    uec_eff_yr_tot, ci_eff_yr_tot, uecst_eff_yr_tot = (0 for n in range(3))
                    if f_refr_assess:
                        f_r_unit_tot = 0
                ...
                energy_compete_eff[yr] = new_cool_units["competed"][yr] * uec_eff_yr
                energy_total_eff[yr] = new_cool_units["total"][yr] * uec_eff_yr_tot
                if eff_capt:
                    energy_total_eff_capt[yr] = new_cool_units["total"][yr] * uec_eff_yr_tot
                # Carbon
                # Reset competed and total carbon
                carb_compete_eff[yr] = new_cool_units["competed"][yr] * ci_eff_yr
                carb_total_eff[yr] = new_cool_units["total"][yr] * ci_eff_yr_tot
                # Energy costs
                # Reset competed and total energy costs
                energy_compete_cost_eff[yr] = new_cool_units["competed"][yr] * uecst_eff_yr
                energy_total_eff_cost[yr] = new_cool_units["total"][yr] * uecst_eff_yr_tot
                # Carbon costs
                # Reset competed and total carbon costs
                carb_compete_cost_eff[yr] = carb_compete_eff[yr] * self.handyvars.ccosts[yr]
                carb_total_eff_cost[yr] = carb_total_eff[yr] * self.handyvars.ccosts[yr]
                # Refrigerants
                if f_refr_assess:
                    # Reset competed and total refrigerant leakage emissions
                    frefr_compete_eff[yr] = new_cool_units["competed"][yr] * f_r_unit
                    frefr_total_eff[yr] = new_cool_units["total"][yr] * f_r_unit_tot
                ...
                (
                    ...
                    energy_compete_cost,
                    carb_compete_cost,
                ) = ({yr: 0 for yr in self.handyvars.aeo_years} for n in range(16))
            ...
            if self.fuel_switch_to is not None:
                ...
                # tech in each year)
                if not hp_rate:
                    fs_energy_eff_remain_base[yr] = energy_tot_comp_base + energy_tot_uncomp_base
                    fs_carb_eff_remain_base[yr] = carb_tot_comp_base + carb_tot_uncomp_base
                ...
                fs_energy_cost_eff_remain_switch[yr] = (
                    (energy_cost_tot_comp_meas_mkt + energy_cost_tot_uncomp_meas_mkt)
                    * tsv_ecost_base
                    * cost_energy_base[yr]
                    * remain_fuel_frac
                )
    ...
    def check_meas_inputs(self):
        ...
        for attr_nm in [
            ...
            if (attr is None and none_ok is False) and (not isinstance(attr, val_types)):
                # Add error message
                attr_errs.append(
                    attr_nm
                    + " for measure '"
                    + self.name
                    + "' must be value of type: "
                    + str(val_types)
                )
            # Check for issue with attribute value under number or dict (check
            # only goes one level down in nested dict)
            elif (
                isinstance(attr, (float, int))
                and (attr < val_min or (val_max and attr > val_max))
                or (
                    ...
                    and any(
                        [
                            (
                                ...
                                and (y is None or (y < val_min or (val_max and y > val_max)))
                            )
        ...
        for x in [
            ...
            if isinstance(x, dict):
                [
                    check_list.extend(x[ms])
                    if isinstance(x[ms], list)
                    else check_list.append(x[ms])
        ...
        if len(invalid_names) > 0 and (
            (
                isinstance(self.climate_zone, list)
                and all([x in self.climate_zone for x in invalid_names])
            )
            or (
                isinstance(self.climate_zone, str)
                and all([x == self.climate_zone for x in invalid_names])
            )
        ):
            raise ValueError(
        ...
        if self.fuel_switch_to == "electricity" and (
            ...
            or (
                isinstance(self.fuel_type, list)
                and not any([x != "electricity" for x in self.fuel_type])
            )
        ):
            raise ValueError(
    ...
    def fill_attr(self):
        ...
        if (
            not isinstance(self.technology, list)
            and self.technology is not None
            and "windows" in self.technology
        ):
        ...
        if isinstance(self.end_use, dict):
            ...
            # is set to 'demand')
            if (
                isinstance(self.end_use["secondary"], list)
                and any(
                    [
                        x not in ["heating", "secondary heating", "cooling"]
                        for x in self.end_use["secondary"]
                    ]
                )
            ) or (
                ...
                self.technology_type = {"primary": self.technology_type, "secondary": "supply"}
            else:
                self.technology_type = {"primary": self.technology_type, "secondary": "demand"}
        ...
        # set 'secondary' key values for relevant attributes to None
            ...
            (
                ...
                self.energy_efficiency_units,
                self.fuel_type,
                self.end_use,
                self.technology,
                self.technology_type,
            ) = [
                {"primary": x, "secondary": None}
                for x in [
                    self.energy_efficiency,
                    self.energy_efficiency_units,
                    self.fuel_type,
            ...
            for end_use_key in ["primary", "secondary"]:
                if isinstance(self.end_use[end_use_key], list):
                    self.end_use[end_use_key] = [
                        "office equipment" if x in ["PCs", "non-PC office equipment"] else x
                        for x in self.end_use[end_use_key]
                    ]
                elif self.end_use[end_use_key] in ["PCs", "non-PC office equipment"]:
        ...
        # Fill out 'warm climates' or 'cold climates' region input'
            ...
            [
                self.climate_zone == x or x in self.climate_zone
                for x in self.handyvars.warm_cold_regs.keys()
            ]
        ):
            ...
            if isinstance(self.climate_zone, str):
                self.climate_zone = self.handyvars.warm_cold_regs[self.climate_zone]
            # Case where region input is formatted as list
            else:
                # Find which of the warm/cold climate strings is present
                wrm_cld_key = [
                    x for x in self.climate_zone if x in self.handyvars.warm_cold_regs.keys()
                ]
                # If both are present, fill out all the warm/cold climates
                if len(wrm_cld_key) == 2:
                    self.climate_zone = [
                        x for v in self.handyvars.warm_cold_regs.values() for x in v
                    ]
                # Otherwise, key in either the full warm or full cold climate
                # lists
                else:
                    self.climate_zone = self.handyvars.warm_cold_regs[wrm_cld_key[0]]
        ...
        if isinstance(self.energy_efficiency["primary"], dict) and all(
            [
                x in self.energy_efficiency["primary"].keys()
                for x in self.handyvars.warm_cold_regs.keys()
            ]
        ):
            ...
            warm_clim_keys, cold_clim_keys = [
                [
                    x
                    for x in self.handyvars.warm_cold_regs[y]
                    if (self.climate_zone == x or x in self.climate_zone)
                ]
                for y in [k for k in self.handyvars.warm_cold_regs.keys()]
            ]
            # Determine values under warm/cold designations in ECM definition
            # to copy across each warm and cold climate region
            warm_clim_vals, cold_clim_vals = [
                self.energy_efficiency["primary"][x] for x in self.handyvars.warm_cold_regs.keys()
            ]
            # Reset energy efficient data with initial breakouts and values
            # for each of the warm regions
            self.energy_efficiency["primary"] = {reg_w: warm_clim_vals for reg_w in warm_clim_keys}
        ...
        for mseg_type in [x[0] for x in self.end_use.items() if x[1] is not None]:
            ...
            if (
                self.bldg_type == "all"
                or ("all" in self.bldg_type and self.bldg_type != "small office")
                or (
                    isinstance(self.bldg_type, list)
                    and any(["all" in b for b in self.bldg_type if b != "small office"])
                )
            ):
                ...
                [
                    self.bldg_type.extend(b[1])
                    for b in self.handyvars.in_all_map["bldg_type"].items()
                    if b[1] not in self.bldg_type
                    and (
                ...
                bldgsect_list = [
                    x[0]
                    for x in self.handyvars.in_all_map["bldg_type"].items()
                    if any([bt in x[1] for bt in self.bldg_type])
                ]
            ...
            # 'fuel_type,' 'end_use,' and 'technology' attributes below)
                ...
                bldgsect_list = [
                    b[0]
                    for b in self.handyvars.in_all_map["bldg_type"].items()
                    if any([bta == self.bldg_type for bta in b[1]])
                    or any([bta in b[1] for bta in self.bldg_type])
                ]

            # Fill out an 'all' fuel type input
            if self.fuel_type[mseg_type] == "all" or "all" in self.fuel_type[mseg_type]:
                ...
                for b in bldgsect_list:
                    [
                        self.fuel_type[mseg_type].append(f)
                        for f in self.handyvars.in_all_map["fuel_type"][b]
                        if f not in self.fuel_type[mseg_type]
                    ]
            ...
            if self.end_use[mseg_type] == "all" or "all" in self.end_use[mseg_type]:
                ...
                for b in bldgsect_list:
                    for f in [
                        ...
                        [
                            ...
                            for e in self.handyvars.in_all_map["end_use"][b][f]
                            if e not in self.end_use[mseg_type]
                        ]
            ...
            if (
                ...
                or self.technology[mseg_type] == ["all"]
                or (
                    isinstance(self.technology[mseg_type], list)
                    and any([t is not None and "all " in t for t in self.technology[mseg_type]])
                )
                or (
                    isinstance(self.technology[mseg_type], str)
                    and self.technology[mseg_type] is not None
                    and "all " in self.technology[mseg_type]
                )
            ):
                ...
                if self.technology[mseg_type] == "all":
                    ...
                    self.technology[mseg_type] = [
                        t for t in self.technology[mseg_type] if ("all " not in t and t != "all")
                    ]
                else:
                ...
                for b in bldgsect_list:
                    ...
                    if self.technology_type[mseg_type] == "demand":
                        [
                            self.technology[mseg_type].append(t)
                            for t in self.handyvars.in_all_map["technology"][b][
                                self.technology_type[mseg_type]
                            ]
                            if t not in self.technology[mseg_type]
                        ]
                    ...
                    # end use(s)
                        ...
                        for f in [
                            ft
                            for ft in fueltype_list
                            if ft in self.handyvars.in_all_map["fuel_type"][b]
                        ]:
                            for e in [
                                eu
                                for eu in enduse_list
                                if eu in self.handyvars.in_all_map["end_use"][b][f]
                            ]:
                                ...
                                [
                                    ...
                                    and (
                                        map_tech_orig == "all"
                                        or map_tech_orig == ["all"]
                                        or any(
                                            [
                                                (
                                                    (
                                                        "secondary heating" not in torig
                                                        and e in torig
                                                    )
                                                    or (
                                                        "secondary heating" in torig
                                                        and e == "secondary heating"
                                                    )
                                                )
                                                for torig in map_tech_orig
                                                if "all " in torig
                                            ]
                                        )
                                    )
                                    or e in map_tech_orig
                                ]
    ...
    def create_keychain(self, mseg_type):
        ...
        if any([x in ht_cl_euses for x in self.end_use[mseg_type]]):
            ...
            eu_non_hc = [x for x in self.end_use[mseg_type] if x not in ht_cl_euses]
            ...
            if len(eu_non_hc) > 0:
                ...
                ms_iterable_init.extend(list(itertools.product(*ms_lists_add)))
                for ind, ms_lists_add_i in enumerate(ms_lists_add):
                    if ind != (len(ms_lists_add) - 1):
                        ms_lists[ind].extend([x for x in ms_lists_add_i if x not in ms_lists[ind]])
                    ...
                    # to last element in 'ms_lists'
                        ...
                        ms_lists[(ind + 1)].extend(
                            [x for x in ms_lists_add_i if x not in ms_lists[(ind + 1)]]
                        )
    ...
    def add_elec_infr_costs(self, cost_meas, mskeys, elec_infr_flag, opts):
        ...
        if (
            self.add_elec_infr_cost
            and "panel" in self.add_elec_infr_cost.keys()
            and self.add_elec_infr_cost["panel"] is True
        ):
        ...
        # Measure does not trigger panel upgrade; set cost to zero
            ...
            self.add_elec_infr_cost
            and "panel" in self.add_elec_infr_cost.keys()
            and self.add_elec_infr_cost["panel"] is False
        ):
            add_pnl_cost = 0
        # User has not specified one way or another whether measure incurs panel upgrade
            ...
            if (
                ...
                and all([x in mskeys for x in ["existing", "heating"]])
                and all([x not in mskeys[-2] for x in ["no panel", "manage"]])
            ):
                add_pnl_cost = self.handyvars.elec_infr_costs["panel replacement"]
            elif (
                "electricity" not in mskeys
                and all([x in mskeys for x in ["existing", "heating"]])
                and "manage" in mskeys[-2]
            ):
                add_pnl_cost = self.handyvars.elec_infr_costs["panel management"]
        ...
        if (
            self.add_elec_infr_cost
            and "240V circuit" in self.add_elec_infr_cost.keys()
            and self.add_elec_infr_cost["240V circuit"] is True
        ):
    ...
    def append_panel_msegs(self, ms_iterable, anchor_techs):
        ...
        segs_to_subset = [
            ...
            for x in ms_iterable
            if (
                all([y in x for y in ["single family home", "existing"]])
                and any([y in x for y in ["heating", "secondary heating", "cooling"]])
            )
        ]
    ...
    def find_scnd_overlp(self, vint_frac, ss_conv, dict1, energy_tot):
        ...
        for k, i in dict1.items():
            ...
            # energy data has not been reached
                ...
                self.find_scnd_overlp(vint_frac, ss_conv, i, energy_tot)
            ...
            # data to the total energy use variable
                ...
                try:  # Handle blank lighting incandescent energy dicts
                    for yr in self.handyvars.aeo_years:
                        energy_tot[yr] += dict1["energy"][yr] * vint_frac[yr] * ss_conv[yr]
                except KeyError:
                    pass
        ...
        for k in dict1:
            if k not in dict2:
                ...
                if k == "efficient-captured-envelope":
                    continue
                raise KeyError(
                    "When adding together two dicts "
                    "for ECM '" + self.name + "' update, dict key structures "
                    "do not match"
                )
            i = dict1[k]
    ...
    def div_keyvals(self, dict1, dict2):
        ...
        for k, i in dict1.items():
            if isinstance(i, dict):
                ...
                if (isinstance(dict2[k], numpy.ndarray) and all(dict2[k]) != 0) or (
                    not isinstance(dict2[k], numpy.ndarray) and dict2[k] != 0
                ):
    ...
    def div_keyvals_float(self, dict1, reduce_num):
        ...
        for k, i in dict1.items():
            if isinstance(i, dict):
                ...
                # Handle zero values
                if (isinstance(reduce_num, numpy.ndarray) and all(reduce_num) != 0) or (
                    not isinstance(reduce_num, numpy.ndarray) and reduce_num != 0
                ):
    ...
    def adj_pkg_mseg_keyvals(self, cmsegs, base_adj, eff_adj, eff_adj_c, base_eff_flag, comp_flag):
        ...
        for k, i in cmsegs.items():
            ...
            if k in "competed":
                comp_flag = True
            if isinstance(i, dict):
                self.adj_pkg_mseg_keyvals(i, base_adj, eff_adj, eff_adj_c, base_eff_flag, comp_flag)
            else:
                ...
                if base_eff_flag == "baseline" and k in self.handyvars.aeo_years:
                    ...
                    base_eff_flag in ["efficient", "efficient-captured"]
                    and k in self.handyvars.aeo_years
                ):
    ...
    def rand_list_gen(self, distrib_info, nsamples):
        ...
        if len(distrib_info) == 3 and distrib_info[0] == "normal":
            ...
            rand_list = numpy.random.gamma(distrib_info[1], distrib_info[2], nsamples)
            ...
            rand_list = numpy.random.triangular(
                distrib_info[1], distrib_info[2], distrib_info[3], nsamples
            )
        else:
            raise ValueError(
    ...
    def breakout_mseg(
        self, mskeys, contrib_mseg_key, adopt_scheme, opts, brk_in_dat, brk_stk_costs
    ):
        ...
        brk_stock_cost_total, brk_stock_cost_total_meas, brk_fs_stk_cost_eff_remain = brk_stk_costs
        ...
        try:
            ...
            swtch_tech_base_fuel = any([x != 0 for x in brk_fs_energy_eff_remain_switch.values()])
            ...
            if self.fuel_switch_to is not None and out_fuel_save:
                ...
                self.eff_fs_splt[adopt_scheme][str(contrib_mseg_key)] = {
                    "energy": [
                        brk_fs_energy_eff_remain_base,
                        brk_fs_energy_eff_remain_switch,
                        brk_energy_total_eff,
                ...
                try:
                    for yr in _aeo_years:
                        for ind, key in enumerate(breakout_vars):
                            self.markets[adopt_scheme]["mseg_out_break"][key]["baseline"][out_cz][
                                out_bldg
                            ][out_eu][out_fuel_save][yr] += base_data[ind][yr]
                            ...
                            if not out_fuel_gain:
                                self.markets[adopt_scheme]["mseg_out_break"][key]["efficient"][
                                    out_cz
                                ][out_bldg][out_eu][out_fuel_save][yr] += eff_data[ind][yr]
                                if key == "energy" and capt_e:
                                    self.markets[adopt_scheme]["mseg_out_break"][key][
                                        "efficient-captured"
                                    ][out_cz][out_bldg][out_eu][out_fuel_save][yr] += capt_e[yr]
                                if key != "stock":  # no stk save
                                    self.markets[adopt_scheme]["mseg_out_break"][key]["savings"][
                                        out_cz
                                ...
                                self.markets[adopt_scheme]["mseg_out_break"][key]["efficient"][
                                    ...
                                    eff_data_fs_base[ind][yr] + eff_data_fs_switch[ind][yr]
                                )
                                ...
                                if key != "stock":  # no stk save
                                    self.markets[adopt_scheme]["mseg_out_break"][key]["savings"][
                                        out_cz
                                    ][out_bldg][out_eu][out_fuel_save][yr] += base_data[ind][yr] - (
                                        eff_data_fs_base[ind][yr] + eff_data_fs_switch[ind][yr]
                                    )
                except KeyError:
                    for ind, key in enumerate(breakout_vars):
                        ...
                        self.markets[adopt_scheme]["mseg_out_break"][key]["baseline"][out_cz][
                            out_bldg
                        ][out_eu][out_fuel_save] = {yr: base_data[ind][yr] for yr in _aeo_years}
                        ...
                        # unswitched in the efficient case and the baseline
                        if not out_fuel_gain:
                            self.markets[adopt_scheme]["mseg_out_break"][key]["efficient"][out_cz][
                                out_bldg
                            ][out_eu][out_fuel_save] = {yr: eff_data[ind][yr] for yr in _aeo_years}
                            if key == "energy" and capt_e:
                                self.markets[adopt_scheme]["mseg_out_break"][key][
                                    "efficient-captured"
                                ][out_cz][out_bldg][out_eu][out_fuel_save] = {
                                    yr: capt_e[yr] for yr in _aeo_years
                                }
                            if key != "stock":  # no stk save
                                self.markets[adopt_scheme]["mseg_out_break"][key]["savings"][
                                    out_cz
                                ][out_bldg][out_eu][out_fuel_save] = {
                                    yr: (base_data[ind][yr] - eff_data[ind][yr])
                                    for yr in _aeo_years
                                }
                        else:
                            self.markets[adopt_scheme]["mseg_out_break"][key]["efficient"][out_cz][
                                ...
                                yr: (eff_data_fs_base[ind][yr] + eff_data_fs_switch[ind][yr])
                                for yr in _aeo_years
                            }
                            # Note that no baseline fuel, baseline technology
                            # consumption (e.g., not in backup service to
                            ...
                            if key == "energy" and capt_e:
                                self.markets[adopt_scheme]["mseg_out_break"][key][
                                    ...
                                    yr: eff_data_fs_switch[ind][yr] for yr in _aeo_years
                                }
                            if key != "stock":  # no stk save
                                self.markets[adopt_scheme]["mseg_out_break"][key]["savings"][
                                    out_cz
                        ...
                        for yr in _aeo_years:
                            for ind, key in enumerate(breakout_vars):
                                ...
                                if key != "stock":  # no stk save
                                    self.markets[adopt_scheme]["mseg_out_break"][key]["efficient"][
                                        out_cz
                                    ][out_bldg][out_eu][out_fuel_gain][yr] += eff_data[ind][yr] - (
                                        eff_data_fs_base[ind][yr] + eff_data_fs_switch[ind][yr]
                                    )
                                    ...
                                    if key == "energy" and capt_e:
                                        self.markets[adopt_scheme]["mseg_out_break"][key][
                                            ...
                                            capt_e[yr] - eff_data_fs_switch[ind][yr]
                                        )
                                    self.markets[adopt_scheme]["mseg_out_break"][key]["savings"][
                                        out_cz
                                    ][out_bldg][out_eu][out_fuel_gain][yr] -= eff_data[ind][yr] - (
                                    ...
                                    if key == "energy" and capt_e:
                                        self.markets[adopt_scheme]["mseg_out_break"][key][
                                            "efficient-captured"
                                        ][out_cz][out_bldg][out_eu][out_fuel_gain][yr] += capt_e[yr]
                    except KeyError:
                        for ind, key in enumerate(breakout_vars):
                            ...
                            self.markets[adopt_scheme]["mseg_out_break"][key]["baseline"][out_cz][
                                out_bldg
                            ][out_eu][out_fuel_gain] = dict.fromkeys(_aeo_years, 0)
                            # Efficient and savings; efficient case energy/
                            ...
                            if key != "stock":  # no stk save
                                ...
                                if key == "energy" and capt_e:
                                    self.markets[adopt_scheme]["mseg_out_break"][key][
                                        ...
                                        yr: (capt_e[yr] - eff_data_fs_switch[ind][yr])
                                        for yr in _aeo_years
                                    }
                                self.markets[adopt_scheme]["mseg_out_break"][key]["savings"][
                                    out_cz
                                ...
                                self.markets[adopt_scheme]["mseg_out_break"][key]["efficient"][
                                    ...
                                    yr: eff_data[ind][yr] for yr in _aeo_years
                                }
                                if key == "energy" and capt_e:
                                    self.markets[adopt_scheme]["mseg_out_break"][key][
                                        "efficient-captured"
                    ...
                    for yr in _aeo_years:
                        for ind, key in enumerate(breakout_vars):
                            ...
                            if key == "energy" and capt_e:
                                self.markets[adopt_scheme]["mseg_out_break"][key][
                                    "efficient-captured"
                                ][out_cz][out_bldg][out_eu][yr] += capt_e[yr]
                            if key != "stock":  # no stk save
                                self.markets[adopt_scheme]["mseg_out_break"][key]["savings"][
                                    out_cz
                                ][out_bldg][out_eu][yr] += base_data[ind][yr] - eff_data[ind][yr]
                except KeyError:
                    for ind, key in enumerate(breakout_vars):
                        self.markets[adopt_scheme]["mseg_out_break"][key]["baseline"][out_cz][
                            out_bldg
                        ][out_eu] = {yr: base_data[ind][yr] for yr in _aeo_years}
                        self.markets[adopt_scheme]["mseg_out_break"][key]["efficient"][out_cz][
                            out_bldg
                        ][out_eu] = {yr: eff_data[ind][yr] for yr in _aeo_years}
                        if key == "energy" and capt_e:
                            self.markets[adopt_scheme]["mseg_out_break"][key]["efficient-captured"][
                                out_cz
                            ][out_bldg][out_eu] = {yr: capt_e[yr] for yr in _aeo_years}
                        if key != "stock":  # no stk save
                            self.markets[adopt_scheme]["mseg_out_break"][key]["savings"][out_cz][
                                out_bldg
                            ][out_eu] = {
                                yr: (base_data[ind][yr] - eff_data[ind][yr]) for yr in _aeo_years
                            }
                ...
                if key_list[3] in f[1]:
                    ...
                    if detail and key_list[3] == "other fuel":
                        ...
                        if f[0] == "Distillate/Other" and (
                            key_list[-2] is not None
                            and any([x in key_list[-2] for x in ["coal", "kerosene"]])
                        ):
                            out_fuel_save = f[0]
                        # Assign commercial other fuel to
                        # Distillate/Other
                        elif f[0] == "Distillate/Other" and (
                            key_list[2] in self.handyvars.in_all_map["bldg_type"]["commercial"]
                        ):
                            out_fuel_save = f[0]
                        # Assign wood tech.
    ...
    def set_lnkd_cost_exclude(self, mskeys):
        ...
        rmv_hp_dblct_meas_stkcosts = (
            ...
            and any([x in self.tech_switch_to for x in ["HP", "heat pump", "Heat Pump"]])
        ) or (self.tech_switch_to in [None, "NA", "same"] and rmv_hp_dblct_base_stkcosts)
        ...
        rmv_scnd_hvac_stkcosts = (
            ...
            and not all(
                [x in self.handyvars.secondary_hvac_tech for x in self.technology["primary"]]
            )
        )
        ...
        if self.linked_htcl_tover_linked_tech:
            rmv_lnkd_tech_costs = (
                self.linked_htcl_tover
                and self.linked_htcl_tover_anchor_eu not in mskeys
                and (
                    mskeys[-2] is None
                    or (
    ...
    def rec_lnkd_costs(
        ...
        if rec_lnkd_costs:
            ...
            try:
                append_tech = [x for x in self.handyvars.alt_panel_names if x in mskeys[-2]][0]
            except IndexError:
                append_tech = False
            ...
            ctb_mseg_to_add_cost_to = [
                x
                for x in self.markets[adopt_scheme]["mseg_adjust"][
                    "contributing mseg keys and values"
                ].keys()
                if "demand" not in x
                and self.linked_htcl_tover_anchor_eu in x
                and all([elem in x for elem in linked_mseg_elems])
                and (
                    (not append_tech and all([y not in x for y in self.handyvars.alt_panel_names]))
                    or (append_tech and append_tech in x)
                )
            ]
            ...
            for add_to_mseg in ctb_mseg_to_add_cost_to:
                    ...
                    cost_key: {
                        ...
                        if (
                            ...
                            and (
                                (
                                    cost_key == "stock"
                                    and not opts.no_lnkd_stk_costs
                                    and not dmd_meas
                                    and not rmv_hp_dblct_meas_stkcosts
                                    and not rmv_scnd_hvac_stkcosts
                                )
                                or cost_key != "stock"
                                and not opts.no_lnkd_op_costs
                            )
                        )
                        else 0
                        for yr in self.handyvars.aeo_years
                ...
                self.markets[adopt_scheme]["mseg_adjust"]["linked mseg values"][add_to_mseg] = (
                    transfer_costs_loop
                )
            ...
            if (
                is_in_package
                and "paired heat/cool mseg adjustments"
                in self.markets[adopt_scheme]["mseg_adjust"].keys()
            ):
...
class MeasurePackage(Measure):
    ...
    def __init__(self, measure_list_package, p, bens, handyvars, handyfiles, opts, convert_data):
        self.name = p
        ...
        if not all(
            [
                all(
                    [
                        m.usr_opts[x] == self.contributing_ECMs[0].usr_opts[x]
                        for x in self.contributing_ECMs[0].usr_opts.keys()
                    ]
                )
        ...
        self.contributing_ECMs_eqp = [
            m for m in self.contributing_ECMs if m.technology_type["primary"][0] == "supply"
        ]
        ...
        # with inconsistent fuel switching settings
            ...
            all(
                [
                    all(
                        [
                            x in ["heating", "secondary heating", "ventilation", "cooling"]
                            for x in m.end_use["primary"]
                        ]
                    )
            ...
            or (
                all([m.end_use["primary"] == "lighting" for m in self.contributing_ECMs_eqp])
                and any([m.measure_type == "add-on" for m in self.contributing_ECMs_eqp])
            )
        ):
            raise ValueError(
        ...
        # different settings for heating/cooling equipment pairings
            ...
            for attr in shared_attrs:
                ...
                if not all(
                    [getattr(m, attr) == ref_meas_attr for m in self.contributing_ECMs_eqp[1:]]
                ):
                    raise ValueError(
        ...
        self.contributing_ECMs_env = [
            m for m in self.contributing_ECMs if m.technology_type["primary"][0] == "demand"
        ]
        ...
        if any(
            [
                x.market_entry_year is None
                or (int(x.market_entry_year) < int(x.handyvars.aeo_years[0]))
                for x in self.contributing_ECMs_eqp
            ]
        ):
            self.market_entry_year = int(handyvars.aeo_years[0])
        else:
            self.market_entry_year = min([x.market_entry_year for x in self.contributing_ECMs_eqp])
        # Set market exit year is latest of all the packaged eqp. measures
        if any(
            [
                x.market_exit_year is None
                or (int(x.market_exit_year) > (int(x.handyvars.aeo_years[0]) + 1))
                for x in self.contributing_ECMs_eqp
            ]
        ):
            self.market_exit_year = int(handyvars.aeo_years[-1]) + 1
        else:
            self.market_exit_year = max([x.market_entry_year for x in self.contributing_ECMs_eqp])
        self.yrs_on_mkt = [str(i) for i in range(self.market_entry_year, self.market_exit_year)]
        if all([m.measure_type == "full service" for m in self.contributing_ECMs_eqp]):
            self.meas_typ = "full service"
        elif all([m.measure_type == "add-on" for m in self.contributing_ECMs_eqp]):
            self.meas_typ = "add-on"
        else:
            self.meas_typ = "full service"
        self.climate_zone, self.bldg_type, self.structure_type = ([] for n in range(3))
        self.fuel_type, self.end_use, self.technology, self.technology_type = (
        ...
        self.sector_shapes = None
        self.htcl_overlaps = {
            adopt: {"keys": [], "data": {}} for adopt in handyvars.adopt_schemes_prep
        }
        for adopt_scheme in handyvars.adopt_schemes_prep:
            ...
            # Add stock breakouts
            self.markets[adopt_scheme]["mseg_out_break"]["stock"] = {
                key: copy.deepcopy(self.handyvars.out_break_in) for key in ["baseline", "efficient"]
            }
            # Add stock cost breakouts, contingent on user selecting these
            if self.usr_opts["cap_invest"]:
                self.markets[adopt_scheme]["mseg_out_break"]["capital cost"] = {
                    key: copy.deepcopy(self.handyvars.out_break_in)
                    for key in ["baseline", "efficient", "savings"]
                }
            # Initialize breakouts of efficient energy captured by measure
            ...
            if self.usr_opts["no_eff_capt"] is not True:
                (
                    self.markets[adopt_scheme]["mseg_out_break"]["energy"]["efficient-captured"],
                    self.markets[adopt_scheme]["mseg_out_break"]["energy"][
                        "efficient-captured-envelope"
                    ],
                ) = (copy.deepcopy(self.handyvars.out_break_in) for n in range(2))
    ...
    def merge_measures(self, opts):
        ...
        if opts.pkg_env_costs is not False and len(self.contributing_ECMs_env) > 0:
            ...
            for m in self.contributing_ECMs_eqp:
                ...
                for a_s in self.handyvars.adopt_schemes_prep:
                    ...
                    stk_cpy = copy.deepcopy(
                        m.markets[a_s]["mseg_adjust"]["contributing mseg keys and values"]
                    )
                    ...
                    for cm in stk_cpy.keys():
                        ...
                        if cm not in common_stk.keys():
                            common_stk[a_s][cm] = {
                                met: {
                                    yr: stk_cpy[cm]["stock"][met]["measure"][yr]
                                    for yr in self.handyvars.aeo_years
                                }
                        ...
                        # adding envelope costs to the package later)
                            ...
                            common_stk[a_s][cm] = {
                                met: {
                                    yr: stk_cpy[cm]["stock"][met]["measure"][yr]
                                    if stk_cpy[cm]["stock"][met]["measure"][yr]
                                    > common_stk[a_s][cm][met][yr]
                                    else common_stk[a_s][cm][met][yr]
                                    for yr in self.handyvars.aeo_years
                                }
                                for met in ["total", "competed"]
                            }
        ...
        for ind, m in enumerate(self.contributing_ECMs_eqp):
            ...
            self.climate_zone.extend(list(set(m.climate_zone) - set(self.climate_zone)))
            ...
            self.fuel_type["primary"].extend(
                list(set(m.fuel_type["primary"]) - set(self.fuel_type["primary"]))
            )
            # Add measure end uses
            self.end_use["primary"].extend(
                list(set(m.end_use["primary"]) - set(self.end_use["primary"]))
            )
            # Add measure technologies
            self.technology["primary"].extend(
                list(set(m.technology["primary"]) - set(self.technology["primary"]))
            )
            ...
            if not self.sector_shapes and m.sector_shapes:
                self.sector_shapes = {
                    a_s: {
                        reg: {
                            yr: {
                                "baseline": [0 for n in range(8760)],
                                "efficient": [0 for n in range(8760)],
                            }
                            for yr in self.handyvars.aeo_years_summary
                        }
                        for reg in self.climate_zone
                    }
                    for a_s in self.handyvars.adopt_schemes_run
                }
            ...
            for adopt_scheme in self.handyvars.adopt_schemes_prep:
                ...
                msegs_meas_init = copy.deepcopy(m.markets[adopt_scheme]["mseg_adjust"])
                ...
                # scenario
                if self.handyvars.full_dat_out[adopt_scheme]:
                    mseg_out_break_init = copy.deepcopy(m.markets[adopt_scheme]["mseg_out_break"])
                    ...
                    if m.sector_shapes:
                        sect_shp_e_init, sect_shp_e_fin = (
                            {
                                reg: {
                                    yr: {"baseline": 0, "efficient": 0}
                                    for yr in self.handyvars.aeo_years_summary
                                }
                                for reg in m.climate_zone
                            }
                            for n in range(2)
                        )
                    else:
                    ...
                    mseg_out_break_init, sect_shp_e_init, sect_shp_e_fin = (None for n in range(3))
                ...
                for k in msegs_meas_init.keys():
                    ...
                    if k == "contributing mseg keys and values":
                        for cm in msegs_meas_init[k].keys():
                            ...
                            if (
                                len(m.eff_fs_splt[adopt_scheme].keys()) != 0
                                and cm in m.eff_fs_splt[adopt_scheme].keys()
                            ):
                                ...
                                if cm not in self.eff_fs_splt[adopt_scheme].keys():
                                    self.eff_fs_splt[adopt_scheme][cm] = copy.deepcopy(fs_eff_splt)
                            else:
                                fs_eff_splt = None
                            ...
                            if sect_shp_e_init is not None:
                                ...
                                # adjustments
                                sect_shp_e_init[key_list[1]] = self.add_sectshape_energy(
                                    cm,
                                    sect_shp_e_init[key_list[1]],
                                    msegs_meas_init[k][cm]["energy"]["total"],
        ...
        for m in mseg_dat_rec:
            ...
            for adopt_scheme in self.handyvars.adopt_schemes_prep:
                ...
                if self.handyvars.full_dat_out[adopt_scheme]:
                    ...
                    if (
                        "efficient-captured-envelope"
                        in self.markets[adopt_scheme]["mseg_out_break"]["energy"].keys()
                    ):
                        mseg_out_break_fin["energy"]["efficient-captured-envelope"] = copy.deepcopy(
                    ...
                    mseg_out_break_fin, sect_shp_e_fin = (None for n in range(2))
                ...
                for cm in msegs_meas_fin.keys():
                    ...
                    # to split efficient-case energy by fuel type
                    if (
                        len(m[adopt_scheme]["eff_fuel_splits"].keys()) != 0
                        and cm in m[adopt_scheme]["eff_fuel_splits"].keys()
                    ):
                    ...
                    # Find the reporting categories to use for output breakouts
                    out_cz, out_bldg, out_eu, out_fuel_save, out_fuel_gain = self.set_out_brk_cats(
                        key_list, m[adopt_scheme]["fuel_switch_to"]
                    )
                    # Further adjust equipment msegs to account for
                    # overlapping envelope performance improvements
                    msegs_meas_fin[cm], mseg_out_break_fin = self.merge_htcl_overlaps(
                        msegs_meas_fin[cm],
                        cm,
                        adopt_scheme,
                        mseg_out_break_fin,
                    ...
                    if sect_shp_e_fin is not None:
                        ...
                        sect_shp_e_fin[key_list[1]] = self.add_sectshape_energy(
                            cm,
                    ...
                    if cm not in msegs_pkg_fin.keys():
                        ...
                        msegs_pkg_fin[cm] = self.add_keyvals(msegs_pkg_fin[cm], msegs_meas_fin[cm])
                    ...
                    if adopt_scheme == "Technical potential":
                        ...
                        if (
                            "paired heat/cool mseg adjustments"
                            in m[adopt_scheme]["microsegments"].keys()
                            and cm
                            in m[adopt_scheme]["microsegments"][
                                "paired heat/cool mseg adjustments"
                    ...
                    for v in self.markets[adopt_scheme]["mseg_out_break"].keys():
                        for s in ["baseline", "efficient"]:
                            self.merge_out_break(
                                self.markets[adopt_scheme]["mseg_out_break"][v][s],
                                mseg_out_break_fin[v][s],
                            )
                        if v != "stock":  # no stk save breakout
                            self.merge_out_break(
                                self.markets[adopt_scheme]["mseg_out_break"][v]["savings"],
                        ...
                        if v == "energy" and self.usr_opts["no_eff_capt"] is not True:
                            ...
                            for v_sub in ["efficient-captured", "efficient-captured-envelope"]:
                                self.merge_out_break(
                                    self.markets[adopt_scheme]["mseg_out_break"]["energy"][v_sub],
                ...
                if sect_shp_e_fin:
                    self.sector_shapes[adopt_scheme] = {
                        reg: {
                            yr: {
                                s: [
                                    ...
                                    + m[adopt_scheme]["sect_shp_orig"][adopt_scheme][reg][yr][s][x]
                                    * (
                                        (
                                            m[adopt_scheme]["sect_shp_e_fin"][reg][yr][s]
                                            / m[adopt_scheme]["sect_shp_e_init"][reg][yr][s]
                                        )
                                        if m[adopt_scheme]["sect_shp_e_fin"][reg][yr][s] != 0
                                        else 1
                                    )
                                    for x in range(8760)
                                ]
                        ...
                        for reg in self.climate_zone
                        if reg in m[adopt_scheme]["regions"]
                    }
        ...
        for adopt_scheme in self.handyvars.adopt_schemes_prep:
            ...
            if all([m.measure_type == "add-on" for m in self.contributing_ECMs_eqp]):
                life_avg_meas = self.contributing_ECMs_eqp
            else:
                life_avg_meas = [
                    m for m in self.contributing_ECMs_eqp if m.measure_type != "add-on"
                ]
            # Average lifetimes
            self.markets[adopt_scheme]["master_mseg"]["lifetime"]["measure"] = numpy.mean(
                [
                    m.markets[adopt_scheme]["master_mseg"]["lifetime"]["measure"]
            ...
            self.markets[adopt_scheme]["master_mseg"]["lifetime"]["baseline"] = {
                yr: numpy.mean(
                    [
                        m.markets[adopt_scheme]["master_mseg"]["lifetime"]["baseline"][yr]
                        for m in life_avg_meas
                    ]
                )
                for yr in self.handyvars.aeo_years
    ...
    def add_sectshape_energy(
        self, cm, sect_shape_init, add_energy, fs_eff_splt_energy, fuel_switch_to
    ):
        ...
        sect_shape_energy_updated = {
            yr: {
                ...
                (
                    ...
                    if (
                        "electricity" in cm
                        or (fuel_switch_to == "electricity" and s == "efficient")
                    )
                    else 0
                )
                for s in ["baseline", "efficient"]
    ...
    def htcl_adj_rec(self, opts):
        ...
        if all([len(x) != 0 for x in [self.contributing_ECMs_eqp, self.contributing_ECMs_env]]):
            ...
            for ind, m in enumerate(self.contributing_ECMs_eqp):
                ...
                for adopt_scheme in self.handyvars.adopt_schemes_prep:
                    ...
                    for cm_key in [
                        x
                        for x in msegs_meas.keys()
                        if (
                            any([e in x for e in ["heating", "cooling", "secondary heating"]])
                            and x not in self.htcl_overlaps[adopt_scheme]["keys"]
                        )
                    ]:
                        ...
                        cm_key_match = [
                            str(x) for x in [keys[1], keys[2], keys[-1], keys[3], keys[4]]
                        ]
                        ...
                        dmd_match_ECMs = [
                            x
                            for x in self.contributing_ECMs_env
                            if any(
                                [
                                    all(
                                        [
                                            (k != "heating" and k in z)
                                            or (
                                                k == "heating"
                                                and k in z
                                                and "secondary heating" not in z
                                            )
                                            for k in cm_key_match
                                        ]
                                    )
                                    for z in x.markets[adopt_scheme]["mseg_adjust"][
                                        "contributing mseg keys and values"
                                    ].keys()
                                ]
                            )
                        ...
                        if len(dmd_match_ECMs) != 0:
                            ...
                            cm_keys_dmd = [
                                [
                                    x
                                    for x in z.markets[adopt_scheme]["mseg_adjust"][
                                        "contributing mseg keys and values"
                                    ].keys()
                                    if all(
                                        [
                                            (k != "heating" and k in x)
                                            or (
                                                k == "heating"
                                                and k in x
                                                and "secondary heating" not in x
                                            )
                                            for k in cm_key_match
                                        ]
                                    )
                                ]
                                for z in dmd_match_ECMs
                            ]
                            ...
                            _ecm_cmsv = [
                                dmd_match_ECMs[mi].markets[adopt_scheme]["mseg_adjust"][
                                    "contributing mseg keys and values"
                                ]
                            ...
                            _has_eff_capt = (
                                "efficient-captured"
                                in m.markets[adopt_scheme]["master_mseg"]["energy"]["total"].keys()
                            )
                            _htcl_yr = self.handyvars.htcl_totals[keys[1]][keys[2]][keys[-1]][
                                keys[3]
                            ][keys[4]]
                            ...
                            for yr in self.handyvars.aeo_years:
                                dmd_save[yr] = sum(
                                    _ecm_cmsv[mi][cm_keys_dmd[mi][k]]["energy"]["total"][
                                        "baseline"
                                    ][yr]
                                    - _ecm_cmsv[mi][cm_keys_dmd[mi][k]]["energy"]["total"][
                                        "efficient"
                            ...
                            if opts.pkg_env_costs is not False:
                                dmd_stk_cost = [
                                    [
                                        dmd_match_ECMs[m].markets[adopt_scheme]["mseg_adjust"][
                                            "contributing mseg keys and values"
                                        ][cm_keys_dmd[m][k]]["cost"]["stock"]
                                        for k in range(len(cm_keys_dmd[m]))
                                    ]
                                    for m in range(len(dmd_match_ECMs))
                                ]
                                dmd_stk = [
                                    [
                                        dmd_match_ECMs[m].markets[adopt_scheme]["mseg_adjust"][
                                            "contributing mseg keys and values"
                                        ][cm_keys_dmd[m][k]]["stock"]
                                        for k in range(len(cm_keys_dmd[m]))
                                    ]
                                    for m in range(len(dmd_match_ECMs))
                                ]
                            else:
                                dmd_stk_cost, dmd_stk = (None for n in range(2))
                            ...
                            # fuel, and end use
                            if cm_key_store not in self.htcl_overlaps[adopt_scheme]["data"].keys():
                                ...
                                self.htcl_overlaps[adopt_scheme]["data"][cm_key_store] = {
    ...
    def merge_direct_overlaps(
        ...
        htcl_key_match = str(
            [str(x) for x in [key_list[1], key_list[2], key_list[-1], key_list[3], key_list[4]]]
        )
        ...
        if htcl_key_match not in self.htcl_overlaps[adopt_scheme]["data"].keys():
            htcl_key_match = ""
        ...
        if len(overlap_meas) != 0:
            ...
            base_adj, eff_adj, eff_adj_c, eff_capt_env_frac = self.find_base_eff_adj_fracs(
                msegs_meas_init, cm_key, adopt_scheme, name_meas, htcl_key_match, overlap_meas
            )
            ...
            for k in ["energy", "carbon"]:
                ...
                (
                    ...
                    k, cm_key, msegs_meas, base_adj, eff_adj, eff_adj_c, eff_capt_env_frac
                )
            ...
            (
                ...
                "stock", cm_key, msegs_meas, base_adj, eff_adj, eff_adj_c, eff_capt_env_frac
            )
            ...
            if meas_typ == "add-on":
                ...
                fullserv_overlp = any([m.measure_type == "full service" for m in overlap_meas])
                    ...
                    (
                    ...
                    ) = [
                        {
                            yr: (
                                ...
                                - msegs_meas["cost"]["stock"][s]["baseline"][yr]
                            )
                            for yr in self.handyvars.aeo_years
                        }
            ...
            if self.usr_opts["fugitive_emissions"] is not False and self.usr_opts[
                "fugitive_emissions"
            ][0] in ["1", "3"]:
            ...
            if not isinstance(msegs_meas["sub-market scaling"], dict) and all(
                [
                    not isinstance(
                        x.markets[adopt_scheme]["mseg_adjust"]["contributing mseg keys and values"][
                            cm_key
                        ]["sub-market scaling"],
                        dict,
                    )
            ...
            # scaling fractions broken out by year
                ...
                if not isinstance(msegs_meas["sub-market scaling"], dict):
                    current_meas_sbmkt = {
                        yr: msegs_meas["sub-market scaling"] for yr in self.handyvars.aeo_years
                    }
                # Otherwise, set as is
                else:
                ...
                for x in overlap_meas:
                    ...
                    overlap_submkt_init = x.markets[adopt_scheme]["mseg_adjust"][
                        "contributing mseg keys and values"
                    ][cm_key]["sub-market scaling"]
                    ...
                    if not isinstance(overlap_submkt_init, dict):
                        overlap_submkt_dicts.append(
                            {yr: overlap_submkt_init for yr in self.handyvars.aeo_years}
                        )
                    # Otherwise, set as is
                    else:
                ...
                max_submkt_scale = {yr: None for yr in self.handyvars.aeo_years}
                for yr in max_submkt_scale.keys():
                    max_submkt_scale[yr] = max(
                        [current_meas_sbmkt[yr], max([x[yr] for x in overlap_submkt_dicts])]
                    ) / (len(overlap_meas) + 1)
    ...
    def merge_htcl_overlaps(
        ...
        if htcl_key_match in self.htcl_overlaps[adopt_scheme]["data"].keys():
            # Make a copy of the mseg info. that is unaffected by subsequent
            # operations in the loop
            msegs_meas_init = copy.deepcopy(msegs_meas)
            ...
            if opts.pkg_env_costs is not False and not all(
                [
                    msegs_meas["stock"]["total"]["measure"][yr] == 0
                    for yr in self.handyvars.aeo_years
                ]
            ):
                mseg_cost_adj = self.add_env_costs_to_pkg(
                    msegs_meas, adopt_scheme, htcl_key_match, key_list, cm_key, common_stk
                )

        return msegs_meas, mseg_out_break_adj

    def add_env_costs_to_pkg(
        self, msegs_meas, adopt_scheme, htcl_key_match, key_list, cm_key, common_stk
    ):
        ...
        overlp_data_stock_cost = self.htcl_overlaps[adopt_scheme]["data"][htcl_key_match][
            "stock costs"
        ]
        # Shorthand for overlapping envelope stock data
        overlp_data_stock = self.htcl_overlaps[adopt_scheme]["data"][htcl_key_match]["stock"]
        ...
        for olm in range(len(overlp_data_stock_cost)):
            ...
            olm_sc, olm_s = [overlp_data_stock_cost[olm], overlp_data_stock[olm]]
            ...
            for c_mseg in range(len(olm_sc)):
                (
                    ...
                    {yr: (x[yr] + y[yr]) for yr in self.handyvars.aeo_years}
                    for x, y in zip(
                        [
                            tot_stk_eff,
                            tot_stk_cost_eff,
            ...
            if any(
                [
                    x in htcl_key_match
                    for x in ["single family home", "mobile home", "multi family home"]
                ]
            ):
                bldg_sect = "residential"
            else:
            ...
            if bldg_sect == "commercial":
                ...
                if "heating" in htcl_key_match:
                    stk_cnv_1 = self.pkg_env_cost_convert_data["cost unit conversions"][
                        "heating and cooling"
            ...
            tot_stk_eff, comp_stk_eff = [
                {yr: s_e[yr] * convert_env_to_hvac_stk_units for yr in self.handyvars.aeo_years}
                for s_e in [tot_stk_eff, comp_stk_eff]
            ]
            ...
            tot_stk_eff_hvac_unadj, comp_stk_eff_hvac_unadj = [
                {yr: s_u[yr] for yr in self.handyvars.aeo_years}
                for s_u in [
            ...
            # merged with
            tot_env_to_hvac_stk = {
                yr: (tot_stk_eff[yr] / tot_stk_eff_hvac_unadj[yr])
                if (tot_stk_eff[yr] < tot_stk_eff_hvac_unadj[yr])
                else (1 if tot_stk_eff_hvac_unadj[yr] != 0 else 0)
                for yr in self.handyvars.aeo_years
            }
            comp_env_to_hvac_stk = {
                yr: (comp_stk_eff[yr] / comp_stk_eff_hvac_unadj[yr])
                if (comp_stk_eff[yr] < comp_stk_eff_hvac_unadj[yr])
                else (1 if comp_stk_eff_hvac_unadj[yr] != 0 else 0)
                for yr in self.handyvars.aeo_years
            }
            ...
            env_cost_eff_tot_unit = {
                yr: ((tot_stk_cost_eff[yr] / tot_stk_eff[yr]) * tot_env_to_hvac_stk[yr])
                if tot_stk_eff[yr] != 0
                else 0
                for yr in self.handyvars.aeo_years
            }
            env_cost_base_tot_unit = {
                yr: ((tot_stk_cost_base[yr] / tot_stk_base[yr]) * tot_env_to_hvac_stk[yr])
                if tot_stk_eff[yr] != 0
                else 0
                for yr in self.handyvars.aeo_years
            }
            ...
            env_cost_inc_tot_unit = {
                ...
                if (env_cost_eff_tot_unit[yr] - env_cost_base_tot_unit[yr]) >= 0
                else 0
                for yr in self.handyvars.aeo_years
            }
            env_cost_eff_comp_unit = {
                yr: ((comp_stk_cost_eff[yr] / comp_stk_eff[yr]) * comp_env_to_hvac_stk[yr])
                if comp_stk_eff[yr] != 0
                else 0
                for yr in self.handyvars.aeo_years
            }
            env_cost_base_comp_unit = {
                yr: ((comp_stk_cost_base[yr] / comp_stk_base[yr]) * comp_env_to_hvac_stk[yr])
            ...
            env_cost_inc_comp_unit = {
                ...
                if (env_cost_eff_comp_unit[yr] - env_cost_base_comp_unit[yr]) >= 0
                else 0
                for yr in self.handyvars.aeo_years
            }
            ...
            msegs_meas["cost"]["stock"]["total"]["efficient"] = {
                yr: msegs_meas["cost"]["stock"]["total"]["efficient"][yr]
                + env_cost_inc_tot_unit[yr] * msegs_meas["stock"]["total"]["measure"][yr]
                for yr in self.handyvars.aeo_years
            }
            # Adjust competed efficient stock cost to account for incr. envelope cost over base
            msegs_meas["cost"]["stock"]["competed"]["efficient"] = {
                yr: msegs_meas["cost"]["stock"]["competed"]["efficient"][yr]
                + env_cost_inc_comp_unit[yr] * msegs_meas["stock"]["competed"]["measure"][yr]
                for yr in self.handyvars.aeo_years
            }

        return msegs_meas

    def find_base_eff_adj_fracs(
        self, msegs_meas, cm_key, adopt_scheme, name_meas, htcl_key_match, overlap_meas
    ):
        ...
        if not overlap_meas and htcl_key_match:
            ...
            overlp_data = self.htcl_overlaps[adopt_scheme]["data"][htcl_key_match]
            ...
            for yr in self.handyvars.aeo_years:
                ...
                with numpy.errstate(all="ignore"):
                    ...
                    if overlp_data["affected savings"][yr] < 0 or rp_overlp_htcl[yr] > 1:
                        rp_overlp_htcl[yr] = 1
                    elif overlp_data["affected savings"][yr] > 0 and rp_overlp_htcl[yr] < 0:
                        rp_overlp_htcl[yr] = 0
                    ...
                    if overlp_data["efficient-captured"]:
                        ...
                        try:
                            ...
                            # efficient values, to avoid a negative fraction.
                                ...
                                eff_capt_env_frac[yr] = abs(
                                    overlp_data["efficient-captured"][yr]
                                ) / (
                                    abs(overlp_data["efficient-captured"][yr])
            ...
            eff_adj, eff_adj_comp = (
                {yr: rp_overlp_htcl[yr] for yr in self.handyvars.aeo_years} for n in range(2)
            )
        # Case 2: direct overlap between two equipment measures (same mseg)
            ...
            for yr in self.handyvars.aeo_years:
                ...
                # Total
                base_mkts_all = [msegs_meas["energy"]["total"]["baseline"][yr]]
                ...
                # Competed
                base_mkts_all_comp = [msegs_meas["energy"]["competed"]["baseline"][yr]]
                ...
                yr: [
                    (
                        overlap_meas[x].markets[adopt_scheme]["mseg_adjust"][
                            "contributing mseg keys and values"
                        ][cm_key]["energy"]["total"]["baseline"][yr]
                        - overlap_meas[x].markets[adopt_scheme]["mseg_adjust"][
                            "contributing mseg keys and values"
                        ][cm_key]["energy"]["total"]["efficient"][yr]
                    )
                    for x in range(len(overlap_meas))
                ]
                for yr in self.handyvars.aeo_years
            }
            ...
            save_comp_overlp = {
                yr: [
                    (
                        overlap_meas[x].markets[adopt_scheme]["mseg_adjust"][
                            "contributing mseg keys and values"
                        ][cm_key]["energy"]["competed"]["baseline"][yr]
                        - overlap_meas[x].markets[adopt_scheme]["mseg_adjust"][
                            "contributing mseg keys and values"
                        ][cm_key]["energy"]["competed"]["efficient"][yr]
                    )
                    for x in range(len(overlap_meas))
                ]
                for yr in self.handyvars.aeo_years
            ...
            save_wt_meas = {
                yr: (
                    (abs(save_meas[yr]) / (abs(save_meas[yr]) + abs(sum(save_overlp[yr]))))
                    if (abs(save_meas[yr]) + abs(sum(save_overlp[yr]))) != 0
                    else 0
                )
                for yr in self.handyvars.aeo_years
            }
            ...
            save_wt_overlp = {
                yr: [
                    save_overlp[yr][x] / sum(save_overlp[yr]) if sum(save_overlp[yr]) != 0 else 0
                    for x in range(len(save_overlp[yr]))
                ]
                for yr in self.handyvars.aeo_years
            }
            ...
            for yr in self.handyvars.aeo_years:
                ...
                if sum(sbmkts_all[yr]) != 0 and sbmkts_all[yr][0] == 0:
                    ...
                    save_overlp_sbmkt = {
                        yr: [
                            save_overlp[yr][(x - 1)] if (sbmkts_all[yr][x] == 0) else 0
                            for x in range(1, len(sbmkts_all[yr]))
                        ]
                        for yr in self.handyvars.aeo_years
                    }
                    ...
                    with numpy.errstate(all="ignore"):
                        try:
                            sbmkt_save_wt_meas = abs(save_meas[yr]) / (
                                abs(save_meas[yr]) + abs(sum(save_overlp_sbmkt[yr]))
                            )
                        except ZeroDivisionError:
                            sbmkt_save_wt_meas = 1 / len([x for x in sbmkts_all[yr] if x == 0])
                        # Handle numpy NaNs
                    ...
                    sbmkt_base_frac = (
                        sum(
                            [
                                sbmkts_all[yr][x] * save_all_wts[yr][x]
                                for x in range(len(sbmkts_all[yr]))
                            ]
                        )
                        / common_base[yr]
                    )
            ...
            base_adj = {
                yr: (save_wt_meas[yr] + sbmkt_wt_meas[yr])
                if save_wt_meas[yr] != 0
                else (
                    (1 / (len(overlap_meas) + 1) + sbmkt_wt_meas[yr])
                    if sum(save_overlp[yr]) == 0
                    else sbmkt_wt_meas[yr]
                )
                for yr in self.handyvars.aeo_years
            }
            ...
            rp_comp_meas = {
                ...
                if (
                    msegs_meas["stock"]["total"]["measure"][yr] != 0
                    and msegs_meas["stock"]["total"]["all"][yr] != 0
                    and msegs_meas["energy"]["total"]["baseline"][yr] != 0
                )
                else 1
                for yr in self.handyvars.aeo_years
            }
                    ...
                    - (
                        ...
                        (
                            (
                                (
                                    sum(
                                        [x * y for x, y in zip(save_wt_overlp[yr], save_overlp[yr])]
                                    )
                                    / common_base[yr]
                                )
                                * msegs_meas["energy"]["total"]["baseline"][yr]
                        ...
                        (msegs_meas["energy"]["total"]["efficient"][yr] * base_adj[yr])
                    )
                )
                if (msegs_meas["energy"]["total"]["efficient"][yr] * base_adj[yr] != 0)
                else 1
                for yr in self.handyvars.aeo_years
            }
            ...
            rp_overlp_comp = {
                yr: (
                    ...
                    - (
                        ...
                        sum([x * y for x, y in zip(save_wt_overlp[yr], save_comp_overlp[yr])])
                        / common_base_comp[yr]
                    )
                )
                if common_base_comp[yr] != 0
                else 1
                for yr in self.handyvars.aeo_years
            }
            ...
            eff_adj = {yr: base_adj[yr] * rp_overlp[yr] for yr in self.handyvars.aeo_years}
            # Implement competed efficient adjustment
            eff_adj_comp = {
                yr: base_adj[yr] * rp_overlp_comp[yr] for yr in self.handyvars.aeo_years
            }
            eff_capt_env_frac = None
    ...
    def make_base_eff_adjs(
        self, k, cm_key, msegs_meas, base_adj, eff_adj, eff_adj_c, eff_capt_env_frac
    ):
        ...
        if k == "stock":
            ...
            if k == "energy" and self.usr_opts["no_eff_capt"] is not True:
                tot_eff_capt_orig = copy.deepcopy(mseg_adj["total"]["efficient-captured"])
            else:
                tot_eff_capt_orig = ""
        ...
        if k in ["stock", "energy"] and mseg_cost_adj:
            ...
            tot_base_orig_cost = copy.deepcopy(mseg_cost_adj["total"]["baseline"])
            # Total efficient stock or energy cost
            tot_eff_orig_cost = copy.deepcopy(mseg_cost_adj["total"]["efficient"])
        ...
        if k == "stock":
            self.adj_pkg_mseg_keyvals(
                mseg_adj, base_adj, base_adj, base_adj, base_eff_flag=None, comp_flag=None
            )
        else:
            self.adj_pkg_mseg_keyvals(
                mseg_adj, base_adj, eff_adj, eff_adj_c, base_eff_flag=None, comp_flag=None
            )
        ...
        if mseg_cost_adj and k != "stock":
            self.adj_pkg_mseg_keyvals(
                mseg_cost_adj, base_adj, eff_adj, eff_adj_c, base_eff_flag=None, comp_flag=None
            )
        ...
        if k == "energy" and eff_capt_env_frac:
            mseg_adj["total"]["efficient-captured-envelope"] = {
                yr: mseg_adj["total"]["efficient-captured"][yr] * eff_capt_env_frac[yr]
                for yr in aeo_years
            }
    ...
    def find_adj_out_break_cats(
        ...
        if all([x for x in [tot_base_orig_cost, tot_eff_orig_cost, tot_save_orig_cost]]):
        ...
        if k == "stock":
            ...
            base_adj, eff_adj = (
                msegs_st_en_crb["total"]["baseline"],
            ...
            if eff_capt:
                eff_capt_orig, eff_capt_adj = [
                    tot_eff_capt_orig,
                    msegs_st_en_crb["total"]["efficient-captured"],
                ]
        # If necessary, shorthands for data used to adjust original cost output
        if cost_brk_key:
            # Shorthands for data used to adjust original output breakouts
            base_cost_orig, eff_cost_orig, save_cost_orig, base_cost_adj, eff_cost_adj = [
                tot_base_orig_cost,
                tot_eff_orig_cost,
                tot_save_orig_cost,
        ...
        if out_fuel_save and out_fuel_gain:
            ...
            if k != "stock":
                fs_eff_splt_var = {
                    yr: ((fs_eff_splt[k][0][yr] + fs_eff_splt[k][1][yr]) / fs_eff_splt[k][2][yr])
                    if fs_eff_splt[k][2][yr] != 0
                    else 1
                    for yr in aeo_years
                }
                ...
                if eff_capt and fs_eff_splt[k][-1]:
                    fs_eff_splt_var_capt = {
                        yr: fs_eff_splt[k][1][yr] / fs_eff_splt[k][3][yr]
                        if fs_eff_splt[k][3][yr] != 0
                        else 1
                        for yr in aeo_years
                    }
                elif eff_capt:
                    fs_eff_splt_var_capt = {yr: 0 for yr in aeo_years}
                else:
            ...
            (
                mseg_out_break_adj[k]["baseline"][out_cz][out_bldg][out_eu][out_fuel_save],
                mseg_out_break_adj[k]["efficient"][out_cz][out_bldg][out_eu][out_fuel_save],
            ) = [
                # Remove adjusted baseline
                {
                    yr: mseg_out_break_adj[k]["baseline"][out_cz][out_bldg][out_eu][out_fuel_save][
                        yr
                    ]
                ...
                {
                    yr: mseg_out_break_adj[k]["efficient"][out_cz][out_bldg][out_eu][out_fuel_save][
                        yr
                    ]
                    - (eff_orig[yr] - eff_adj[yr]) * fs_eff_splt_var[yr]
                    for yr in aeo_years
                },
            ]
            ...
            if eff_capt and fs_eff_splt[k][-1]:
                ...
                # base fuel
                mseg_out_break_adj[k]["efficient-captured"][out_cz][out_bldg][out_eu][
                    out_fuel_save
                ] = {
                    yr: mseg_out_break_adj[k]["efficient-captured"][out_cz][out_bldg][out_eu][
                        out_fuel_save
                    ][yr]
                    - (eff_capt_orig[yr] - eff_capt_adj[yr]) * fs_eff_splt_var_capt[yr]
                    for yr in aeo_years
                }
                ...
                if eff_capt_env_frac:
                    mseg_out_break_adj[k]["efficient-captured-envelope"][out_cz][out_bldg][out_eu][
                        out_fuel_save
                    ] = {
                        yr: mseg_out_break_adj[k]["efficient-captured"][out_cz][out_bldg][out_eu][
                            out_fuel_save
            ...
            if k != "stock":
                ...
                mseg_out_break_adj[k]["savings"][out_cz][out_bldg][out_eu][out_fuel_save] = {
                    yr: mseg_out_break_adj[k]["savings"][out_cz][out_bldg][out_eu][out_fuel_save][
                        yr
                    ]
                    - (
                        (base_orig[yr] - base_adj[yr])
            ...
            mseg_out_break_adj[k]["efficient"][out_cz][out_bldg][out_eu][out_fuel_gain] = {
                yr: mseg_out_break_adj[k]["efficient"][out_cz][out_bldg][out_eu][out_fuel_gain][yr]
                - ((eff_orig[yr] - eff_adj[yr]) * (1 - fs_eff_splt_var[yr]))
                for yr in aeo_years
            }
            ...
            if eff_capt:
                mseg_out_break_adj[k]["efficient-captured"][out_cz][out_bldg][out_eu][
                    ...
                    - (eff_capt_orig[yr] - eff_capt_adj[yr]) * (1 - fs_eff_splt_var_capt[yr])
                    for yr in aeo_years
                }
                # Update efficient captured for envelope portion of pkg. if
            ...
            if k != "stock":
                ...
                mseg_out_break_adj[k]["savings"][out_cz][out_bldg][out_eu][out_fuel_gain] = {
                    yr: mseg_out_break_adj[k]["savings"][out_cz][out_bldg][out_eu][out_fuel_gain][
                        yr
                    ]
                    + ((eff_orig[yr] - eff_adj[yr]) * (1 - fs_eff_splt_var[yr]))
                    for yr in aeo_years
            ...
            if cost_brk_key:
                ...
                if "energy" in cost_brk_key:
                    ...
                    fs_eff_splt_cost = {
                        yr: (
                            (fs_eff_splt["energy cost"][0][yr] + fs_eff_splt["energy cost"][1][yr])
                            / fs_eff_splt["energy cost"][2][yr]
                        )
                        if fs_eff_splt["energy cost"][2][yr] != 0
                        else 1
                        for yr in aeo_years
                    }
                else:
                ...
                (
                    mseg_out_break_adj[cost_brk_key]["baseline"][out_cz][out_bldg][out_eu][
                        out_fuel_save
                    ],
                    mseg_out_break_adj[cost_brk_key]["efficient"][out_cz][out_bldg][out_eu][
                        out_fuel_save
                    ],
                    mseg_out_break_adj[cost_brk_key]["savings"][out_cz][out_bldg][out_eu][
                        out_fuel_save
                    ],
                ) = [
                    # Remove adjusted baseline
                    {
                        yr: mseg_out_break_adj[cost_brk_key]["baseline"][out_cz][out_bldg][out_eu][
                            out_fuel_save
                        ][yr]
                    ...
                    # fuel
                    {
                        yr: mseg_out_break_adj[cost_brk_key]["efficient"][out_cz][out_bldg][out_eu][
                            out_fuel_save
                        ][yr]
                        - (eff_cost_orig[yr] - eff_cost_adj[yr]) * fs_eff_splt_cost[yr]
                        for yr in aeo_years
                    },
                    ...
                    {
                        yr: mseg_out_break_adj[cost_brk_key]["savings"][out_cz][out_bldg][out_eu][
                            out_fuel_save
                        ][yr]
                        - (
                            (base_cost_orig[yr] - base_cost_adj[yr])
                ...
                (
                    ...
                    # fuel
                    {
                        ...
                        - ((eff_cost_orig[yr] - eff_cost_adj[yr]) * (1 - fs_eff_splt_cost[yr]))
                        for yr in aeo_years
                    },
                    # Adjusted efficient is added to the existing savings for
                    # baseline fuel (e.g., savings becomes less negative)
        ...
        # Fuel splits without fuel switching
            ...
            if k != "stock":
                ...
                mseg_out_break_adj[k]["savings"][out_cz][out_bldg][out_eu][out_fuel_save] = {
                    ...
                    - (save_orig[yr] - (base_adj[yr] - eff_adj[yr]))
                    for yr in aeo_years
                }
            ...
            if cost_brk_key:
                (
                    ...
                    # baseline/efficient
                    {
                        ...
                        - (save_cost_orig[yr] - (base_cost_adj[yr] - eff_cost_adj[yr]))
                        for yr in aeo_years
                    },
                ]
        # All other cases
            ...
            (
                ...
                # Remove adjusted efficient
                {
                    yr: mseg_out_break_adj[k]["efficient"][out_cz][out_bldg][out_eu][yr]
                    - (eff_orig[yr] - eff_adj[yr])
                    for yr in aeo_years
                },
            ]
            ...
            if eff_capt:
                mseg_out_break_adj[k]["efficient-captured"][out_cz][out_bldg][out_eu] = {
                    yr: mseg_out_break_adj[k]["efficient-captured"][out_cz][out_bldg][out_eu][yr]
                    - (eff_capt_orig[yr] - eff_capt_adj[yr])
                    for yr in aeo_years
                }
                # Update efficient captured for envelope portion of pkg. if
                # this is being tracked
                if eff_capt_env_frac:
                    mseg_out_break_adj[k]["efficient-captured-envelope"][out_cz][out_bldg][
                        out_eu
                    ] = {
                        yr: mseg_out_break_adj[k]["efficient-captured"][out_cz][out_bldg][out_eu][
                            yr
    ...
    def apply_pkg_benefits(self, msegs_meas, adopt_scheme, cm_key):
            ...
            for x in ["energy", "carbon"]:
                for cs in ["competed", "total"]:
                    ...
                    msegs_meas[x][cs]["efficient"] = {
                        key: eff[key] * (1 - energy_ben) for key in aeo_years
                    }
                    ...
                    msegs_meas["cost"][x][cs]["efficient"] = {
                        key: eff_c[key] * (1 - energy_ben) for key in aeo_years
                    }
    ...
    def merge_out_break(self, pkg_brk, meas_brk):
        ...
        for (k, i), (k2, i2) in zip(sorted(pkg_brk.items()), sorted(meas_brk.items())):
            if isinstance(i2, dict) and (sorted(list(i2.keys())) != aeo_years):
                ...
                if k == k2 and (isinstance(i, dict) == isinstance(i2, dict)):
                    ...
                    if len(i.keys()) == 0:
                        pkg_brk[k] = {yr: i2[yr] for yr in aeo_years}
                    else:
                        pkg_brk[k] = {yr: pkg_brk[k][yr] + i2[yr] for yr in aeo_years}
                else:
                    raise KeyError(
                        "Output data dicts to merge for ECM '"
                        + self.name
...
class ECMPrep:
    ...
    def prepare_measures(
        ...
        meas_update_objs = [
            Measure(base_dir, handyvars, handyfiles, opts_dict, **m) for m in measures
        ]
        ...
        if any(
            [
                isinstance(m.energy_efficiency, dict)
                and (
                    "undefined" in m.energy_efficiency.keys()
                    and m.energy_efficiency["undefined"] == "From EnergyPlus"
                )
        ...
        for m_ind, m in enumerate(meas_update_objs):
            ...
            try:
                m.fill_mkts(
                    msegs, msegs_cpl, convert_data, tsv_data, opts, ctrb_ms_pkg_prep, tsv_data_nonfs
                )
            except Exception:
                ECMPrepHelper.prep_error(m.name, handyvars, handyfiles)
                # Add measure index to removal list
        ...
        meas_update_objs = [
            m for m_ind, m in enumerate(meas_update_objs) if m_ind not in remove_inds
        ]
            ...
            try:
                ...
                for m in measures_to_add:
                    ...
                    if len(meas_summary_data) == 1:
                        ...
                        meas_obj = Measure(
                            base_dir, handyvars, handyfiles, opts_dict, **meas_summary_data[0]
                        )
                        ...
                        meas_obj.technology_type = meas_summary_data[0]["technology_type"]
                        ...
                        for adopt_scheme in handyvars.adopt_schemes_prep:
                            meas_obj.markets[adopt_scheme]["master_mseg"] = meas_summary_data[0][
                                "markets"
                ...
                measure_list_package_rmv = [x for x in measure_list_package if x.remove is True]

                # Warn user of no valid measures to package
                if len(measure_list_package_rmv) > 0:
                    warnings.warn(
                        "WARNING (CRITICAL): Package '"
                        + p["name"]
...
def filter_invalid_packages(
    ...
    if invalid_pkgs:
        invalid_pkgs_txt = fmt.format_console_list(invalid_pkgs)
        msg = (
            f"WARNING: Package(s) in package_ecms.json {package_opt_txt}have contributing ECMs"
...
def main(opts: argparse.NameSpace):  # noqa: F821
    ...
    meas_toprep_package_init = ECMPrepHelper.downselect_packages(
        meas_toprep_package_init, opts.ecm_packages
    )
    ...
    meas_toprep_indiv_names = ECMPrepHelper.retrieve_valid_ecms(
        meas_toprep_package_init, opts, handyfiles
    )
        ...
        if opts.pkg_env_sep is True:
            ...
            try:
                ...
                try:
                    ...
                    raise ValueError(
                        f"Error reading in '{handyfiles.ecm_prep_env_cf}': {str(e)}"
                    ) from None
                ecf.close()
                ...
                meas_summary_env_cf_indiv = [
                    m for m in meas_summary_env_cf if "contributing_ECMs" not in m.keys()
                ]
            ...
            try:
                ...
                try:
                    ...
                    raise ValueError(
                        f"Error reading in '{handyfiles.ecm_prep_env_cf_shapes}'"
                    ) from None
                ecf_ss.close()
            except FileNotFoundError:
                meas_shapes_env_cf = []
        else:
            meas_summary_env_cf, meas_shapes_env_cf = (None for n in range(2))
    else:
        ctrb_ms_pkg_all, pkg_copy_flag, meas_summary_env_cf, meas_shapes_env_cf = (
            None for n in range(4)
        )
    ...
    for mi in meas_toprep_indiv_names:
        ...
        try:
            ...
            match_in_prep_file = [
                y
                for y in meas_summary
                if ("contributing_ECMs" not in y.keys() and y["name"] == meas_dict["name"])
                or ("contributing_ECMs" in y.keys() and meas_dict["name"] in y["contributing_ECMs"])
            ]
            ...
            # on results
            compete_files = [
                x for x in handyfiles.ecm_compete_data.iterdir() if not x.name.startswith(".")
            ]
            ignore_opts = [
                "verbose",
                "yaml",
            ...
            update_indiv_ecm = (
                ecm_prep_exists
                and stat(handyfiles.indiv_ecms / mi).st_mtime > stat(handyfiles.ecm_prep).st_mtime
            ) or (
                len(match_in_prep_file) == 0
                or (
                    "(CF)" not in meas_dict["name"]
                    and all(
                        [
                            all([x["name"] != Path(y.stem).stem for y in compete_files])
                            for x in match_in_prep_file
                        ]
                    )
                )
                or (
                    ...
                    and not all(
                        [
                            all([m["usr_opts"][k] is False for k in m["usr_opts"].keys()])
                            for m in match_in_prep_file
                        ]
                    )
                )
                or
                ...
                (
                    not all(
                        [
                            all(
                                [
                                    m["usr_opts"].get(x) == vars(opts)[x]
                                    for x in [k for k in vars(opts).keys() if k not in ignore_opts]
                                ]
                            )
                            for m in match_in_prep_file
                        ]
            ...
            if update_indiv_ecm:
                ...
                if (
                    "backup_fuel_fraction" in meas_dict.keys()
                    and meas_dict["backup_fuel_fraction"] is not None
                    and opts.alt_regions != "State"
                ):
                    ...
                    msg = (
                        f"WARNING: Measure {meas_name} is a dual fuel measure, "
            ...
            meas_in_pkgs = any(
                [meas_dict["name"] in pkg["contributing_ECMs"] for pkg in meas_toprep_package_init]
            )
            if update_indiv_ecm or meas_in_pkgs:
                ...
                if meas_dict["tech_switch_to"] == "NA" and (
                    meas_dict["fuel_switch_to"] is not None
                    or (
                        any(
                            [
                                x in meas_dict["name"]
                                for x in ["LED", "solid state", "Solid State", "SSL"]
                            ]
                        )
                        or (
                            any([x in meas_dict["name"] for x in ["HP", "heat pump", "Heat Pump"]])
                            and (
                                meas_dict["technology"] is not None
                                and (
                                    any(
                                        [
                                            x in handyvars.resist_ht_wh_tech
                                            for x in meas_dict["technology"]
                                        ]
                                    )
                                    or meas_dict["technology"] in handyvars.resist_ht_wh_tech
                                )
                            )
                        )
                    )
                ...
                if opts.health_costs is True and "PHC" not in meas_dict["name"]:
                    ...
                    if (
                        (
                            (type(meas_dict["fuel_type"]) is not list)
                            and meas_dict["fuel_type"] not in ["electricity", "all"]
                        )
                        or (
                            (type(meas_dict["fuel_type"]) is list)
                            and all(
                                [x not in ["electricity", "all"] for x in meas_dict["fuel_type"]]
                            )
                        )
                    ) and meas_dict["fuel_switch_to"] != "electricity":
                        ...
                        for scn in handyvars.health_scn_names:
                            ...
                            # copied as well
                            pkgs_to_copy = [
                                x[0] for x in ctrb_ms_pkg_all if meas_dict["name"] in x[1]
                            ]
                ...
                if add_ref_meas:
                    ...
                    if meas_dict["fuel_switch_to"] is not None and opts.exog_hp_rates is False:
                        new_meas["fuel_switch_to"], new_meas["tech_switch_to"] = (
                ...
                if (
                    opts is not None
                    and opts.pkg_env_sep is True
                    and len(ctrb_ms_pkg_all) != 0
                    and (any([meas_dict["name"] in x[1] for x in ctrb_ms_pkg_all]))
                    and (
                        (
                            isinstance(meas_dict["end_use"], list)
                            and any([x in ["heating", "cooling"] for x in meas_dict["end_use"]])
                        )
                        or meas_dict["end_use"] in ["heating", "cooling"]
                    )
                    and (
                        not (
                            (
                                isinstance(meas_dict["technology"], list)
                                and all(
                                    [x in handyvars.demand_tech for x in meas_dict["technology"]]
                                )
                            )
                            or meas_dict["technology"] in handyvars.demand_tech
                        )
                    )
                ):
                    ...
                    pkgs_to_copy = [x[0] for x in ctrb_ms_pkg_all if meas_dict["name"] in x[1]]
                    # Add the package name, the package copy name,
            ...
            raise ValueError("Error reading in ECM '" + mi.stem + "': " + str(e)) from None
    ...
    meas_toprep_package_init, pkgs_skipped = ECMPrepHelper.filter_invalid_packages(
        meas_toprep_package_init, ecm_names, opts
    )
    ...
    supply_ecms = [
        ecm["name"]
        for ecm in meas_toprep_indiv
        if (
            not (
                (
                    isinstance(ecm["technology"], list)
                    and all([x in handyvars.demand_tech for x in ecm["technology"]])
                )
                or ecm["technology"] in handyvars.demand_tech
            )
        )
    ]
    ...
    excluded_ind_ecms = [
        ecm
        for ecm in opts.ecm_files_user
        if ecm in ctrb_ms and (opts.pkg_env_costs != "1" or ecm not in supply_ecms)
    ]
    include_ctrb_ecms = [
        ecm for ecm in opts.ecm_files_user if ecm in ctrb_ms and (ecm not in excluded_ind_ecms)
    ]
    run_setup = ECMPrepHelper.update_active_measures(
        run_setup, to_active=non_ctrb_ms + include_ctrb_ecms, to_inactive=excluded_ind_ecms
    )
    if excluded_ind_ecms:
        excluded_ind_ecms_txt = fmt.format_console_list(excluded_ind_ecms)
        warnings.warn(
            "The following ECMs were selected to be prepared, but due to their"
    ...
    for m in meas_toprep_package_init:
        ...
        name_mask = all(
            m["name"] != Path(y.stem).stem for y in handyfiles.ecm_compete_data.iterdir()
        )
        ...
        if len(m_exist) == 1:
            exst_ecms_mask = sorted(m["contributing_ECMs"]) != sorted(
                m_exist[0]["contributing_ECMs"]
            )
            ...
            exst_cost_red_mask = (
                m["benefits"]["cost reduction"] != m_exist[0]["benefits"]["cost reduction"]
            )
            exst_pkg_env_mask_1 = (
                opts is not None
                and opts.pkg_env_costs is not False
                and m_exist[0]["pkg_env_costs"] is False
            )
            exst_pkg_env_mask_2 = (
                opts is None
                or opts.pkg_env_costs is False
                and m_exist[0]["pkg_env_costs"] is not False
            )
        ...
        if (
            ...
            or name_mask
            or (
                (exst_ecms_mask or exst_engy_save_mask or exst_cost_red_mask)
                or (exst_pkg_env_mask_1 or exst_pkg_env_mask_2 or updated_contrib_mask)
            )
        ):
        ...
        # multiple previously prepared packages
            ...
            raise ValueError("Multiple existing ECM names match '" + m["name"] + "'")
    ...
    meas_toprep_indiv = [
        m
        for m in meas_toprep_indiv
        if any([m["name"] in x for x in [meas_toprep_indiv_nopkg, ctrb_ms_pkg_prep]])
    ]
    ...
    if len(meas_toprep_indiv) > 0 or len(meas_toprep_package) > 0:
        ...
        if opts.alt_regions in ["EMM", "State"] and (
            ...
            if (
                (opts.sect_shapes is True)
                and opts.tsv_metrics is False
                and all(
                    [
                        ("tsv_features" not in m.keys() or m["tsv_features"] is None)
                        for m in meas_toprep_indiv
                    ]
                )
            ):
                ...
                with gzip.GzipFile(tsv_c_zip, "r") as zip_ref_c:
                    tsv_cost_data = json.loads(zip_ref_c.read().decode("utf-8"))
                ...
                if handyfiles.tsv_cost_data_nonfs is not None:
                    ...
                    with gzip.GzipFile(tsv_c_nonfs_zip, "r") as zip_ref_nonfs_c:
                        tsv_cost_nonfs_data = json.loads(zip_ref_nonfs_c.read().decode("utf-8"))
                else:
                ...
                with gzip.GzipFile(tsv_cb_zip, "r") as zip_ref_cb:
                    tsv_carbon_data = json.loads(zip_ref_cb.read().decode("utf-8"))
                ...
                if handyfiles.tsv_carbon_data_nonfs is not None:
                    ...
                    with gzip.GzipFile(tsv_cb_nonfs_zip, "r") as zip_ref_nonfs_cb:
                        tsv_carbon_nonfs_data = json.loads(zip_ref_nonfs_cb.read().decode("utf-8"))
                else:
                ...
                tsv_carbon_yrmap = ECMPrepHelper.tsv_cost_carb_yrmap(
                    tsv_carbon_data["average carbon emissions rates"], handyvars.aeo_years
                )
                ...
                # factors for before grid decarbonization for non-fuel
                # switching measures
                if all([x is not None for x in [tsv_cost_nonfs_data, tsv_carbon_nonfs_data]]):
                    tsv_data_nonfs = {
                        "load": tsv_load_data,
                        "price": tsv_cost_nonfs_data,
                        "price_yr_map": tsv_cost_yrmap,
        ...
        if len(handyvars.skipped_ecms) != 0:
            meas_toprep_package, pkgs_skipped = ECMPrepHelper.filter_invalid_packages(
                meas_toprep_package, meas_check_list, opts
            )
        ...
        if len(handyvars.skipped_ecms) != 0:
            ...
            rel_share = (
                ...
                + str(
                    int(
                        (
                            len(handyvars.skipped_ecms)
                            / (len(meas_toprep_indiv) + len(meas_toprep_package))
                        )
                        * 100
                    )
                )
            ...
            warnings.warn(
                "WARNING: "
                + str(len(handyvars.skipped_ecms))
                + single_plural
        ...
        run_setup = ECMPrepHelper.update_active_measures(
            run_setup, to_skipped=handyvars.skipped_ecms
        )
        ...
        meas_prepped_compete, meas_prepped_summary, meas_prepped_shapes, meas_eff_fs_splt = (
            ECMPrepHelper.split_clean_data(meas_prepped_objs, handyvars.full_dat_out)
        )
        ...
        for m_i, m in enumerate([x for x in meas_prepped_summary]):
                ...
                if opts.sect_shapes is True:
                    ...
                    if len(m_ss.keys()) != 0:
                        ...
                        if m_ss["name"] in [x["name"] for x in meas_shapes]:
                            [x.update(m_ss) for x in meas_shapes if x["name"] == m_ss["name"]]
                        # Measure is new (add high-level data for measure)
                        else:
            ...
            # be written to a separate ecm_prep file
                ...
                if m["name"] in [x["name"] for x in meas_summary_env_cf]:
                    [x.update(m) for x in meas_summary_env_cf if x["name"] == m["name"]]
                # Measure is new (add high-level data for measure)
                else:
        ...
        for ind, m in enumerate(meas_prepped_objs):
            ...
            if "(CF)" not in m.name and (
                ...
                or (opts.pkg_env_costs == "1" and m.technology_type["primary"][0] == "supply")
            ):
                meas_file_name = m.name + ".pkl.gz"
                fs_dat = meas_eff_fs_splt[ind] if len(meas_eff_fs_splt[ind].keys()) != 0 else None
                write_tasks.append(
                    (
        ...
        if opts is not None and opts.pkg_env_sep is True and meas_summary_env_cf is not None:
