...
class ECMUtils:
    ...
    def prep_error(meas_name, handyvars, handyfiles):
        ...
        # fmt.verboseprint(opts.verbose, err_msg)
    ...
    def tsv_cost_carb_yrmap(tsv_data, aeo_years):
        ...
        # Set up a matrix mapping each AEO year to the years available in the TSV data
...
def main(opts: argparse.NameSpace):  # noqa: F821
    ...
    meas_toprep_package_init, invalid_pkgs = ECMUtils.filter_invalid_packages(
        meas_toprep_package_init, ecm_names)
    ...
    if len(meas_toprep_indiv) > 0 or len(meas_toprep_package) > 0:
        ...
        if len(handyvars.skipped_ecms) != 0:
            meas_toprep_package, pkgs_skipped = ECMUtils.filter_invalid_packages(
                meas_toprep_package, meas_check_list)
        ...
        if len(handyvars.skipped_ecms) != 0:
            ...
            if pkgs_skipped:
                pkgs_skipped_txt = fmt.format_console_list(pkgs_skipped)
                warnings.warn(
