...
def main(opts: argparse.NameSpace):  # noqa: F821
    ...
    meas_toprep_package_init = ECMPrepHelper.downselect_packages(meas_toprep_package_init,
                                                                 opts.ecm_packages)
    meas_summary = ECMPrepHelper.get_existing_measure_summary_data(handyfiles)
    meas_shapes = ECMPrepHelper.get_measure_savings_shapes(opts, handyfiles)
    ...
    if opts.pkg_env_sep is True:
        ...
        meas_summary_env_cf, \
            meas_shapes_env_cf, \
            meas_summary = ECMPrepHelper.get_pkg_env_sep_info(handyfiles, meas_summary)

    # Import all individual measure JSONs
    meas_toprep_indiv, \
        meas_toprep_indiv_nopkg, \
        pkg_copy_flag = ECMPrepHelper.get_indiv_measures(
            opts,
            handyfiles,
            handyvars,
            meas_summary,
    ...
    meas_toprep_package_init, _ = ECMPrepHelper.filter_invalid_packages(
        meas_toprep_package_init,
        ecm_names,
        opts)
    ...
    run_setup = ECMPrepHelper.update_run_setup(run_setup, opts, meas_toprep_package_init)
    meas_toprep_package, ctrb_ms_pkg_prep = ECMPrepHelper.get_measure_packages(opts,
                                                handyfiles,
                                                meas_toprep_package_init,
                                                meas_prepped_pkgs,
                                                meas_toprep_indiv_nopkg,
