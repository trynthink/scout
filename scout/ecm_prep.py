...
class Measure(object):
                    ...
                    try:
                        ...
                        if "incentives" in base_cpl["installed cost"].keys():
                            ...
                            cost_base_init = base_cpl["installed cost"][
                                "before incentives"]
                            # Ensure that non ref. case add-on measures (e.g., controls) are not
                            # assigned incentives that are meant for equipment replacements
                            # to which the add-ons are attached. Also do not proceed
                            # w/ incentives data for measures with relative performance
                            # units as these cannot be linked back to EIA
                            # incentives levels
                            if (self.measure_type != "add-on" or ref_addon) and \
                ...
                # explicit check of baseline units needed in this case)
                    ...
                    if cost_incentives:
                        ...
                        if all([x is not None for x in [cnv_b, cnv_m]]):
                            ...
                            meas_incent_flag = self.apply_incentives(
                                ...
                                incent_mod_swtch, mskeys, meas_incent_flag)
                        else:
    ...
    def apply_incentives(
            ...
            incent_mod_base, incent_mod_swtch, mskeys, meas_incent_flag):
        ...
        for seg in seg_loop:
            ...
            for yr_str in self.handyvars.aeo_years:
                ...
                base_val_yr_max_init, meas_val_yr_max_init = [
                    max(x) if len(x) >= 1 else False for x in [base_val_yr, meas_val_yr]]
                ...
                for val_adjs_i, val_adjs in enumerate([base_val_yr_max_init, meas_val_yr_max_init]):
    ...
    def add_elec_infr_costs(self, cost_meas, mskeys, elec_infr_flag, opts):
        ...
        if self.add_elec_infr_cost and "panel" in self.added_elec_infrastructure.keys() and \
                self.added_elec_infrastructure["panel"] is True:
        ...
        elif self.add_elec_infr_cost and "panel" in self.added_elec_infrastructure.keys() and \
                self.added_elec_infrastructure["panel"] is False:
            add_pnl_cost = 0
        ...
        if self.add_elec_infr_cost and "240V circuit" in self.added_elec_infrastructure.keys() and \
                self.added_elec_infrastructure["240V circuit"] is True:
        ...
        elif self.add_elec_infr_cost and "240V circuit" in self.added_elec_infrastructure.keys() \
                and self.added_elec_infrastructure["240V circuit"] is False:
            add_crct_cost = 0
        # User has not specified one way or another whether measure incurs panel upgrade
        else:
