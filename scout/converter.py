...
class UsefulVars(object):
    ...
    def __init__(self):
        ...
        self.state_baseline_files = list(
            Path(fp.CONVERT_DATA).glob("EIA_State_Emissions_Prices_Baselines_*.csv")
        )
...
class EIAQueries(object):
    ...
    def __init__(self, yr, scen):
        ...
        self.data_names_gasprice = ["ng_res_price", "ng_com_price"]
...
def api_query(api_key, query_str, expect_table_id):
    ...
    try:
        ...
        data = [
            [str(x["period"]), float(x["value"])] for x in data if x["tableId"] == expect_table_id
        ]
    except KeyError:
...
def data_processor(data):
    ...
    if str(aeo_min) not in years:
        ...
        new_years = np.array([aeo_min + np.timedelta64(i, "Y") for i in range(diff)])
        ...
        years = np.array([np.datetime_as_string(year, unit="Y") for year in years])
...
def updater(conv, api_key, aeo_yr, scen_elec, scen_gas, web):
    ...
    z_foss, yrs_foss = data_getter(
        api_key, dq_foss.data_names, dq_foss.query, dq_foss.data_table_ids
    )
    z_elec, yrs_elec = data_getter(
        api_key, dq_elec.data_names, dq_elec.query, dq_elec.data_table_ids
    )
    ...
    if conv["site-source calculation method"] == "captured energy":
        renew_factor = (
            z_elec["elec_renew_hydro"]
            + z_elec["elec_renew_geothermal"]
            + z_elec["elec_renew_wind"]
            + z_elec["elec_renew_solar_thermal"]
            + z_elec["elec_renew_solar_pv"]
        ) / (z_elec["elec_tot_energy_site"] + z_elec["elec_tot_energy_loss"])
    ...
    try:
        ss_conv = (z_elec["elec_tot_energy_site"] + z_elec["elec_tot_energy_loss"]) / z_elec[
            "elec_tot_energy_site"
        ]
        for idx, year in enumerate(yrs_elec):
            conv["electricity"]["site to source conversion"]["data"][year] = round(
                ss_conv[idx] * capnrg[idx], 6
            )
    except KeyError:
        print(
    ...
    try:
        co2_res_ints = z_elec["elec_res_co2"] / (
            z_elec["elec_res_energy_site"] + z_elec["elec_res_energy_loss"]
        )
        for idx, year in enumerate(yrs_elec):
            conv["electricity"]["CO2 intensity"]["data"]["residential"][year] = round(
                co2_res_ints[idx] / capnrg[idx], 6
            )
    except KeyError:
        print(
    ...
    try:
        co2_com_ints = z_elec["elec_com_co2"] / (
            z_elec["elec_com_energy_site"] + z_elec["elec_com_energy_loss"]
        )
        for idx, year in enumerate(yrs_elec):
            conv["electricity"]["CO2 intensity"]["data"]["commercial"][year] = round(
                co2_com_ints[idx] / capnrg[idx], 6
            )
    except KeyError:
        print(
    ...
    try:
        ...
        for idx, year in enumerate(yrs_foss):
            conv["natural gas"]["CO2 intensity"]["data"]["residential"][year] = round(
                co2_res_ng_ints[idx], 6
            )
    except KeyError:
        print(
            "\nDue to failed data retrieval from the API, residential "
    ...
    try:
        ...
        for idx, year in enumerate(yrs_foss):
            conv["natural gas"]["CO2 intensity"]["data"]["commercial"][year] = round(
                co2_com_ng_ints[idx], 6
            )
    except KeyError:
        print(
            "\nDue to failed data retrieval from the API, commercial "
    ...
    try:
        ...
        for idx, year in enumerate(yrs_foss):
            conv["other"]["CO2 intensity"]["data"]["residential"][year] = round(
                co2_res_ot_ints[idx], 6
            )
    except KeyError:
        if web:
            print(
    ...
    try:
        ...
        for idx, year in enumerate(yrs_foss):
            conv["other"]["CO2 intensity"]["data"]["commercial"][year] = round(
                co2_com_ot_ints[idx], 6
            )
    except KeyError:
        if web:
            print(
    ...
    try:
        for idx, year in enumerate(yrs_elec):
            conv["electricity"]["price"]["data"]["residential"][year] = round(
                z_elec["elec_res_price"][idx] / (ss_conv[idx] * capnrg[idx]), 6
            )
    except KeyError:
        print(
            "\nDue to failed data retrieval from the API, residential "
    ...
    try:
        for idx, year in enumerate(yrs_elec):
            conv["electricity"]["price"]["data"]["commercial"][year] = round(
                z_elec["elec_com_price"][idx] / (ss_conv[idx] * capnrg[idx]), 6
            )
    except KeyError:
        print(
            "\nDue to failed data retrieval from the API, commercial "
    ...
    try:
        for idx, year in enumerate(yrs_foss):
            conv["natural gas"]["price"]["data"]["residential"][year] = round(
                z_foss["ng_res_price"][idx], 6
            )
    except KeyError:
        print(
            "\nDue to failed data retrieval from the API, residential "
    ...
    try:
        for idx, year in enumerate(yrs_foss):
            conv["natural gas"]["price"]["data"]["commercial"][year] = round(
                z_foss["ng_com_price"][idx], 6
            )
    except KeyError:
        print(
            "\nDue to failed data retrieval from the API, commercial "
    ...
    try:
        for idx, year in enumerate(yrs_foss):
            conv["propane"]["price"]["data"]["residential"][year] = round(
                z_foss["lpg_res_price"][idx], 6
            )
    except KeyError:
        if not web:
            print(
    ...
    try:
        for idx, year in enumerate(yrs_foss):
            conv["propane"]["price"]["data"]["commercial"][year] = round(
                z_foss["lpg_com_price"][idx], 6
            )
    except KeyError:
        if not web:
            print(
    ...
    try:
        for idx, year in enumerate(yrs_foss):
            conv["distillate"]["price"]["data"]["residential"][year] = round(
                z_foss["distl_res_price"][idx], 6
            )
    except KeyError:
        if not web:
            print(
    ...
    try:
        for idx, year in enumerate(yrs_foss):
            conv["distillate"]["price"]["data"]["commercial"][year] = round(
                z_foss["distl_com_price"][idx], 6
            )
    except KeyError:
        if not web:
            print(
    ...
    try:
        ...
        for idx, year in enumerate(yrs_foss):
            conv["other"]["price"]["data"]["residential"][year] = round(res_other_price[idx], 6)
    except KeyError:
        if web:
            print(
                "\nDue to failed data retrieval from the API, residential "
    ...
    try:
        ...
        com_other_price = (
            z_foss["lpg_com_price"] * z_foss["lpg_com_energy"] / denom
            + z_foss["distl_com_price"] * z_foss["distl_com_energy"] / denom
            + z_foss["rsid_com_price"] * z_foss["rsid_com_energy"] / denom
        )
        for idx, year in enumerate(yrs_foss):
            conv["other"]["price"]["data"]["commercial"][year] = round(com_other_price[idx], 6)
    except KeyError:
        if web:
            print(
                "\nDue to failed data retrieval from the API, commercial "
...
def updater_emm(conv, api_key, aeo_yr, scen_elec):
    ...
    for key, value in ValidQueries().regions_dict.items():
        ...
        try:
            co2_ints = (
                (z["elec_co2_total_" + key].astype("float") * conv_factor)
                /
                # account for T&D losses by multiplying sales by 5%
                (z["elec_sales_total_" + key].astype("float") * 1.05)
            )
            for idx, year in enumerate(yrs):
                conv["CO2 intensity of electricity"]["data"][value][year] = round(co2_ints[idx], 6)
            # Ensure years are ordered chronologically
            conv["CO2 intensity of electricity"]["data"][value] = OrderedDict(
                sorted(conv["CO2 intensity of electricity"]["data"][value].items())
            )
        except KeyError:
            print(
        ...
        try:
            for idx, year in enumerate(yrs):
                conv["End-use electricity price"]["data"]["residential"][value][year] = round(
                    (z["elec_enduse_price_res_" + key][idx].astype("float") / 100), 6
                )
            # Ensure years are ordered chronologically
            conv["End-use electricity price"]["data"]["residential"][value] = OrderedDict(
                sorted(conv["End-use electricity price"]["data"]["residential"][value].items())
            )
        ...
        try:
            for idx, year in enumerate(yrs):
                conv["End-use electricity price"]["data"]["commercial"][value][year] = round(
                    (z["elec_enduse_price_com_" + key][idx].astype("float") / 100), 6
                )
            # Ensure years are ordered chronologically
            conv["End-use electricity price"]["data"]["commercial"][value] = OrderedDict(
                sorted(conv["End-use electricity price"]["data"]["commercial"][value].items())
            )
...
def updater_state(conv_emm, aeo_min, conv_gas):
        ...
        while True:
            try:
                ...
                if 1 <= file_index <= len(UsefulVars().state_baseline_files):
                    state_baseline_data = UsefulVars().state_baseline_files[file_index - 1]
                    break
                else:
    ...
    emm_state_map = (
        pd.read_csv(UsefulVars().emm_state_map, delimiter="\t")
        .dropna(axis=0)
        .set_index("EMM")
        .drop(["AK", "HI"], axis=1)
        .sort_index()
    ...
    emm_price_res = pd.DataFrame.from_dict(
        conv_emm["End-use electricity price"]["data"]["residential"], orient="index"
    )
    emm_price_com = pd.DataFrame.from_dict(
        conv_emm["End-use electricity price"]["data"]["commercial"], orient="index"
    )
    ...
    # Divide each year in dataframe by base year
    emm_price_res_ratios = emm_price_res[emm_price_res_cols].div(emm_price_res[aeo_min], axis=0)
    emm_price_com_ratios = emm_price_com[emm_price_com_cols].div(emm_price_com[aeo_min], axis=0)
    # Re-insert base year into new dataframe
    ...
    state_co2 = {
        state: {
            yr: np.average(emm_co2_ratios.loc[:, yr], weights=emm_state_map.loc[:, state])
            for yr in emm_co2_ratios.columns
        }
        for state in emm_state_map.columns
    }
    state_co2_proj = {
        state: {
            yr: state_co2[state][yr] * state_baselines.loc[state, "Emissions Rate (Mt/TWh)"]
            for yr in emm_co2_ratios.columns
        }
        for state in state_co2.keys()
    }

    # Prices - residential electric and gas
    state_elec_price_res = {
        state: {
            yr: np.average(emm_price_res_ratios.loc[:, yr], weights=emm_state_map.loc[:, state])
            for yr in emm_price_res_ratios.columns
        }
        for state in emm_state_map.columns
    }
    state_elec_price_res_proj = {
        state: {
            ...
            * state_baselines.loc[state, "Residential Electricity Price ($/kWh)"]
            for yr in emm_price_res_ratios.columns
        }
        for state in state_elec_price_res.keys()
    }
    # Ensure that gas price trends do not apply to years before/leading up to anchor year
    state_gas_price_res = {
        yr: (conv_gas["residential"][yr] / conv_gas["residential"][anchor_yr])
        if int(yr) > int(anchor_yr)
        else 1
        for yr in conv_gas["residential"].keys()
    }
    state_gas_price_res_proj = {
        state: {
            yr: state_gas_price_res[yr]
            * state_baselines.loc[state, "Residential Gas Price ($/MCF)"]
            * mcf_mmbtu
            for yr in state_gas_price_res.keys()
        }
        for state in state_baselines.index
    }

    # Prices - commercial electric and gas
    state_elec_price_com = {
        state: {
            yr: np.average(emm_price_com_ratios.loc[:, yr], weights=emm_state_map.loc[:, state])
            for yr in emm_price_com_ratios.columns
        }
        for state in emm_state_map.columns
    }
    state_elec_price_com_proj = {
        state: {
            ...
            * state_baselines.loc[state, "Commercial Electricity Price ($/kWh)"]
            for yr in emm_price_com_ratios.columns
        }
        for state in state_elec_price_com.keys()
    }
    # Ensure that gas price trends do not apply to years before/leading up to anchor year
    state_gas_price_com = {
        yr: (conv_gas["commercial"][yr] / conv_gas["commercial"][anchor_yr])
        if int(yr) > int(anchor_yr)
        else 1
        for yr in conv_gas["commercial"].keys()
    }
    state_gas_price_com_proj = {
        state: {
            yr: state_gas_price_com[yr]
            * state_baselines.loc[state, "Commercial Gas Price ($/MCF)"]
            * mcf_mmbtu
            for yr in state_gas_price_com.keys()
        }
        for state in state_baselines.index
    }
    ...
    conv_emm["End-use electricity price"]["data"]["residential"] = state_elec_price_res_proj
    conv_emm["End-use electricity price"]["data"]["commercial"] = state_elec_price_com_proj
    ...
    conv_emm["End-use gas price"] = {
        "data": {"residential": state_gas_price_res_proj, "commercial": state_gas_price_com_proj},
        "source": (
...
def aeo_min_extract():
    ...
    with open(UsefulVars().metadata, "r") as aeo_yrs:
        try:
            ...
            raise ValueError(
                "Error reading in '" + UsefulVars().metadata + "': " + str(e)
            ) from None
...
def main():
    ...
    parser.add_argument("-f", required=True, help="Name of file to be updated, without the path.")
    parser.add_argument("-y", help="Desired AEO publication year")
    parser.add_argument("-s_e", help="Desired AEO electricity scenario in given year")
    parser.add_argument("-s_g", help="Desired AEO fossil scenario in given year")
    ...
    if geography == "national":
        ...
        fuels = ["CO2 price", "electricity", "natural gas", "propane", "distillate"]
        ...
        for fuel in fuels:
            for metric in metrics:
                for bldg in bldgs:
                    ...
                    for year_remove in list(
                        conv[fuels[1]]["site to source conversion"]["data"].keys()
                    ):
                        if int(year_remove) < aeo_min:
                            conv[fuels[1]]["site to source conversion"]["data"].pop(year_remove)
                    try:
                        for year_remove in list(conv[fuel][metric]["data"][bldg].keys()):
                            if int(year_remove) < aeo_min:
                                conv[fuel][metric]["data"][bldg].pop(year_remove)
                    except KeyError:
                        pass
        ...
        for bldg in bldgs:
            for reg in list(conv[metrics[0]]["data"].keys()):
                try:
                    for year_remove in list(conv[metrics[0]]["data"][reg].keys()):
                        if int(year_remove) < aeo_min:
                            conv[metrics[0]]["data"][reg].pop(year_remove)
                except KeyError:
                    for year_remove in list(conv[metrics[1]]["data"][bldg][reg].keys()):
                        if int(year_remove) < aeo_min:
                            conv[metrics[1]]["data"][bldg][reg].pop(year_remove)
        ...
        print("\nUpdating EMM region CO2 emissions and prices conversion factors.")
