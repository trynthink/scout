...
tech_eia_nonlt = {
    "ASHP": ["EIA_EQUIP", "ELEC_HP", "ELEC_HP2", "ELEC_HP4", "COP"],
    "GSHP": ["EIA_EQUIP", "GEO_HP", "GEO_HP2", "GEO_HP4", ["COP", "EER"]],
    "NGHP": ["EIA_EQUIP", "NG_HP", "NG_HP2", "NG_HP2", "COP"],
    "boiler (NG)": ["EIA_EQUIP", "NG_RAD", "NG_RAD2", "NG_RAD4", "AFUE"],
    "boiler (distillate)": ["EIA_EQUIP", "DIST_RAD", "DIST_RAD1", "DIST_RAD4", "AFUE"],
    "resistance heat": ["EIA_EQUIP", "ELEC_RAD", "ELEC_RAD2", "ELEC_RAD2", "AFUE"],
    "furnace (distillate)": ["EIA_EQUIP", "DIST_FA", "DIST_FA2", "DIST_FA4", "AFUE"],
    "furnace (kerosene)": ["EIA_EQUIP", "KERO_FA", "KERO_FA2", "KERO_FA4", "AFUE"],
    "furnace (LPG)": ["EIA_EQUIP", "LPG_FA", "LPG_FA2", "LPG_FA4", "AFUE"],
    "furnace (NG)": ["EIA_EQUIP", "NG_FA", "NG_FA2", "NG_FA4", "AFUE"],
    "stove (wood)": ["EIA_EQUIP", "WOOD_HT", "WOOD_HT2", "WOOD_HT4", "HHV"],
    "solar WH": ["EIA_EQUIP", "SOLAR_WH", "SOLAR_WH2", "SOLAR_WH2", "SEF"],
    ...
    "central AC": ["EIA_EQUIP", "CENT_AIR", "CENT_AIR2", "CENT_AIR4", "COP"],
    "room AC": ["EIA_EQUIP", "ROOM_AIR", "ROOM_AIR3", "ROOM_AIR4", "COP"],
    "clothes washing": [
        "EIA_EQUIP",
        "CL_WASH",
    ...
    "dishwasher": ["EIA_EQUIP", "DS_WASH", "DS_WASH3", "DS_WASH4", "cycle/kWh"],
    "water heating": [
        "EIA_EQUIP",
    ...
    "freezers": ["EIA_EQUIP", "FREZ", ["FREZ_C1", "FREZ_U2"], ["FREZ_C4", "FREZ_U4"], "kWh/yr"],
}
...
#   performance units (* Note: cost units are addressed separately)]
tech_eia_lt = {
    "linear fluorescent (T-12)": ["EIA_LT", ["LFL", "T12"], "lm/W"],
    ...
    "reflector (incandescent)": ["EIA_LT", ["REF", "INC"], "lm/W"],
    ...
    "external (high pressure sodium)": ["EIA_LT", ["EXT", "HPS"], "lm/W"],
    ...
    for key, item in json_dict.items():
        ...
        # list for the current location and update the data in the dict
            ...
            if key_list[1] in mseg.bldgtypedict.keys():
                ...
                if leaf_node_keys[-1] not in ["total square footage", "new homes", "total homes"]:
                    data_dict = list_generator_techdata(
                        eia_nlt_cp,
                        eia_nlt_l,
                        eia_lt,
                        tech_eia_nonlt,
...
def list_generator_techdata(
    ...
    if (
        end_use not in mseg_enduse_translate.keys()
        and end_use not in ["lighting", "secondary heating", "other"]
        and end_use not in tech_non_eia
    ):
        end_use = None
    ...
    if all([x is not None for x in [cdiv, end_use, tech_dict]]):
        ...
        if filter_info[0] == "EIA_EQUIP":
            ...
            for idx, row in enumerate(eia_nlt_cp):
                ...
                if (row["CDIV"] == mseg_cdiv_translate[cdiv] or row["CDIV"] == 11) and row[
                    "ENDUSE"
                    ...
                    if tech_dict_key in ["refrigeration", "freezers", "clothes washing"]:
                        ...
                        # (chest, upright and top-loading, front-loading)
                            ...
                            tech_configs = 2
                            typ_filter_name = (
                                "(" + filter_info[2][0] + "|" + filter_info[2][1] + ")"
                            )
                            best_filter_name = (
                                "(" + filter_info[3][0] + "|" + filter_info[3][1] + ")"
                            )
                    ...
                    # row
                    match_typ = re.search(comparefrom_typ, compareto, re.IGNORECASE)
                    # If there is no match for the "typical" performance/cost/
                    # consumer choice case, check for a "best" case performance
                    # and cost match
                    match_best = re.search(comparefrom_best, compareto, re.IGNORECASE)
                    ...
                    if (not match_typ and not match_best) and typ_row_found and not best_row_found:
                        match_list_incent_perfcost.append(row)
            ...
            if all(
                ...
                match_list_typ_perfcost, match_list_best_perfcost, match_list_incent_perfcost = [
                    numpy.array(x, dtype=eia_nlt_cp.dtype)
                    for x in [
                        match_list_typ_perfcost,
                        match_list_best_perfcost,
                        match_list_incent_perfcost,
                ...
                raise ValueError(
                    "No EIA performance/cost data match for" + " non-lighting technology!"
                )
            ...
            for idx, row in enumerate(eia_nlt_l):
                ...
                if row["ENDUSE"] == mseg_enduse_translate[end_use]:
                    ...
                    if match:
                        ...
                        [life_avg, life_range] = [
                            dict.fromkeys(project_dict.keys(), n)
                            for n in [life_avg_set, life_range_set]
                        ]

            # If there were no matches for lifetime data on the technology,
            # yield an error
            if life_match_ct == 0:
                raise ValueError("No EIA lifetime data match for non-lighting technology!")
        ...
        # tech_eia_lt dict
            ...
            if len(match_list) > 0:
                ...
                print(filter_info)
                raise ValueError(
                    "No performance/cost/lifetime data match for" + " lighting technology!"
                )
        ...
        # directly specified in tech_non_eia above
            ...
            [perf_typ, perf_best, cost_typ, cost_best, life_avg, life_range] = [
                dict.fromkeys(project_dict.keys(), n)
                for n in [
                    filter_info[1][0],
                    filter_info[1][1],
        ...
        data_dict["installed cost"] = {
            ...
            "incentives": {"by performance tier": incentives, "performance units": perf_units},
        }
        ...
        data_dict["consumer choice"]["competed market share"]["parameters"]["typical"]["b1"] = b1
        data_dict["consumer choice"]["competed market share"]["parameters"]["typical"]["b2"] = b2
        data_dict["consumer choice"]["competed market share"]["parameters"]["best"]["b1"] = b1_best
        data_dict["consumer choice"]["competed market share"]["parameters"]["best"]["b2"] = b2_best
        data_dict["consumer choice"]["competed market share"]["source"] = tech_choice_source
    else:
        data_dict = 0
        ...
        for ind, x in enumerate(unique_yrs):
            ...
            if len(match_list_inds[0]) == ntypes:
                ...
                match_list_new.append(
                    (
                        unique_yrs[ind],
                        numpy.average(match_list[match_list_inds]["BASE_EFF"]),
                        numpy.average(match_list[match_list_inds]["INST_COST"]),
                        numpy.average(match_list[match_list_inds]["RETAIL_COST"]),
                        numpy.average(match_list[match_list_inds]["EFF_CHOICE_P1"]),
                        numpy.average(match_list[match_list_inds]["EFF_CHOICE_P2"]),
                    )
                )
            else:
...
def fill_years_nlt_incent(match_list, project_dict, tech_dict_key):
    ...
    fed_exist = stitch(match_list, project_dict, "FD_REPL_SUB", incent_flag=True)
    ...
    nf_exist = stitch(match_list, project_dict, "NF_REPL_SUB", incent_flag=True)
    # Sum federal/non-federal new incentives
    incent_new = {yr: [x + y for x, y in zip(fed_new[yr], nf_new[yr])] for yr in fed_new.keys()}
    # Sum federal/non-federal existing incentives
    incent_exist = {
        yr: [x + y for x, y in zip(fed_exist[yr], nf_exist[yr])] for yr in fed_exist.keys()
    }
    # Combine everything together in an output dict
    output = {
        "new": {yr: [[x, y] for x, y in zip(perf[yr], incent_new[yr])] for yr in perf.keys()},
        "existing": {
            yr: [[x, y] for x, y in zip(perf[yr], incent_exist[yr])] for yr in perf.keys()
        },
    }
...
def fill_years_lt_incent(match_list, project_dict):
    ...
    for ind_i, ee_i_col in enumerate(lgt_ee_incent_cols):
        ...
        if ind_i == 0:
            incent_sums = stitch(match_list, project_dict, ee_i_col, incent_flag=True)
        # Add to dict with subsequent incentives columns
        else:
            incent_sums_add = stitch(match_list, project_dict, ee_i_col, incent_flag=True)
            # Loop through all years and add incentives for the year
            for yr in project_dict.keys():
                incent_sums[yr] = [x + y for x, y in zip(incent_sums[yr], incent_sums_add[yr])]
    # Pull together performance and incentives information into output dict
    output = {yr: [[x, y] for x, y in zip(perf[yr], incent_sums[yr])] for yr in perf.keys()}
            ...
            # with a "START_EQUIP_YR" that is closest to the looped year
                ...
                if incent_flag is True:
                    array_close_ind = numpy.where(
                        (
                            abs(int(yr) - input_array["START_EQUIP_YR"].astype(int))
                            == min(abs(int(yr) - input_array["START_EQUIP_YR"].astype(int)))
                        )
                        & (input_array["NAME"] == t_name)
                    )[0]
                else:
                    array_close_ind = numpy.where(
                        abs(int(yr) - input_array["START_EQUIP_YR"].astype(int))
                ...
                # case (in which this is expected) and otherwise yield error
                    ...
                    if len(array_close_ind) > 1 and (
                        ...
                        if incent_flag is True:
                            output_dict[yr].append(float(input_array[array_close_ind][0][col_name]))
                        else:
                            output_dict[yr] = float(input_array[array_close_ind][0][col_name])
                    else:
                        raise ValueError("Multiple identical years in filtered array!")
...
def main():
    ...
    eia_nlt_cp = numpy.genfromtxt(
        eiadata.r_nlt_costperf,
        names=r_nlt_cp_names,
        dtype=None,
        comments=None,
