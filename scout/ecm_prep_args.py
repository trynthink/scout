...
def translate_inputs(opts: argparse.NameSpace) -> argparse.NameSpace:  # noqa: F821
    ...
    if missing_ecms:
        msg = (
            "WARNING: The following ECMs specified with the `ecm_files` argument are not"
    ...
    ecm_file_matches = [
        file
        for file in ecm_dir_files
        if any(re.match(pattern, file) for pattern in opts.ecm_files_regex)
    ]
    if opts.ecm_files_regex and not ecm_file_matches:
        msg = (
            "WARNING: A regular expression was specified with `ecm_files_regex`, but it does"
    ...
    if (opts.alt_regions not in ["EMM", "State"]) and any(
        [x is not False for x in [opts.tsv_metrics, opts.sect_shapes, opts.grid_decarb]]
    ):
        opts.alt_regions = "EMM"
    ...
    sys_shape = {
        ...
        "net renewable high renewables": "4",
    }.get(opts.tsv_sys_shape_case, "0")
    ...
    if (opts.bps is not None or opts.codes is not None) and (
        opts.split_fuel is False
        or (
            not opts.detail_brkout
            or (
                "all" not in opts.detail_brkout
                and (
                    "regions" not in opts.detail_brkout
                    or (all([x not in opts.detail_brkout for x in ["buildings", "codes/bps"]]))
                )
            )
        )
    ):
