...
class IndexLists(object):
    ...
    def __init__(self):
        self.climate_zone = ["AIA_CZ1", "AIA_CZ2", "AIA_CZ3", "AIA_CZ4", "AIA_CZ5"]
...
def ecm_kw_regex_select(ecm_names_list, list_of_match_str):
    ...
    if list_of_match_str:
        ...
        nom_regex = re.compile(r"(?:%s)" % "|".join(list_of_match_str), re.IGNORECASE).search
        # Construct the inverse regex search pattern from list of strings given
        nom_regex_inv = re.compile(
            r"(?:%s)" % "|".join([x.strip("! ") for x in list_of_match_str if "!" in x]),
            re.IGNORECASE,
        ).search
        ...
        if len(matches_inv) > 0:
            matches.extend(
                [x for x in ecm_names_list if (x not in matches_inv and x not in matches)]
            )
...
def fix_ecm_move_conflicts(conflict_ecm_list, move_order_text):
    ...
    while True:
        ...
        try:
            move_ecm_numbers = [int(x.strip()) for x in selections.split(",") if x.strip()]
        ...
        # their desired entries from the list again
            ...
            input("An invalid non-numeric entry was given. Please try again: ")
    ...
    if move_ecm_numbers:
        while max(move_ecm_numbers) > len(conflict_ecm_list):
            selections = input("An out of range number was given. Please try again: ")
            move_ecm_numbers = [int(x.strip()) for x in selections.split(",") if x.strip()]

    # Create a list of all of the ECMs that are going to be kept
    # in place/not moved
    keep_in_place = [
        conflict_ecm_list[i - 1]
...
def ecm_list_kw_update(active_list, inactive_list):
    ...
    print(
        "You can use short strings to quickly move groups of ECMs "
    ...
    print(
        "You may input more than one search term. Please separate "
        "each term with a comma, for example: Efficient, 20%\n"
    )
    print(
        'Adding a "!" character before any of your terms will invert the '
    ...
    print(
        "Search terms are not case-sensitive. Search terms should "
        "not be enclosed by any quotes or other special characters.\n"
    )
    print(
        "If you are unsure of the ECMs currently listed as active "
    ...
    to_inactive_prompt_text = (
        "Enter ECM name keywords separated by commas to move ECMs active -> inactive: "
    )
    to_active_prompt_text = (
        "Enter ECM name keywords separated by commas to move ECMs inactive -> active: "
    )
    ...
    move_to_inactive, active_list = ecm_kw_regex_select(active_list, kw_to_inactive)
    move_to_active, inactive_list = ecm_kw_regex_select(inactive_list, kw_to_active)
    ...
    if kw_to_active and move_to_inactive:
        back_to_active, _ = ecm_kw_regex_select(move_to_inactive, kw_to_active)
        ...
        if back_to_active:
            ...
            keep_active = fix_ecm_move_conflicts(back_to_active, "active to inactive")
            ...
            move_to_inactive = [ecm for ecm in move_to_inactive if ecm not in keep_active]
    ...
    if kw_to_inactive and move_to_active:
        back_to_inactive, _ = ecm_kw_regex_select(move_to_active, kw_to_inactive)
        ...
        if back_to_inactive:
            # Get user input on how to move the ECMs
            keep_inactive = fix_ecm_move_conflicts(back_to_inactive, "inactive to active")
            ...
            move_to_active = [ecm for ecm in move_to_active if ecm not in keep_inactive]
...
def user_input_baseline_market_filters(market_cat):
    ...
    print(
        ...
        "include all of the",
        selection_name,
        'simply type "enter" or "return" to skip.\n',
    )
    ...
    while True:
        ...
        # no extraneous trailing or leading spaces
        try:
            user_select_keep_numbers = [int(x.strip()) for x in selections.split(",") if x.strip()]
        # Handle the exception if a non-integer entry is passed
        # to the input by requesting the user attempt to enter
        # their desired entries from the list again
        except ValueError:
...
def ecm_list_market_update(ecm_folder, active_list, inactive_list, filters, market_cat):
...
def main():
    ...
    active, inactive = ecm_list_kw_update(setup_json["active"], setup_json["inactive"])
    ...
    for market in ref.market_filters:
        ...
        if user_filter_choices:
            active, inactive = ecm_list_market_update(
                ref.ecm_folder_location, active, inactive, user_filter_choices, market
            )
