...
class EIAFiles(object):
    ...
    def res_gsl_lt_update(self):
        ...
        with open(self.r_lgt_in, "r", encoding="latin1") as f_ltin:
            modified = [
                line.replace("HAL", "INC") if re.search(r"(?<=GSL\s)HAL", line) else line
                for line in f_ltin
            ]
    ...
    def generate_rsclass(self):
        ...
        with open(self.r_class, "w+", encoding="utf-8") as f:
            ...
            f.write("\t".join(str(rsclass.cell(row=20, column=c).value) for c in cols) + "\n")
            # names row
            f.write("\t".join(rmt.r_nlt_l_names) + "\n")
            # data rows
            for row_num in range(22, 52):
                f.write(
                    "\t".join(str(rsclass.cell(row=row_num, column=c).value) for c in cols) + "\n"
                )
    ...
    def generate_rsmeqp(self):
        ...
        with open(self.r_meqp, "w+", encoding="utf-8") as f:
            # header
            f.write(
                "\t".join(str(rsmeqp.cell(row=22, column=c).value) for c in range(3, 31)) + "\n"
            )
            # names
            ...
            for row_num in range(rstart, rend):
                f.write(
                    "\t".join(str(rsmeqp.cell(row=row_num, column=c).value) for c in range(3, 31))
                    + "\n"
                )
        ...
        if not os.path.isfile(self.c_tech_out):
            ...
            with open(self.c_tech_out, "w+", newline="") as f:
                ...
                for row_n, row in enumerate(ktekx.rows):
                    ...
                    if (row_n + 1) != ktekx.max_row or (
                        ((row_n + 1) == ktekx.max_row) and not all([x is None for x in row_vals])
                    ):
