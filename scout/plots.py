...
def run_plot(
    meas_summary, a_run, handyvars, measures_objlist, regions, codes_bps_objlist, trim_out
):
    # Set base directory
    ...
    years = list(
        a_run.output_all[meas_names[0]]["Markets and Savings (Overall)"][adopt_scenarios[0]][
    ...
    if regions == "AIA":
        ...
        # Set region legend names
        if (
            compete_results_agg["Output Resolution"] == "detail"
            or "reg" in compete_results_agg["Output Resolution"]
        ):
            emm_det_flag = True
        else:
            emm_det_flag = False
    ...
    cmc = mpl.colors.LinearSegmentedColormap.from_list("tab20_c", clr, len(czones_out))
    ...
    if (
        compete_results_agg["Output Resolution"] == "detail"
        or "bldg" in compete_results_agg["Output Resolution"]
    ):
        bclasses_out_finmets = [
            ...
            [
                "Hospitals (New)",
                "Large Offices (New)",
                "Small/Medium Offices (New)",
                "Retail (New)",
                "Hospitality (New)",
                "Education (New)Assembly/Other (New)",
                "Warehouses (New)",
                "Large Offices (Existing)",
    ...
    euses_out_finmets = [
        ["Heating (Equip.)", "Cooling (Equip.)", "Ventilation"],
    ...
    plot_axis_labels_ecm = [
        e_site_source + " (" + e_axis_units + ss_calc + append_txt,
    ...
    var_names_compete_base_m = [
        "Baseline Energy Use (MMBtu)",
        "Baseline CO\u2082 Emissions (MMTons)",
        "Baseline Energy Cost (USD)",
    ]
    var_names_compete_base_l = [
        "Baseline Energy Use (low) (MMBtu)",
        "Baseline CO\u2082 Emissions (low) (MMTons)",
        "Baseline Energy Cost (low) (USD)",
    ]
    var_names_compete_base_h = [
        "Baseline Energy Use (high) (MMBtu)",
    ...
    plot_names_agg = ["Total Energy Savings", "Total Avoided CO2", "Total Cost Savings"]
    ...
    plot_axis_labels_agg_ann = [
        "Annual " + e_site_source + " Savings (" + e_axis_units + ss_calc + append_txt,
        r"Annual Avoided CO$_2$ Emissions (" + c_axis_units + append_txt,
        "Annual Energy Cost Savings (" + cs_axis_units + append_txt,
    ]
    plot_axis_labels_agg_cum = [
        "Cumulative " + e_site_source + " Savings (" + e_axis_units + ss_calc + append_txt,
        r"Cumulative Avoided CO$_2$ Emissions (" + c_axis_units + append_txt,
        "Cumulative Energy Cost Savings (" + cs_axis_units + append_txt,
    ]
    # Set names of variables used to retrieve savings data to aggregate
    var_names_compete_save = [
        "Energy Savings (MMBtu)",
        "Avoided CO\u2082 Emissions (MMTons)",
    ...
    plot_axis_labels_finmets_x = [
        ...
        + ss_calc
        + append_txt,
        snap_yr + r" Avoided CO$_2$ Emissions (Competed, " + c_axis_units + append_txt,
        snap_yr + " Energy Cost Savings (Competed, " + cs_axis_units + append_txt,
    ]
    ...
    xlsx_names_finmets = [
        "IRR (%)," + snap_yr,
        "Payback (years)," + snap_yr,
        "CCE ($/MMBtu saved)," + snap_yr,
        "CCC ($/tCO2 avoided)," + snap_yr,
    ]
    ...
    def _plot_one_scenario(a):
        ...
        for v in range(len(var_names_uncompete)):
            ...
            xlsx_names_years = [str(yr) + " (" + var_units[v] + ")" for yr in years]
            ...
            xlsx_data = pd.DataFrame(
                columns=col_names_xlsx,
                index=range(3 + len(czones_out) + len(bclasses_out_agg) + 11 + len(meas_names) * 4),
            )
            ...
            if (var_names_uncompete[v] == "energy") or (var_names_uncompete[v] == "cost"):
            ...
            results_agg[0, 0:3] = [
                czones_out,
                [[0] * len(years) for _ in range(len(czones_out))],
                czones_out_col,
            ]
            results_agg[1, 0:3] = [
                bclasses_out_agg,
                [[0] * len(years) for _ in range(len(bclasses_out_agg))],
                bclasses_out_agg_col,
            ]
            results_agg[2, 0:3] = [
                euses_out_agg,
                [[0] * len(years) for _ in range(len(euses_out_agg))],
                euses_out_agg_col,
            ]
            ...
            results_finmets = numpy.empty(
                (len(meas_names_no_all), len(fin_metrics) + 4), dtype=object
            )
            ...
            for axa, m in zip(fig.axes, range(len(meas_names))):
                ...
                if meas_names[m] == "All ECMs":
                    ...
                    row_ind_start = 5 + len(czones_out) + len(bclasses_out_agg) + 11 + (m - 1) * 4
                # Add measure name to Excel sheet
                xlsx_data.iloc[row_ind_start : (row_ind_start + 4), 0] = meas_names[m]
                ...
                if meas_names[m] == "All ECMs":
                    ...
                    czones = ", ".join(
                        [
                            str(elem)
                            for elem in (
                                compete_results[1][meas_names[m]]["Filter Variables"][
                                    "Applicable Regions"
                                ]
                            )
                        ]
                ...
                xlsx_data.iloc[row_ind_start : (row_ind_start + 4), 3] = bldg_types
                ...
                if meas_names[m] == "All ECMs" or any(
                    ...
                    results_database_finmets = compete_results[1][meas_names[m]][
                        "Financial Metrics"
                    ]
                ...
                for cp in range(len(comp_schemes)):
                    ...
                    xlsx_data.iloc[
                        (row_ind_start + (cp) * 2) : (row_ind_start + (cp) * 2 + 2), 1
                    ] = ["Baseline " + comp_schemes[cp], "Efficient " + comp_schemes[cp]]
                    ...
                    if comp_schemes[cp] == "uncompeted" and (
                        ...
                        and not any([x in meas_names[m] for x in ["Codes", "Standards"]])
                    ):
                        ...
                        if var_names_uncompete[v] != "cost":
                            results_database = uncompete_results[uc_name_ind]["markets"][
                                adopt_scenarios[a]
                            ]["uncompeted"]["master_mseg"][var_names_uncompete[v]]["total"]
                        else:
                            results_database = uncompete_results[uc_name_ind]["markets"][
                                adopt_scenarios[a]
                            ]["uncompeted"]["master_mseg"][var_names_uncompete[v]]["energy"][
                                "total"
                            ]
                        ...
                        for yr in range(len(years)):
                            r_temp[0:3, yr] = results_database["baseline"][str(years[yr])]
                            # Set mean, low, and high values for case with
                            # ECM input/output uncertainty
                            if len([results_database["efficient"][str(years[0])]]) > 1:
                                # Take mean of list of values from uncompeted
                                # results
                                r_temp[3, yr] = numpy.mean(
                                    results_database["efficient"][str(years[yr])]
                                )
                                # Take 5th/95th percentiles of list of values
                                # from uncompeted results
                                r_temp[4:6, yr] = numpy.quantile(
                                    results_database["efficient"][str(years[yr])], [0.05, 0.95]
                                )
                                uncertainty = True
                                ...
                                r_temp[3:6, yr] = results_database["efficient"][str(years[yr])]
                    ...
                    # Find data for competed energy, carbon, and/or cost
                        ...
                        if meas_names[m] == "All ECMs":
                            results_database = compete_results[0][meas_names[m]][
                                "Markets and Savings (Overall)"
                            ][adopt_scenarios[a]]
                        else:
                            results_database = compete_results[1][meas_names[m]][
                                "Markets and Savings (Overall)"
                            ][adopt_scenarios[a]]
                        # Set the appropriate database of ECM competed energy,
                        ...
                        if meas_names[m] != "All ECMs":
                            ...
                            results_database_agg = compete_results[1][meas_names[m]][
                            ...
                            results_database_filters = compete_results[1][meas_names[m]][
                                "Filter Variables"
                            ]
                        ...
                        _base_has_uncertainty = (
                            numpy.array(
                                numpy.where(
                                    base_uncertain_check[:, 0] & base_uncertain_check[:, 1] is True
                                )
                            ).size
                        ) > 0
                        _eff_has_uncertainty = (
                            numpy.array(
                                numpy.where(
                                    eff_uncertain_check[:, 0] & eff_uncertain_check[:, 1:2] is True
                                )
                            ).size
                        ) > 0
                        for yr in range(len(years)):
                            if _base_has_uncertainty:
                                ...
                                # results
                                r_temp[1, yr] = results_database[var_names_compete_base_m[v]][
                                    str(years[yr])
                                ]
                                # Take 'low' value output directly from
                                # competed results (represents 5th percentile)
                                r_temp[2, yr] = results_database[var_names_compete_base_l[v]][
                                    str(years[yr])
                                ]
                                # Take 'high' value output directly from
                                # competed results (represents 5th percentile)
                                r_temp[3, yr] = results_database[var_names_compete_base_h[v]][
                                    str(years[yr])
                                ]
                                ...
                                # Take 'low' value output directly from
                                # competed results (represents 5th percentile)
                                r_temp[0:3, yr] = results_database[var_names_compete_base_m[v]][
                                    str(years[yr])
                                ]
                            ...
                            if _eff_has_uncertainty:
                                # Take mean value output directly from competed
                                # results
                                r_temp[4, yr] = results_database[var_names_compete_eff_m[v]][
                                    str(years[yr])
                                ]
                                # Take 'low' value output directly from
                                # competed results (represents 5th percentile)
                                r_temp[5, yr] = results_database[var_names_compete_eff_l[v]][
                                    str(years[yr])
                                ]
                                # Take 'high' value output directly from
                                # competed results (represents 95th percentile)
                                r_temp[6, yr] = results_database[var_names_compete_eff_h[v]][
                                    str(years[yr])
                                ]
                                # Flag output uncertainty in the current plot
                                uncertainty = True
                                ...
                                r_temp[3:6, yr] = results_database[var_names_compete_eff_m[v]][
                                    str(years[yr])
                                ]
                            ...
                            if meas_names[m] != "All ECMs":
                                ...
                                for fv in range(results_agg.shape[0]):
                                    ...
                                    for levone in range(len(results_agg[0, 0])):
                                        ...
                                        for levtwo in range(len(results_agg[1, 0])):
                                            ...
                                            for levthree in range(len(results_agg[2, 0])):
                                                # Set the end use name to use
                                                # in retrieving data values
                                                euse = results_agg[2, 0][levthree]
                                    ...
                                    for fvo in range(len(fv_opts)):
                                        results_agg[fv, 1][fvo][yr] = (
                                            results_agg[fv, 1][fvo][yr] + add_val[fvo]
                                        )
                                ...
                                if str(years[yr]) == snap_yr and not any(
                                    ...
                                    # to XLSX sheet
                                    xlsx_data.iloc[
                                        row_ind_start : (row_ind_start + 4),
                                        5 : (5 + len(plot_title_labels_finmets)),
                                    ...
                                    results_finmets[(m - 1), len(fin_metrics)] = (
                                        results_database[var_names_compete_save[v]][str(years[yr])]
                                        * unit_translate
                                    )
                                    ...
                                    bldg_match = numpy.empty(
                                        (len(bclasses_out_finmets) * len(bclasses_out_finmets[0])),
                                        dtype=object,
                                    )

                                    for b in range(len(bclasses_out_finmets)):
                                        if (
                                            sum([item in bclasses_out_finmets[b] for item in bldg])
                                            > 0
                                        ):
                                            bldg_match[b] = b
                                    ...
                                    if pd.Series(bldg_match).nunique() >= 1:
                                        ...
                                        results_finmets[(m - 1), (len(fin_metrics) + 2)] = (
                                            bclasses_out_finmets_shp[
                                                [
                                                    x
                                                    for x in list(pd.Series(bldg_match))
                                                    if x is not None
                                                ][0]
                                            ]
                                        )
                                    ...
                                    if pd.Series(euse_match).nunique() >= 1:
                                        results_finmets[(m - 1), (len(fin_metrics) + 3)] = "#7f7f7f"
                                    else:
                                        results_finmets[(m - 1), (len(fin_metrics) + 3)] = (
                                            euses_out_finmets_col[
                                                [
                                                    x
                                                    for x in list(pd.Series(euse_match))
                                                    if x is not None
                                                ][0]
                                            ]
                                        )
                ...
                base_uc = [
                    y * unit_translate if y else 0
                    for y in [[] if x is numpy.NaN else x for x in [results[0, 0]]][0]
                ]
                base_c_m = [
                    y * unit_translate
                    for y in [[] if x is numpy.NaN else x for x in [results[0, 2]]][0]
                ]
                base_c_l = [
                    y * unit_translate
                    for y in [[] if x is numpy.NaN else x for x in [results[1, 2]]][0]
                ]
                base_c_h = [
                    y * unit_translate
                    for y in [[] if x is numpy.NaN else x for x in [results[2, 2]]][0]
                ]
                ...
                eff_uc_m = [
                    y * unit_translate if y else 0
                    for y in [[] if x is numpy.NaN else x for x in [results[0, 1]]][0]
                ]
                eff_uc_l = [
                    y * unit_translate if y else 0
                    for y in [[] if x is numpy.NaN else x for x in [results[1, 1]]][0]
                ]
                eff_uc_h = [
                    y * unit_translate if y else 0
                    for y in [[] if x is numpy.NaN else x for x in [results[2, 1]]][0]
                ]
                eff_c_m = [
                    y * unit_translate
                    for y in [[] if x is numpy.NaN else x for x in [results[0, 3]]][0]
                ]
                eff_c_l = [
                    y * unit_translate
                    for y in [[] if x is numpy.NaN else x for x in [results[1, 3]]][0]
                ]
                eff_c_h = [
                    y * unit_translate
                    for y in [[] if x is numpy.NaN else x for x in [results[2, 3]]][0]
                ]
                ...
                xlsx_data.iloc[
                    row_ind_start : (row_ind_start + 4),
                    (5 + (len(plot_title_labels_finmets))) : len(xlsx_data.columns),
                ...
                try:
                    ...
                    continue
                axa.set_ylim(min(ylims) - (0.1 * max(ylims)), max(ylims) + (0.1 * max(ylims)))
                ...
                # carbon, or cost totals
                axa.plot(years, base_uc, color=plot_col_uc_base, label="", lw=5)
                ...
                if meas_names[m] == "All ECMs":
                    ...
                    for s in range(len(snap_yr_set)):
                        ...
                        yval_snap_eff = eff_c_m[
                            numpy.where(numpy.array(years) == int(snap_yr_set[s]))[0][0]
                        ]
                        yval_snap_base = base_c_m[
                            numpy.where(numpy.array(years) == int(snap_yr_set[s]))[0][0]
                        ]
                        ...
                        axa.plot(
                            [xval_snap, xval_snap], [yval_snap_base, yval_snap_eff], **marker_style
                        )
                        axa.text(
                            x=xval_snap - xval_snap * 0.002,
            ...
            for axb, f in zip(fig.axes, range(results_agg.shape[0])):
                ...
                min_max_ann = numpy.zeros([len(results_agg[f, 1]), 2], dtype=object)
                ...
                for cat in range(len(results_agg[f, 1])):
                    ...
                    # ranking purposes
                    total_ranks[cat] = numpy.multiply(
                        numpy.array(results_agg[f, 1][cat]), unit_translate
                    )[-1]
                    # Record min./max. y values for category
                    min_max_ann[cat, 0:2] = [
                        min(numpy.multiply(numpy.array(results_agg[f, 1][cat]), unit_translate)),
                        max(numpy.multiply(numpy.array(results_agg[f, 1][cat]), unit_translate)),
                    ]
                ...
                min_val_ann = min([x[0] for x in min_max_ann] + [min(total_ann)])
                max_val_ann = max([x[1] for x in min_max_ann] + [max(total_ann)])
                # Force very small negative min. y value to zero
                if (max_val_ann > 0) and ((abs(min_val_ann) / max_val_ann) < 0.01):
                    min_val_ann = 0
                ...
                if any([not math.isclose(x, 0, abs_tol=1e-9) for x in total_cum]):
                ...
                if f == 0:
                    ...
                    # row/column range
                    xlsx_data.iloc[
                        agg_row_ind, (5 + (len(plot_title_labels_finmets))) : len(xlsx_data.columns)
                    ] = total_ann
                ...
                for catnm in range(len(results_agg[f, 1])):
                    ...
                    if results_agg[f, 1][catnm][-1] != 0:
                        axb.plot(
                            years,
                            numpy.multiply(numpy.array(results_agg[f, 1][catnm]), unit_translate),
                            color=results_agg[f, 2][catnm],
                            lw=3,
                        )
                    ...
                    xlsx_data.iloc[agg_row_ind, col_active] = results_agg[f, 0][catnm]
                    # Set inactive filter variable values to 'All'
                    xlsx_data.iloc[agg_row_ind, col_inactive] = ["All", "All"]
                ...
                legend_entries = ["Total (Annual)", "Total (Cumulative)"] + [
                    x
                    for r, x in sorted(zip(total_ranks, results_agg[f, 0]), reverse=True)
                    if r != 0
                ]
                # Set legend colors
                color_entries = ["#4d4d4d", "#7f7f7f"] + [
                    x
                    for r, x in sorted(zip(total_ranks, results_agg[f, 2]), reverse=True)
                    if r != 0
                ]
                # Set legend linestyles
                lty_entries = ["solid", "dotted"] + ["solid"] * len(color_entries[2:])
                ...
                for i in range(len(legend_entries)):
                    plotsaxb += axb.plot(
                        [], [], color=color_entries[i], lw=lw_entries[i], linestyle=lty_entries[i]
                    )
            ...
            for axc, fmp in zip(fig.axes, range(len(fin_metrics))):
                ...
                sorted_ind = sorted(range(len(s_x)), key=lambda k: s_x[k], reverse=True)
                ...
                final_index_non_na = [
                    ...
                    if (
                        (s_y[i] > -500)
                        and (s_y[i] < 500)
                        and not math.isclose(s_x[i], 0, abs_tol=1e-9)
                    )
                ]
                # Indices of sorted x data constrained to points where
                # associated y data are within pre-defined range for plots
                ...
                if fmp == 0:
                    final_index_ce = [
                        i for i in final_index_non_na if (s_y[i] >= plot_ablines_finmets[fmp])
                    ]
                else:
                    final_index_ce = [
                        i for i in final_index_non_na if (s_y[i] <= plot_ablines_finmets[fmp])
                ...
                # parameters
                results_sort_x, results_sort_y, results_sort_pch, results_sort_bg = [
                ...
                meas_names_sort = [meas_names_no_all[i] for i in final_index_ce]
                ...
                for mn in range(len(meas_names_lgnd)):
                    meas_names_lgnd[mn] = str(mn + 1) + " " + meas_names_lgnd[mn]
                ...
                if len(results_sort_x) > 0:
                    ...
                    overlap_chk = [
                        (x, y)
                        for x, y in zip(results_sort_x, results_sort_y)
                    ...
                    if len(overlap_chk) > 0:
                        ylim_fm = pretty(min(ylim_fm), (max(ylim_fm) + (0.5 * max(ylim_fm))), 10)
                    ...
                    axc.set_xlim(min(xlim) - buff_x, max(xlim) + buff_x)
                    axc.set_ylim(
                        min(ylim_fm) - max(ylim_fm) * 0.1, max(ylim_fm) + max(ylim_fm) * 0.1
                    )
                    ...
                    # threshold
                    axc.axhline(
                        y=plot_ablines_finmets[fmp], color="black", linestyle="dotted", zorder=1
                    )
            ...
            for i in range(len(bclasses_shp)):
                plots1.append(axc.scatter([], [], color="black", marker=bclasses_shp[i]))
            # End use plots
            plots2 = []
            for i in range(len(euses_col)):
                plots2.append(axc.scatter([], [], marker="o", color=euses_col[i]))
            ...
            xlsx_var_name_list.append(
                xlsx_data.iloc[2 : xlsx_data.shape[0], 0 : len(xlsx_data.columns)]
            )
        # Write out Excel results
        writer = pd.ExcelWriter(xlsx_file_name, engine="xlsxwriter")
        for i in range(len(xlsx_var_name_list)):
            xlsx_var_name_list[i].to_excel(writer, sheet_name=file_names_ecms[i], index=False)
        writer.close()
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=len(adopt_scenarios)) as pool:
        futures = [pool.submit(_plot_one_scenario, a) for a in range(len(adopt_scenarios))]
