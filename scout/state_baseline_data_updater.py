...
def get_baseline_data_path():
    ...
    files = list(Path(fp.CONVERT_DATA).glob("EIA_State_Emissions_Prices_Baselines_*.csv"))
...
def generate_query_string(key, freq):
    ...
    query_params = {
        "source-disposition": {
            ...
            "frequency": freq[0],
            "data": [f"data[{i}]={field}" for i, field in enumerate(DATA_SERIES_DICT[key])],
            "sort": "&sort[0][column]=state&start=2015&sort[0][direction]=asc&offset=0&length=5000",
        },
...
def clean_source_disposition_data(data):
    ...
    df["total_disposition"] = df.apply(
        lambda x: x["total-net-generation"]
        + abs(x["net-interstate-trade"] + x["total-international-imports"])
        if x["net-interstate-trade"] < 0
        else x["total-net-generation"] + x["total-international-imports"],
        axis=1,
    )
    ...
    # direct use)
    df["TD_loss_factor"] = df["estimated-losses"] / (df["total_disposition"] - df["direct-use"])
...
def clean_emissions_data(data):
    ...
    df = pd.DataFrame(data)
    # select relevant columns
    df = df[df["fuelid"] == "ALL"][["period", "stateid", "co2-thousand-metric-tons"]]
...
def clean_retail_sales_data(data):
    ...
    df = df.pivot(
        index=["period", "stateid", "stateDescription"], columns="sectorName", values="price"
    ).reset_index()
    # select relevant columns
    df.columns = ["period", "stateid", "stateDescription", "commercial", "residential"]
...
def clean_and_aggregate_data(data_dict, year):
    ...
    df["co2_int"] = df["co2_mmt"] / (df["generation_twh"] * (1 - df["TD_loss_factor"]))
    ...
    df = df[
        [
            ...
            "Total Disposition (MWh)",
            "Direct Use (MWh)",
            "Estimated Losses (MWh)",
            "T&D Loss Fraction",
            "Emissions Rate (Mt/TWh)",
        ]
    ]
...
def main():
    ...
    parser.add_argument(
        "-y", "--year", choices=VALID_UPDATE_YEARS, help="Desired year to update state-level data"
    )
    parser.add_argument(
        "-o", "--overwrite", action="store_true", help="Overwrite existing state baseline data?"
    )
    ...
    for key in gas_prices.keys():
        ...
        gas_prices_init = [
            (x["duoarea"], x["value"])
            for x in api_query(gas_prices[key], api_key)
            if x["period"] == year
        ]
    ...
    if baseline_data_path:
        output_path = (
            str(baseline_data_path.parent) + f"/EIA_State_Emissions_Prices_Baselines_{year}.csv"
        )
    else:
        print(
        ...
        output_path = str(fp.CONVERT_DATA / f"EIA_State_Emissions_Prices_Baselines_{year}.csv")
    if overwrite == "y":
