$(document).ready(function(){
	...
	$.getJSON('data/site_source_co2_conversions-2015.json', function(data){
	...
	var fuel_type_values = ['electricity', 'natural gas', 'distillate', 'solar', 'other fuel', 'other fuel'];
	...
	var heating_equip = ['Resistance Heating', 'Air-Source Heat Pump', 'Ground-Source Heat Pump', 'Furnace', 'Boiler', 'Heat Pump', 'Furnace', 'Boiler', 'Furnace (Kerosene)', 'Furnace (LPG)', 'Wood Stove', 'Resistance'];
	var heating_equip_values = ['resistance heat', 'ASHP', 'GSHP', 'furnace (NG)', 'boiler (NG)', 'NGHP', 'furnace (distillate)', 'boiler (distillate)', 'furnace (kerosene)', 'furnace (LPG)', 'stove (wood)', 'resistance'];
	...
	var com_elec_cooking_equip_values = ['Range, Electric-induction, 4 burner, oven, ', 'Range, Electric, 4 burner, oven, 11-inch gr'];
	var com_ng_cooking_equip_values = ['Range, Gas, 4 powered burners, convect. ove', 'Range, Gas, 4 burner, oven, 11-inch griddle'];
	...
	$(document).on('change', '#fuel-type', function(){
		...
		if ($('input[name=bldg-class]:checked').val() === 'residential') {
			if (selected_end_use === 'heating') {
				if (the_fuel === 'electricity') {var eq_select = heating_equip_el;}
				...
				if (the_fuel === 'electricity') {var eq_select = cooling_equip_el;}
...
////////////////////////////////////////////////////////////////////////////////
		...
		// commercial - PCs, non-PCs
			...
			if (/^(heating|cooling|secondary heating)$/.test($('#end-use').val())) {
				...
				if ($('input[name=eq-env]:checked').val() === undefined) { // supply/demand selection
					...
					else if (!(/^(electricity|natural gas|distillate)$/.test($('option:selected', '#fuel-type').val()) && $('#end-use').val() === 'secondary heating')) {
				...
				if (selected_end_use === 'heating' || selected_end_use === 'secondary heating' || selected_end_use === 'cooling') {
					...
					if (ft_only_sel === 'electricity') {
						...
						for (var a = 0; a < climate_zone.length; a++) {
							// Loop over all building types selected
							for (var i = 0; i < selected_buildings.length; i++) {
								amtToAdd = data[climate_zone[a]][selected_buildings[i]]['electricity'][selected_end_use]['solar WH']['energy'][proj_year] * energy_conv;
								total_energy += amtToAdd;
								total_co2 += amtToAdd/1e9 * co2_conv;
							}
						}
					...
					// [climate zone][building type]['electricity'][end use][tech type]['energy'][year]
					...
					// [climate zone][building type]['electricity'][end use]['energy'][year]
				...
				if (selected_end_use === 'heating' || selected_end_use === 'cooling') {
					...
					if (radio_selection === 'supply') {
						...
						for (var a = 0; a < climate_zone.length; a++) {
							...
							for (var i = 0; i < selected_buildings.length; i++) {
								// Loop over all tech types selected
								for (var j = 0; j < hvac_tt.length; j++) {
									amtToAdd = data[climate_zone[a]][selected_buildings[i]][the_fuel][selected_end_use][radio_selection][hvac_tt[j]]['energy'][proj_year]/1e6 * energy_conv;
									total_energy += amtToAdd;
						...
						for (var a = 0; a < climate_zone.length; a++) {
							...
							for (var k = 0; k < ft_select_f.length; k++) {
								...
								for (var i = 0; i < selected_buildings.length; i++) {
									// Loop over all tech types selected
									for (var j = 0; j < hvac_tt.length; j++) {
										amtToAdd = data[climate_zone[a]][selected_buildings[i]][com_fuel_type_values[ft_select_f[k]]][selected_end_use][radio_selection][hvac_tt[j]]['energy'][proj_year]/1e6 * energy_conv;
										total_energy += amtToAdd;
										total_co2 += amtToAdd/1e3 * co2_conv;
									}
								}
					...
					for (var a = 0; a < climate_zone.length; a++) {
						...
						for (var i = 0; i < selected_buildings.length; i++) {
							// Loop over all tech types selected
							for (var j = 0; j < hvac_tt.length; j++) {
								amtToAdd = data[climate_zone[a]][selected_buildings[i]][the_fuel][selected_end_use][hvac_tt[j]]['energy'][proj_year]/1e6 * energy_conv;
								total_energy += amtToAdd;
								total_co2 += amtToAdd/1e3 * co2_conv;
							}
						}
					...
					else if (ft_only_sel === 'natural gas') {var cook_tt = com_ng_cooking_equip_values;}

					// Loop over all climate zones selected
					for (var a = 0; a < climate_zone.length; a++) {
						// Loop over all building types selected
						for (var i = 0; i < selected_buildings.length; i++) {
							// Loop over the two cooking technology types (users cannot select specific commercial cooking types, only the fuel used)
							for (var j = 0; j < 2; j++) {
								amtToAdd = data[climate_zone[a]][selected_buildings[i]][ft_only_sel][selected_end_use][cook_tt[j]]['energy'][proj_year]/1e6 * energy_conv;
								total_energy += amtToAdd;
								total_co2 += amtToAdd/1e3 * co2_conv;
							}
						}
              ...
              for (var j = 0; j < other_tt.length; j++) {
                for (var q = 0; q < com_lighting_indices[other_tt[j]].length; q++) {
                  ...
                  total_co2 += amtToAdd/1e3 * co2_el[proj_year];
					...
					// [climate zone][building type]['electricity']['MELs'][MEL type]['energy'][year]
  ...
  function errorHandler(txt, cz_flag) {
	...
	function primaryEnergyConversion(fuel_type, year) {
		...
		if (fuel_type === 'electricity') { return ss_el[year]; }
		else { return 1; }
	}
